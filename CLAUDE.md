...
- **sim/map.ts** — seeded deterministic FRACTAL map generation (`generateMap`): landmass budget, archetype rolls (blob/ridge/archipelago), placement, and the navigability-grid validator; islands never travel on the wire, both sides rebuild the identical polygon coastlines from the seed.
- **sim/islandShape.ts** — the fractal landmass shape builder: skeleton → capsule samples → periodic-midpoint-displacement boundary polygon (`buildIsland`). Owns the star-shape/concavity clamp.
...
- **Fractal islands (cycle 51, Eric ruling 2026-08-05)** — islands are TRUE POLYGON coastlines, generated fractally; the circle era is over. Each landmass is a 1-3 point **skeleton** (1pt = blob, 2-3pt = ridge) wrapped in a boundary polygon whose outward offset comes from periodic midpoint displacement clamped to `[0.45, 1.6]`. That clamp keeps the polygon **star-shaped about its skeleton**, which is what makes push-out escape structural (no concave stuck-traps) and makes an intra-island lagoon impossible — both invariants fall out of the clamp rather than needing separate enforcement. `Island` still carries `{x,y,r}` as its **bounding circle**, so it stays structurally assignable to `Circle`; every consumer goes through the `shared/src/sim/island.ts` seam — bounding-circle broadphase first, exact polygon second. All polygon math delegates to `silhouette.ts` — there is exactly one polygon library, never a second. Archipelago channels come from **gaps between polygons**, never from a concavity inside one. A single 32u navigability grid, flood-filled from the spawn ring, enforces three invariants at once: no enclosed lagoons, no unreachable water, no channel narrower than the widest hull. Spawn-ring clearance (64u) is the fourth, checked separately. Land coverage lands in the ratified 3-5% band (measured 4.0-4.8%); landmass count runs 9 at playerCap 2, rising to ~44 at playerCap 20. `PROTOCOL_VERSION` 25 → 26 because the same seed now builds a different ocean — an un-bumped old client would desync catastrophically.
