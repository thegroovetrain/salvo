...
// amendments 62-70 + 63: a radar paint stops being a hull silhouette and becomes
...
//     amendment 72 requires it: a presentation knob in CLIENT_CONFIG, never a
//     new constant in shared `CONFIG.vision`.
//
//   • THE SEEDED BLOB (amendment 70). Jitter derives from (track id, paint
...
//     off a shape, which is the whole reversal amendment 62 ordered.
//
//   • ISLAND RETURNS (amendment 69, ruling R4). Islands are already client-known
...
//
//   • THE GARMIN ECHO SCALE (amendment 74). `returnStrength` reduces an echo to
 ...
 * exactly the collision the three-channel split (amendment 64) exists to avoid —
 ...
 *
 * `across` is the information channel (amendment 66: hull geometry × relative
 ...
 * size — a second collision of the kind amendment 64 exists to prevent. Strength
 ...
 *
 * Weak → strong runs blue → green → yellow → red (amendment 74). Stops must be
 ...
 * amendment 70's exact contract — stable while it decays, different next sweep.
 *
