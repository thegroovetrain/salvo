...
//                     "denied pulses" plural), plus the HP GLOBE's pulse in its
...
//   Tier 3 — ECONOMY  The XP STRIP's bank-chip breath. Freezes at its DIM
//                     keyframe under ANY higher tier — including Tier 2 alone.
//                     The asymmetry
...
//                     the XP STRIP's track does not animate, so they have nothing
//                     to freeze rather than an invented animation.
//
// THE AMBER COROLLARY. Two amber channels can be live at once — the ring's
// final-10s pulse (Tier 2) and the HP GLOBE's amber band (25-50%, untiered by
// amendment 239). Only the highest-tier active amber pulses, so the ring wins
// and the amber globe holds lit (`amberPulseWinner`).
//
...
// it only COMPOSES the predicates their owning modules export (the HP globe's
// gates from render/hpGlobe.ts, the denied pulse's liveness from
...
import { railCritical } from './hpGlobe.js';
 ...
 *
 * The HP condition is the CRIMSON band — the globe's own `railCritical` gate
 ...
 * not a threat. The globe's DISPLAY grammar is untouched — it still breathes
 ...
 * amber HP channel that was Tier 1 would outrank the final-10s ring in every
 * wounded endgame, which is the exact opposite of the corollary's purpose.
 */
 ...
 * resolver. The rank is `CLIENT_CONFIG.attention.amberRank` (the ring, then the
 * HP channel).
 *
 * Read as design: at 40% hull with the storm closing in 8 seconds, the STORM is
 * what kills you, so the ring pulses and the amber globe holds lit. Below 25%
 * the globe turns crimson — it stops being an amber channel at all and becomes
