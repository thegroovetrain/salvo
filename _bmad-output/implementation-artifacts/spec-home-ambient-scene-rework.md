...
- Given the scene is running, when hulls are under way, then each lays a real wake through `WakeSources`/`Effects` on the water AND the radar paints the corresponding wake returns (`setWakeSources` wired).
