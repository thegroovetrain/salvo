...
- **Class loadouts:** TB = torpedo tubes + activated speed boost; BB = long-range cannon + star shells (fog-illuminating); ML = proximity mines + a signature ability that was OPEN and must be resolved with Eric before implementation.
...
- **Firing arcs:** all weapons fire within real arcs; mouse aim is constrained to the selected weapon's arc, which renders exactly as the weapon fires; per-weapon arc geometry is redesigned for the class era (usable in more situations while still rewarding positioning).
...
- **Decisions requiring Eric before/within their story:** precision bonus (1.4), boost × torpedo self-hit rule (1.6), Mine Layer signature ability + mine mechanics (1.8), and per-weapon firing-arc geometry (1.10 — arc values are design decisions). The GDD still marks per-weapon arc geometry as open design work "TBD alongside the new armory's numbers."
