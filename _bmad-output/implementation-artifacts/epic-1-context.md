...
Pick any of four classes and the game feels genuinely different at 0:00 — hull envelope, silhouette, personal color, fitted loadout, and the class-select experience. This epic replaces the three prototype classes with the beta's four (Torpedo Boat, Battleship, Mine Layer, Gunboat), unifies all fitted systems under one Equipment interface with the universal slot grammar, lands fire-time latency compensation, and installs the signal-registry perception foundation that every later epic's fog-of-war features build on. Class identity and the loadout spine come first in the sequence because everything after (economy, texture, modes) touches them.
...
**Classes & loadouts.** Four classes, each a distinct hull envelope (size/speed/toughness/turning) with a fitted loadout. Universal slot grammar: slot 1 = universal standard gun (byte-identical on every class — differentiation never lives in the gun), slots 2–3 = two class specials (at least one a weapon), slot 4 = extra slot filled mid-match by the upgrade economy (empty-capable plumbing only in this epic). Loadouts: Torpedo Boat = torpedo tubes + smoke screen; Battleship = long-range cannon + star shells; Mine Layer = proximity mines + decoy buoy; Gunboat = AP punch (form open) + speed boost.

**Combat model.** Projectiles only, never hitscan: no dispersion, no damage falloff, flat single-pool damage — no sectional damage, no crits, no weak points, no torpedo variety, no damage-control parties (hard scope vetoes). Shells fly with travel time to the clicked point or first obstacle. Torpedo laws: outrun every hull at base speed, real bow clearance + brief owner-only grace (self-hit at base speed must be impossible, by test), run until impact, never painted by radar. Mines arm after a delay, trigger by proximity, per-player live cap (oldest evicted) plus a global cap. The decoy buoy is a real server-side entity whose emitted signals are wire-indistinguishable from a genuine ship's — payload AND timing (same RNG/jitter stream).
...
- **Silhouette IS the hitbox.** Class silhouette geometry lives in shared/ so server collision and client render derive from the same source and can never disagree. Hull dims per the ratified board: TB ~9:1 at 100 u, BB 124 u, ML 88 u, GB 60 u. Decoupling render from hitbox is the named fallback, not the plan.
...
- **Ability effects go through shared paths.** The Gunboat speed boost applies via `effectiveStats()`/shared hooks so client prediction survives the speed change; boost state produces no wire field revealing it to enemies beyond observed kinematics.
- **Smoke screen is a server-side perception effect** (blocks truesight LOS through it for all observers) built on the registry; star-shell illumination is a registry row with its own invariant case, never revealing beyond the lit zone.
...
- **Tokens & type (1.11).** The ratified token set becomes the single styling source: surfaces, functional colors (phosphor family, amber, storm violet, `denied` #FF3B3B consolidating both legacy reds), combat-effect colors, drone greys, 20 Regatta player hues. No hex outside the token source; deprecated #111111/#232937 surfaces die. Geist display/body + Geist Mono for every label/readout/stat — uppercase letter-spaced labels, tabular-nums digits, no mono below 9 px post-scale. Styling-only story: no gameplay or wire change rides along.
...
- **Silhouettes & nameplates (1.13).** Four genuinely distinct top-down silhouettes, consistent everywhere a hull appears (water, class card, results); the legacy chevron is reserved for PvE drones — a fifth silhouette no player wears. Every truesight combatant hull wears a nameplate (hud-micro register, text-safe hue variant; drones tagged "DRONE"); never on blips or radar paints; fades with truesight resolution.
- **Home & class-select (1.14).** Home renders over a live ambient CIC canvas (never blank): wordmark, callsign field (14-char cap), Class Chip, Color Hoist (20 swatches + preference caption), amber SET SAIL button with mode/class sub-line, How-to-Play link, server status, settings gear. Class-select: four 356 px Class Cards on a horizontal rail (silhouette box, fantasy line, SPEED/TOUGHNESS/TURNING pips derived from the real CONFIG envelopes — not placeholders) plus a dashed "MORE CLASSES IN DEVELOPMENT" ghost card; keys 1–4/arrows highlight, Enter picks, ESC closes without change. Connection failure reports on the status line, never a dead screen.
...
- **1.1 (registry) is the foundation story** — the smoke screen (1.6), star shells (1.7), and especially the decoy's counterIntel row (1.8) build on it. Land it early.
- **1.2 (equipment/loadout) precedes all four loadout stories (1.6–1.9)**; 1.3 (envelopes) precedes 1.4 (gun), 1.13 (silhouettes), and 1.14 (class cards derive pip values from its CONFIG).
- **1.5's latency harness gates gun behavior** — 1.4 tuning and D1 are validated by its metrics.
- **1.10 (arcs) needs all eight fitted systems** from 1.4–1.9.
- **1.11 (tokens) precedes 1.12 (Hoist)**, which consumes the hue set.
- **Decisions to resolve WITH ERIC, in-story:** precision bonus adopt-or-drop (1.4), AP-gun form — separate gun vs activatable buff (1.9, before implementation), per-weapon arc values (1.10), Gunboat as first-run default class (1.14, flagged unconfirmed — confirm before ship).
- **Epic 2 boundary:** legacy upgrade economy and current bindings survive this epic (the interregnum); heal stays an open question — keep the boon mechanism and lifecycle heal-compatible without committing.
