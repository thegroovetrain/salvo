...
- 2026-08-02 (POST-LANDING Eric ruling R7, direct message; shipped as its own interstitial cycle 35 → 0.17.35 on branch `dev-auto-star-shell-radius-preview`): **star shells DO get the effect-radius preview.** This OVERRULES the implementer-drafted omission recorded in cycle 34's Auto Run Result ("star shells get a travel line but no circle — the flare is damageless, so it has no blast; surfaced for Eric"), which was the drafted call flagged for exactly this decision. Eric's reason: he wants to see the lit radius at the aim point in order to POSITION the illumination before spending a one-shot 20s-cooldown flare. Shipped as: a circle at the range/map-clamped burst point sized to the owner's EFFECTIVE `stats.starShells.litRadius` × the incendiary shrink (mirroring `equipment/starShells.ts` exactly), same island-clip/blocked semantics as the gun family it flies like, drawn in a new quieter EFFECT register (implementer-drafted `CLIENT_CONFIG.aimPreview.effectAlpha`/`effectFillAlpha`) because an illumination area is not a threat area and the lit radius is ~7× a gun blast. Client-only, zero wire changes.
