...
- **2026-09-16 — Eric ruling after the PR opened (epic-8 amendment 20), applied post-review.** Trigger: Eric, on reading the run report: *"If I did not DIRECTLY click on the mine, then UNDER NO CIRCUMSTANCES WHATSOEVER SHOULD IT BLOCK A SHOT, REGISTER A HIT OR MISS, OR GIVE ANY INDICATION WHATSOEVER TO THE SHOOTER THAT ANYTHING MIGHT BE THERE. AT ALL."* Amended: amendment 17 (in-flight point-target contact) is superseded in full — a mine is never a collision subject for a projectile in flight; only the burst at the clicked aim point covering the mine's centre detonates it. The intent-contract's ruling (17) text and orchestrator ruling 5's "a shell that contacts a mine is CONSUMED" are therefore void; the contract block is left unedited by rule and this entry overrides it. Known-bad state avoided: a shell dying on an unseen mine (any mark or silence either way still told the shooter something was there, and it blocked the shot). KEEP: the collector, the gate, burst-path detonation with cross-owner chains (18), captive/arming immunity (16), and the hc-on-wreck rule (19) are untouched; `sweepMask` strips `mine` from the sweep list only, so torpedo/flare masks keep their memoized identity.
...
**Amendment 20 pass (2026-09-16, commit `11e9692`):** the in-flight mine contact path is deleted end to end (`stepShells` sweeps `hitTargets(sweepMask(hits))`, which strips `mine`; shared `earliestTarget`/`steerHoming` skip `mine` as a belt-and-braces pin; `detonateShotMine` → `detonateBurstMine`, reached only from `resolveBurst`). New pins compare the shooter's frame-event stream tick by tick with and without a mine on the shell path (outside sight, inside sight, own mine): identical. Nine fail-first failures against the pre-change code; review pin P1 retired as moot. Gate on the orchestrator's own rerun: **847 / 1882 / 3269**, lint 0 errors, golden frames unchanged, PV 51; `weaponsSmoke` OK on a scratch port. Ruling (13) of the review list is void (no mine-consumed shell exists).
