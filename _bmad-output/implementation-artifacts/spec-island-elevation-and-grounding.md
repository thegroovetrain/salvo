...
**Pass 1 (2026-08-06) — two independent gates: Fable adversarial + Codex cross-model.**

Counts: intent_gap 0 · bad_spec 0 · patch 2 (1 high, 1 low) · defer 1 (low) · reject 0.

| # | Finding | Source | Verdict | Disposition |
|---|---------|--------|---------|-------------|
| 1 | **Push-out could translate a hull 209.7u in ONE TICK.** The push direction came from the hull CENTRE, but the hull can overlap a DIFFERENT coastal feature than the centre's nearest coast: bow inside an east-west arm while the centre's nearest boundary lies abeam east yields a normal pointing WEST, nearly PARALLEL to the penetrated coast — so the first clearing translation is only found where that arm ENDS. The existing anti-teleport cap did not bind because it scales with island radius (~300u), not per-tick motion. Measured ~1 visible teleport per 6,000 contact-ticks across all three hull classes; drones grind coasts constantly, so it fired in most matches and broke the ratified "islands block movement" promise. | Fable gate, reproduced deterministically (`generateMap(555555, 20)`, mineLayer at `(-1004.08, -202.34)` h=1.401) | CONFIRMED (high) | PATCHED at both halves of the cause — the normal is now aimed from the DEEPEST PENETRATING HULL VERTEX, and the applied translation is bounded by `|speed|*dt + beam` (prev is overlap-free by induction, so real penetration cannot exceed this tick's travel plus a rotation sweep; anything larger means travelling ALONG a feature, not escaping into one). No clearing translation within the bound => apply NOTHING and fall through to the 3-step rollback ladder. Post-fix: 16 seeds x 3 hulls, 364,000 ticks (123,501 in contact), **zero jumps >= 35u, worst case 16.5u**, zero ticks ending overlapped. NOTE the wave-2 ram-and-escape suite missed this entirely because it drives scripted head-on rams, never sustained PARALLEL coastal contact — the new regression test exercises that geometry and provably fails pre-fix. |
| 2 | Closure-filled lagoons read as sea level in the retained height raster: closure flips cells WATER->LAND in the MASK, but `buildHeightRaster` quantizes the FIELD, which closure never touches. A sealed lagoon sat inside a land polygon reading height 0, contradicting map.ts's own stated raster/coastline agreement. | Fable gate | CONFIRMED (low, latent) | PATCHED — the raster is stamped from the final sea-level mask, making `height > 0 iff the shipped mask says LAND` exact. Raising the FIELD instead was rejected: crossings interpolate field values, so it would move real coastline and feed back through every re-threshold pass, changing the shipped ocean. Harmless this cycle (nothing reads the raster) but it is the ratified radar-shadow substrate, so a future raymarch would have seen transparent water inside solid land. |
| 3 | The no-transcendentals source-scan tests do not read `silhouette.ts`, so its generation-path purity is unguarded against future edits. Verified correct TODAY by hand (`Math.sqrt` only on the gen path; `hypot`/`cos`/`sin` live only in non-generation functions). | Fable gate | PLAUSIBLE (low) | DEFERRED — a latent guard gap, not a live defect. Natural home is whichever cycle next touches `silhouette.ts` or the determinism suite. |

**Codex cross-model pass:** reviewed the same diff against branch point `70a6f25` (44 files, +5,547/-1,772) and returned *"I did not identify any discrete, actionable bugs introduced by this change set."* Honest characterisation of the agreement picture: this is a CLEAN PASS ON THE FIXED CODE, **not** independent corroboration of finding 1 — Codex was run after the fix commit and never saw the pre-fix state. Finding 1 was therefore a single-model finding, and it was accepted only because the Fable gate supplied a deterministic reproduction that the orchestrator confirmed, not on the reviewer's assertion alone.

**Verified sound under adversarial attack (no findings):** determinism (byte-identical SHA-256 across separate processes and `--jitless` over 8 seeds — full vertex/pole/core/contour/raster/pyramid fingerprint; server/client call-argument parity confirmed; the contour-flag fork was deliberately closed by always building contours on both sides); grounding-fix preservation (`polygonFitLimit`, boundary-press-is-not-contact, `headOn`, and the speed-cap damp all byte-untouched by the rewrite); navigability across an independent 400-seed sweep disjoint from the test suite (zero `MapGenerationError`, coverage [2.41%, 2.76%], min spawn clearance 104.9u vs 64u required, vertex hard cap held); `pole`/`core` migration completeness (every remaining `isle.x/y/r` consumer is broadphase-only or a render heuristic); contours render-only; and the perception/anti-cheat chokepoint (untouched, no new wire fields, `heightRaster` never serialized).
