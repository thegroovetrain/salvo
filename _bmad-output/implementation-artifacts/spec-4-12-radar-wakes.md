...
- [ ] `shared/src/__tests__/zone.test.ts` — ADD (do not edit the ladder pins) the assertion that the wake material's solved reference range puts its crossover exactly at `CONFIG.vision.muzzleFlash`, and that `wakeLifeMs` is a fixed literal rather than a computed quantity. — Amendment 203's reach is derived onto the ladder; an unpinned fit drifts silently.
...
- [ ] `client/src/render/radarField.ts` + `radarSources.ts` — give `stampCoverage` a material argument (it hardcodes `hullSample`), add `wakeSample` and `chopSample`, and insert both into `buildField`'s priority chain below surf and above clutter. Chop is a per-hull disc/ellipse synthesized at paint-creation time from a coverage mask or `Contact`, at the clutter coefficient scaled by speed fraction. — Amendment 202: wake carries information and comes off the wire; chop carries none and must not.
