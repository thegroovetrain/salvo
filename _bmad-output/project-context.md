...
- `PROTOCOL_VERSION` (`shared/src/index.ts`, currently 3) must be bumped on ANY wire-contract break (`shared/src/types.ts`).
...
- `npm run check` (lint + tsc for all three workspaces + all 653 tests) is the gate — it must pass before any ship.
