...
| **Color Hoist** | Row of 20px round swatches (the 20 Regatta hues), displayed in ascending HUE ANGLE so the row reads as a wheel (Eric ruling 2026-08-21; a presentation order derived from the hex values at render time — the wheel INDEX is identity and its array order is unchanged); selected swatch ringed. Caption: "PREFERENCE PICK — YOU GET IT UNLESS CLAIMED, THEN NEAREST FREE HUE." Must not imply claiming/locking. |
