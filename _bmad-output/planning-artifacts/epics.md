...
inputDocuments:
  ...
  - _bmad-output/planning-artifacts/gdds/gdd-Hullcracker.io-2026-07-16/decision-log.md
  - _bmad-output/forge/deck-model-v3/forged-idea.md
  - _bmad-output/forge/identity-fork/forged-idea.md
  - _bmad-output/implementation-artifacts/deferred-work.md
  - _bmad-output/planning-artifacts/ux-designs/ux-Hullcracker.io-2026-07-16/.decision-log.md
extensions:
...
FR13: **[REDUCED TO TWO TIERS 2026-08-21 (Story 7.6) — the third tier is DEFERRED, not cut.]** **Hull microphones / the listening ring were DEFERRED** (Eric ruling 2026-08-04, Story 4-1; re-affirmed 2026-08-21: *"very deferred. sonar might come back in the future, but radar is plenty deep enough"*) and never built. The **shipped** suite is TWO tiers, both scaled off one ruler (the eighths ladder, Story 4.9): a truesight bubble (live, LOS-clear contacts; `CONFIG.vision.sight` = 330 u) and a rotating radar sweep (`CONFIG.vision.radar` = `SIGHT * 2` = 660 u — DERIVED, never a literal) painting decaying phosphor blips, with height-aware terrain shadowing (Story 4.11) and radar wakes (Story 4.12). The bearing-grade audio channel the third tier was to carry is served in shipped play by the **foghorn chevron** (Story 4.5, epic-4 amendment 56 — bound to `F`). The old 220 u / 650 u reference figures are also stale. Original: Universal three-tier sensor suite on every hull: truesight bubble (live, LOS-clear contacts; reference 220 u), rotating radar sweep (reference 650 u / 4 s revolution) painting decaying phosphor blips, and hull microphones (listening ring — bearing-grade audio detection of engines, torpedoes, foghorns).
...
FR17: **[SUPERSEDED 2026-08-21 (Story 7.6) — Eric rulings 2026-08-10, epic-4 amendments 216-227.]** **The bloom clause is STRUCK**: there is no radar paint, bloom, ring, bearing, range or area disclosure of the kill leader, ever, and with it the *"one sanctioned non-sweep radar paint"* exception disappears (the master perception invariant still has exactly SIX declared exceptions). Shipped: the kill leader is disclosed by **IDENTITY ONLY** (`ArenaState.bountyId`) — information every client could already derive from the roster's per-player kill counts — the throne transfers only on a STRICT OVERTAKE of captain-only kills, sinking the holder is worth `CONFIG.bounty.killLevels` (+1) extra, and the player-facing name is **KILL LEADER**. Original: The Bounty: the kill leader periodically blooms on every player's radar at true position (the one sanctioned non-sweep radar paint) and is worth extra XP; the feed announces the active bounty.
...
**Epic 8 · The Deck (upgrades v3 + catalog v3) and Epic 9 · The Account — inventory added 2026-09-11**
...
- **Results show the FINAL LOADOUT only**; brought / drawn / taken is match-history content (E9). Supersedes GDD E8 story 9 and FR33's "accrued boons + last offer" as applied to v3.
- **ARMOR / SPEED / TURNING / DECK GUN start at Tier I EQUIPPED** (first card reads `I → II`); RADAR SWEEP and RELOAD have no base tier (cards I–V); no ladder gains a Tier VI. Corrects catalog R14's wording.
- **Deck NAMES are allowed**, chosen from a dropdown (Eric 2026-09-11) — REVERSES the forge/GDD "deck names rejected" entry.
- **COPIES are acquired per copy**, not a whole line per token (Eric 2026-09-11) — REVERSES the GDD's "a token unlocks a whole line"; `matchesToCatalog` is now measured over 114 cards, and **starter-deck QUANTITIES are to be re-cut by Eric (OPEN)**.
- **Ship Select and the deckbuilder are ONE screen**; the standalone `/decks` route and the `DECKS` home link are gone.
- **The dev/staging host is ACCOUNT-ONLY (FIRM).** **Anonymous play on PRODUCTION is OPEN — Eric is thinking it through; it must not be re-asked.**
- **Keyboard-only access is OUT OF SCOPE** (*"NO ONE is keyboard-only"*); the Windows Sticky Keys hazard on `Shift` is ACCEPTED and documented.
- Every number the catalog marks `[DRAFT]` is a CONFIG dial the batch-sim harness tunes once bots sail v3 decks — a target, never a contract; **an implementer may not invent a card, a number, a consumable or a mechanic**; every `[DRAFT]` is Eric's to confirm.

**Rulings Eric took 2026-09-11 while confirming this inventory (the step-1 walk):** (1) **NO FRIENDLY FIRE, ever** — your own ordnance never damages your own hull at any speed; the ONE exception is shooting your OWN mines to detonate them early; the "torpedoes outrun every hull" law is therefore RETIRED as a requirement (FR53). (2) The free per-level auto-heal STAYS as currently built, may be removed later (FR47). (3) Heals during the collapse: SETTLED — allowed; the card bound is the guard (FR47). (4) SHIELD BLOCK: a second shield REPLACES; the shooter gets NO tell; the owner sees the recoloured `HULL n/max` number ONLY (FR55, UX-DR46). (5) DECOY BUOY: radar PAINTS it; own torpedoes/missiles PASS THROUGH own decoy; own flak never touches own ordnance (FR55). (6) History shows only the pool cards you DREW (FR66). (7) Aimed-vs-instant consumables are learned from the hover tooltip and How-to-Play ONLY — no glyph (FR46, UX-DR43). (8) A door refusal renders in the QUEUE pop-up (UX-DR71). (9) All working words ratified EXCEPT the player-facing "starter deck", which becomes **`DEFAULT`** (`DEFAULT` deck name, `DEFAULT DECK` column head, `SAIL DEFAULT DECK` button; internal naming unchanged) (UX-DR77). (10) The ghost "MORE CLASSES IN DEVELOPMENT" card is DROPPED (UX-DR60). (11) BARREL's 12 u spacing stays as shipped for now (AR62). (12) **KEY REBINDING IS IN — Epic 9**: saved in localStorage when anonymous, as an account setting when signed in (FR72, UX-DR78). (13) Starter-deck COUNTS: Eric has already re-cut them and holds them; delivered to the implementing agent at the deck-rules story (FR56). (14) `matchesToCatalog`: still no number — Eric: *"It should take a while, but not too long"* (FR65). **Four more, from the party-mode review of the epic list the same day:** (15) **deck legality is checked BEFORE queueing — you cannot enter ANY mode with an illegal deck selected** (the mode buttons refuse on the home screen; the server's door check stays as the backstop, and because Solo vs AI has no queue pop-up, ruling 8's "queue pop-up" is superseded by this) (FR63, UX-DR71). (16) The How-to-Play re-cut (FR71) moves to **Epic 8**, where the features it explains live. (17) Epic 9 CLOSES with a **design & doc reconciliation story** (the Story 7.6 pattern) that writes the UX pass's "corrections owed upstream" back into `gdd.md`, `game-architecture.md` and the ledger (AR63). (18) Wording: Story 8.0 is a FLOOR story with no player-visible change (the Story 0.1 precedent), and Epic 8's "works without an account" is split into "needs nothing from Epic 9" (code) and "anonymous play on production is Eric's open question" (the door).

**The deck — model, legality, pool, draw (E8)**

FR41: A captain plays every match on a **deck of exactly 40 authored cards** (`CONFIG.deck.size`; final for launch — the forge's 25–40 band test is DROPPED, Eric 2026-09-03) belonging to one hull **as a LABEL, never a lock** (the catalog is hull-agnostic; nothing is class-locked). Legality has exactly TWO composition rules — exactly 40 cards, and **no more than three lines whose copy 1 fits an equipment slot** (`CONFIG.deck.maxEquipmentLines`) — plus two OWNERSHIP bounds (every card unlocked on the account; a line carries no more copies than its catalog cap). No other constraint exists: a pure gunboat deck and a zero-heal deck are both legal by design. The server checks legality **exactly once, at the door** (the standard queue's `onJoin`; the arena's `onJoin` for Solo vs AI, which has no queue) and freezes the deck there; edits after that apply to the next match. Starter decks are ordinary decks that pass the same rules against a fresh account's unlocks.
FR42: **Card model (A).** A catalog line is `{ id, kind: equipment | ladder | addon | consumable, cap, tiers[] }`. For an equipment line, **copy 1 IS the weapon at base, fitted into an empty weapon slot (tier I is the bare weapon — Eric's standing rule)** and copies 2–5 are authored tier bundles; **copies in the deck = the tier ceiling reachable that match**. Universal ladders are ladders on the same rule; **ARMOR / SPEED / TURNING / DECK GUN start at Tier I EQUIPPED** so the first card reads `I → II` and four copies reach V, while RADAR SWEEP and RELOAD have no base tier (cards I–V); nothing reaches VI (Eric 2026-09-11). **Add-ons** are one-copy cards drawable at any time, holdable AHEAD of the weapon they modify (*"it's a choice"*), targeting an explicit `appliesTo` list of equipment ids (a family by enumeration, never a family enum). The acquisition card and its "taking one burns the rest" rule, the rarity tiers and the rarity-weighted draw, and exclusivity are all RETIRED. The fold is order-independent by construction (pinned: any permutation of a legal pick list yields byte-identical stats).
FR43: **The match consumable pool** (Eric, catalog v3 R4/R44). Every match rolls **10 consumable cards** (`CONFIG.pool.size`) uniformly from the five launch consumables, each line ≤ its cap WITHIN THE POOL alone, on a ROOM-PRIVATE seeded stream (a per-room nonce never derivable from `mapSeed` — the storm-ring pattern), **once at room creation before any seat is taken, at both doors**, so every captain and bot receives the identical 10. The pool is **hidden from everyone** (never on the wire, never in `WelcomeMsg`; players know only that it exists), holds consumables only, and is appended to a deck ONLY AFTER that deck passed legality — a pool can never make a deck legal or illegal and never adds an equipment line. Deck at the door = 40 authored + 10 = 50. The pool reaches persistence only as `MatchRecord.pool` (Eric's metrics) and is logged as a COUNT, never contents. The batch-sim harness rolls the pool per match.
FR44: **Draw and offer.** Each banked level carries a pre-drawn offer of `CONFIG.offer.size` (4) DIFFERENT lines drawn from the player's own 50, **weight = copies remaining in the deck and nothing else** — no rarity weighting, no class weighting, no pity (the 2026-07-30 soft pity is retired). A line already at its cap on the ship is never offered. **A card leaves the deck when taken** — a consumable on pick, a ladder copy when fitted — and one level buys any one card (a heal is a card like any other). Reopening the refit window never rerolls; the countdown mulligan (FR48) is the ONE asserted exception. Exhaustion is legal but should be rare and means maximum power: an empty draw banks the level silently (no `pt`) and logs `deck.exhausted` once per ship. **There is NO draw-pile counter anywhere** — not on the HUD, the chrome bar or the refit window (Eric 2026-09-10; supersedes the GDD's *"Yes"*). The `Tab` offer is otherwise UNTOUCHED: no hand, no second clock, no storefront — merge, lock, sell, refresh, discard and reshuffle are all dead.

**Slots, consumables, heal, the opening (E8)**

FR45: **Slot grammar — one flat nine-slot loadout, identical for every hull at 0:00.** Slot 0 = the **slotless universal deck gun** (permanently selected, keyless); slot 1 = the **Shift boost** (FR49); slots 2–4 = **three GENERIC weapon slots on `Q`/`E`/`R`, EMPTY at 0:00**, each filled by the first EMPTY slot when a weapon line's copy 1 is drawn (no positional mounts, no per-slot arcs — every weapon carries its own fixed arc); slots 5–8 = **four consumable slots on `1`–`4`**. Because a deck holds ≤ 3 equipment lines the weapon slots can never overflow, so there is no "replace which" flow, no swap-out and no sell-back; the carried laws — **swap cheese is a NEVER** and **the slot keeps its clock across replacement** — survive as "fitting a weapon never touches another slot's timer". A weapon slot never empties; class identity leaves the loadout entirely and lives in the hull envelope and the deck's composition. The PvE fleet's `[gun, torpedo, mine]` fit becomes a fixed card list (`FLEET_FIT`: heavy torpedo + naval mines at tier I) applied at spawn through the ordinary card path.
FR46: **Consumables.** Taking a consumable from an offer STOCKS it in a slot (the slot already holding that line, else the first empty consumable slot) and it leaves the deck **on pick, not on use**; a stack is `{ n: copiesHeld }` with `maxAmmo` = the line's cap, and **consumables have NO reload, ever**. With every eligible slot full the card is GREYED in the offer by the same shared `canStock` predicate the server runs, and **the server refuses the pick as a silent no-op** (the card stays in the never-rerolling offer; no instant-use-on-pick; no `denied` view because a fair client cannot reach the case). Slot contents are **server-owned ship state**, so a refresh or reconnect keeps them for free. Two activation shapes on the unchanged Equipment interface: **key-fires** (HULL REPAIR, SHIELD BLOCK, SMOKE SCREEN, CHAFF — the ability channel, no aim) and **key-primes-then-click-fires** (DECOY BUOY — the click channel, the mine's rear arc); which shape a card is, a player learns from the hover tooltip and How-to-Play ONLY — no glyph on the face or slot (Eric 2026-09-11). A depleted slot clears to empty the same tick; consumables obey the existing single sinking-activation gate with no per-consumable policy; **no consumable ships without a bot use rule** (FR59).
FR47: **Heal is a card.** The `5` key, `HEAL_CHOICE` and the permanent DAMAGE CONTROL rail are RETIRED; the heal is the **HULL REPAIR** consumable — the shipped 100 hp effect unchanged (50 instant + 50 pooled at 5 hp/s), **afloat-only** (so a sinking hull can no longer heal and D4's reversibility question is not reopened), cap 5, five copies in every starter to start (harness-tuned per hull; the match pool may add up to five more). Heals are scarcer than today and never renewable: a captain holds at most four stocked and never more than the deck carries (≤ 10 under catalog v3), so **"heals during the collapse" is bounded by construction** — the sudden-death heal-hoard ceiling (NFR6) is re-derived on that bound, and **the long-open question is SETTLED (Eric 2026-09-11): heals stay allowed during the collapse; the card bound is the guard.** The FREE per-level auto-heal shipped at cycle 129 **STAYS AS CURRENTLY BUILT** under `CONFIG.damageControl` (Eric 2026-09-11: *"leave it as is for now"*; *"ultimately this may be removed"*) — described here by what the code on `development` does (10 % of missing hull over 5 s on each level earned); Eric recalls a passive 1 %/s out-of-combat version on dev, which is NOT on `development` at this writing — a discrepancy to check before the story that touches it.
FR48: **The opening.** Every hull spawns with the deck gun and the Shift boost ONLY — nothing from the deck. **Level zero is granted at countdown start** (before `activate()`, to every captain and bot alike) so the first offer opens during the 10 s countdown and may be taken before live or held. The **weighted first draw** (the default, built first) guarantees the level-zero offer contains ≥ 1 ACTIVELY USABLE card — a consumable, or any equipment's tier I — by drawing the first card uniformly among usable cards and the rest from everything else; a legal deck with no usable card (a pure gunboat deck) makes the guarantee vacuous and the draw plain — pinned so nobody "fixes" it into a reroll. **The mulligan is ONE free redraw of the level-zero offer, countdown only** — a spend sentinel (`MULLIGAN_CHOICE`) honoured iff the match is in countdown, the ship has not yet mulliganed and holds an offer; it redraws with the same guarantee and re-queues the offer; a second mulligan or a live-phase mulligan is a silent no-op that leaves the offer byte-identical; FR19's never-reroll pin gains exactly this one asserted exception; bots never redraw. A **pinned-card spawn** (one deck card start-fitted) is a later CONFIG-gated experiment, not a launch feature.

**Movement (E8)**

FR49: **The Shift boost is a universal ability on every hull — no card, no equipment.** A permanent slot (slot 1) on the unchanged Equipment interface, keyed `Shift` as a TAP (keydown edge on either Shift code; OS auto-repeat dropped; a held Shift is one press; the Windows Sticky Keys hazard is ACCEPTED and documented, no mitigation). **+25 % of the hull's LADDER-RAISED max speed (Eric: the SPEED ladder is inside it — a capped Torpedo Boat boosts to 68.75 u/s) for 10 s on a 20 s reload `[DRAFT]`**, the reload taking `cooldownScale` through the one multiply so the RELOAD ladder scopes it. It aims nothing. The card-fitted speed-boost EQUIPMENT and `CONFIG.speedBoost` are DELETED; the boost stays applied outside `EffectiveStats`, so the bots' ring-deadband `max(rated, actual)` reading stands.
FR50: **Wake drafting is a base rule of the shared sim.** Riding inside any OTHER hull's wake gives a small forward-cap lift — the MAX (never the sum) over every other hull's ribbon within `CONFIG.wake.draft.halfWidthU`, own and torpedo ribbons excluded, scaled `lift × maxSpeed`; **`lift` and `halfWidthU` are `[DRAFT]`, Eric's to set, no placeholder** (*"semi-realistic"*). Trails merge; echoes don't — both hulls still paint (not a hide mechanic). The per-tick kinematics fold becomes `boosted → slowed → drafted → hooks` and is asserted byte-identical at both call sites (server `stepShips`, client prediction/replay); the draft rides `OwnShip` as a self-private scalar carrying no position, and the client replays the latest scalar across un-acked ticks. Drafting is FELT with NO HUD readout. Bots receive it passively.

**Catalog v3 content (E8) — Eric's authored document `catalog-v3.md` (2026-09-09) is the build reference: 29 lines / 114 cards — 11 equipment (55), 5 universal ladders (22), 3 deck-gun lines (7), 5 add-ons (5), 5 consumables (25). Its §4 per-line stat sheets are the numbers; this epic builds what it says and never invents a line or a number.**

FR51: **The five universal ladders.** ARMOR cap 4 (+25 max hp per tier, heals the difference on grant → TB 350 / ML 400 / BS 450 at cap); SPEED cap 4 (+2.5 u/s forward max per tier, reverse untouched); **TURNING (new)** cap 4 (+0.05 rad/s flat per tier `[DRAFT]`); RADAR SWEEP cap 5 (+3 rpm per tier to 30 rpm — the 30 rpm clamp stays; **rate, never reach — the eighths ladder stays frozen**); **RELOAD (global)** cap 5 at **−5 % per tier → `cooldownScale` 0.75 at cap** (was −10 %/card to 0.5), whose floor therefore moves 0.1 → 0.75 by construction and whose scope is every equipment reload AND the Shift cooldown. **Composition:** `reload = base × (1 − 0.05 × equipmentTier) × (1 − 0.05 × globalTiers)` — every equipment line steps its OWN reload −5 % per tier as additive 5-point steps (100 → 95 → 90 → 85 → 80 %, never compounding; a line that departs says so), composed BEFORE the global ladder, so a line maxed on both runs at 60 % of base. **Fractional integer steps ACCUMULATE AND FLOOR once at the end** (tubes, barrels, turrets, flares, held mines: `floor(base + step × tiers)`; nothing shows until a whole completes — Eric's standing rule). Both derivations live in `clampStats`; neither is a stat path a card can write.
FR52: **The three deck-gun lines.** DECK GUN cap 4 (+1.25 damage AND −5 % own reload per tier: 15 → 20 dmg, 5 s → 4 s, 3 s under max RELOAD — a REAL tier I step because the gun is always fitted); DECK GUN TURRET cap 1 (pool 1 → 2); DECK GUN BARREL cap 2 (+1 parallel barrel per copy, 1 → 3 shells at 12 u spacing, full damage each). The base gun is unchanged: 360°, 500 u/s, 15 dmg bursting r15 u, 6 contact, range 660 u, 5 s. **The gun's subdeck is three lines because it is never acquirable** — DECK GUN / TURRET / BARREL are ladders, not equipment.
FR53: **The eleven equipment lines** (cap 5 each; tier I = the bare weapon; tiers II–V per catalog §4 — typically −5 % reload, +damage, and a fractional integer step): **LIGHT TORPEDO** (twin sector ±45° about both beams — 90° dead zones fore and aft; 45 u/s, 40 dmg, 1 tube, 25 s, no max range; TB starter); **HEAVY TORPEDO** (bow ±30°, 65 u/s — was 60 — 50 dmg, 1 tube, 30 s; TB starter); **SUPERCAVITATING TORPEDO** (bow ±15°, 195 u/s, 50 dmg, 45 s, a straight-runner that NEVER homes, speed and tubes fixed; unhomed); **NAVAL MINES** (rear ±60° to a 150 u leash, arm 3 s, trigger 32 u / blast 48 u, 55 dmg, pool 2, 15 s; **NO per-player live cap**; blast ×1.1 compounding with trigger = 2/3 blast; ML starter); **CAPTIVE MINES as its own line** (the shipped captive doctrine on the mine chassis on a longer clock: trigger 144 u / blast 32 u, holds one un-upgraded torpedo dealing MINE damage at mine blast radius, cannot be self-detonated, the fish has no max range, pool 1 / 20 s `[DRAFT]`; ML starter); **HORIZONTAL MISSILE** (bow ±50°, 250 u/s, flies FLAT to the click and bursts r20 u there, an en-route hull is a direct hit, range 660 u, **islands BLOCK it**, 1 missile, 30 s; BS starter); **MACHINE GUN** (a held-fire STREAM — click-and-hold; arc bow ±90° ruled; 4 dmg/shell every 0.25 s, 250 u, 6 s of fire per pool, 15 s reload — everything but the arc `[DRAFT]`; direct hits, no burst; TB starter); **FLAK GUN** (360°; one shell AIR-BURSTING at the click in a wide weak blast that hits hulls AND ordnance; 10 dmg r40 u, 660 u, 8 s — base numbers `[DRAFT]`; the brainstorm's SHRAPNEL GUN renamed; ML starter); **MONITOR GUN** (bow ±10°, ARCING — fires OVER islands; 500 u/s, 660 u, 75 dmg, **NO burst** — hits only what is under it at the click; 50 s; BS starter); **BROADSIDE GUN** as shipped (twin sector, 4 → 6 turrets, 15 dmg, 18 s, the 5/8-rung range; unhomed — in no starter); **STAR SHELLS** as shipped (360°, lit r165 u for 10 s, 20 s; BS starter). The AR16 "no torpedo variety" veto is NARROWED to within-a-weapon (Eric 2026-09-03, GDD note 17): three torpedo LINES are legal, and one fit may carry two of them. **FR7's "torpedoes outrun every hull / never self-hit" law is RETIRED as a requirement (Eric 2026-09-11): the game has NO FRIENDLY FIRE — your own ordnance never damages your own hull at any speed, so a Light Torpedo slower than a boosted Torpedo Boat is simply a slow torpedo; the catalog speeds stand and the boost stands.** FLAK's air-burst destroys ENEMY ordnance only (own ordnance is immune, same ruling).
FR54: **The five add-ons** (one copy each; add-ons STACK — exclusivity is gone; variant behaviours are expressly upgrades, never starting kit): **ACOUSTIC HOMING** (0.5 rad/s steer, acquire 120 u, dies at 1300 u; applies to LIGHT AND HEAVY torpedoes, never the Supercavitating; one card homes every torpedo carried; TB starter); **FOULING MINES** (victim ×0.75 both speed caps for 5 s, refresh not stack; NAVAL MINES ONLY — the captive's fish does not foul; ML starter); **HEAT SEEKING** (the acoustic-homing numbers on HORIZONTAL MISSILES; BS starter); **DAZZLE SHELLS** (enemies inside the lit zone: sight ×0.5; never changes the lit radius; BS starter); **PHOSPHOR SHELLS** (5 hp/s burn on non-owner hulls inside 0.8× the lit radius; unlock only). DAZZLE + PHOSPHOR on one flare is buildable.
FR55: **The five consumables** (cap 5 each; no reload; launch content is exactly these five — the 09-04 damage-reduction buff is OUT and no damage-reduction concept exists anywhere): **HULL REPAIR** (FR47; key); **SHIELD BLOCK** (key; absorbs the next 100 hp for 10 s **from EVERY damage source, storm and burn included — Eric**; unused shield expires at 10 s; **a second shield REPLACES rather than stacks (Eric 2026-09-11); the shooter gets NO tell (Eric 2026-09-11) — an absorbed hit still fires their Hit Call and the victim's damage reads 0**; unhomed); **SMOKE SCREEN** (key; a trail laid astern for 5 s, each puff living 30 s and expanding r40 → r60 u at a `[DRAFT]` rate; **blocks SIGHT only — radar unaffected**; a puff hides its occupant, hides everything behind it and blinds an observer inside it, by symmetry rather than by rule; unhomed); **CHAFF** (key; bursts at YOUR OWN position: 10 fake radar returns in r120 u for 15 s, server-generated and wire-indistinguishable from real returns — the jamming machinery reused; the owner never sees them; unhomed); **DECOY BUOY** (key primes, click fires into the mine's rear arc; 50 hp; lasts until destroyed — no lifetime; homing ordnance retargets onto it AND it physically blocks any torpedo or missile, homing or not, which detonates there; clearable by deck gun or flak burst; **radar PAINTS it (Eric 2026-09-11); the owner's own torpedoes and missiles PASS THROUGH the owner's own decoy, and the owner's own flak never destroys the owner's own ordnance (Eric 2026-09-11 — no friendly fire)**; unhomed).
FR56: **The three starter decks** (catalog v3 §1; each exactly 40, exactly three equipment lines, legal against a fresh account): every starter carries HULL REPAIR ×5, DECK GUN ×1, TURRET ×1, BARREL ×2, RADAR SWEEP ×3, RELOAD ×3 plus its own ARMOR/SPEED/TURNING tilt — **Torpedo Boat** LIGHT TORPEDO ×5 · HEAVY TORPEDO ×5 · MACHINE GUN ×5 · ACOUSTIC HOMING ×1 · ARMOR 2 / SPEED 4 / TURNING 3; **Battleship** HORIZONTAL MISSILE ×5 · MONITOR GUN ×5 · STAR SHELLS ×5 · DAZZLE SHELLS ×1 · HEAT SEEKING ×1 · ARMOR 4 / SPEED 2 / TURNING 2; **Mine Layer** NAVAL MINES ×5 · CAPTIVE MINES ×5 · FLAK GUN ×5 · FOULING MINES ×1 · ARMOR 3 / SPEED 3 / TURNING 3. **Unhomed at launch** (unlock or pool only): SUPERCAVITATING TORPEDO, BROADSIDE GUN, PHOSPHOR SHELLS, DECOY BUOY, SHIELD BLOCK, SMOKE SCREEN, CHAFF. **These QUANTITIES have ALREADY been re-cut by Eric (2026-09-11) and he holds the new counts — they are delivered to the implementing agent when the deck-rules story is reached (the Story 7.5 "Eric states the plan" gate); the LINE composition above stands, and the counts printed here are catalog v3's, superseded on delivery.**
FR57: **Mines and deception, re-ruled.** **Shooting ANY mine you can see detonates it** (the owner-only rule widened, Eric 2026-09-04) — implemented PERCEPTION-BLIND, since *"you can see"* is descriptive (Eric 2026-09-10); chains resolve the same tick. **No per-player cap and NO room-wide ceiling on mines** (Eric: no ceiling) — `maxLive`, `globalCap` and the oldest-eviction branches are DELETED; a mine exists until triggered or destroyed, absolutely; the only bound is pools × reloads. Own mines ARE hit by own shells (clear your own field). **The RADAR BUOY is DELETED entirely** — equipment, view, gate, its GUN and JAMMING doctrines and its CONFIG; the jamming fake-blip generator survives as CHAFF's. The 2026-08-19 *"nothing fakes a ship contact any more"* claim is RETIRED: CHAFF fakes radar returns and the decoy draws homing ordnance, so FR10's counter-intel law has live cases again.
FR58: **Arcs, flight and denial for the new register.** Three 360° weapons (deck gun, star shells, flak), five bow sectors (HEAVY ±30°, SUPERCAV ±15°, MONITOR ±10°, MISSILE ±50°, MACHINE GUN ±90°), two twin sectors (broadside 60°/side, LIGHT TORPEDO 45°/side — the side containing the click fires, neither = denied), and one rear sector (±60° to a fixed leash) shared by both mine lines and the DECOY BUOY. Denied fire stays authoritative and self-private with a reason (out-of-arc / no-ammo / cooling / blocked placement); an island- or boundary-blocked placement is refused WITHOUT spending the charge. Flight: the missile flies flat and islands block it; the monitor shell arcs over terrain and resolves at the click (shell radius vs hull at the landing point); the machine gun is a stream of direct-hit shells that ends on release, arc exit (ONE out-of-arc denial, then silence), an empty pool, the refit window opening, or the window losing focus, and never resumes without a fresh press. Every fitted system keeps its own pool and timer ticking every tick regardless of selection (FR5 stands). No endure axis: Armor IS HP — no torpedo bulge, no directional armor, no crits, no fire.

**Bots, results, disclosure (E8)**

FR59: **Bots sail decks.** Each of the six profiles carries an **authored 40-card deck in the player format**, legality-checked in a BOOT-TIME test (the suite fails, never a room), seated through the same path as a human and dealt the room's pool. A **TOTAL consumable-tactic table** (five rows; the type refuses a consumable without one) and a tactic per new equipment line (the total `EquipmentTactic` record widened to every new id) so no card can ship without a bot that can spend it; the specific tactic bodies are facilitator proposals `[DRAFT]`. Bot spending drops `HEAL_CHOICE` (healing is HULL REPAIR's tactic), weights LINE ids including consumables, consults `canStock`, and never mulligans by default. **Bots must actually buy equipment lines** — Story 7-5's measured gap (0 of 2,495 offers took an acquisition card; ~12 % of every bot hand dead) must not recur under the new grammar. The harness runs **authored** and **random-legal** deck arms (legal against every cap, pool rolled per match) and pins the bars: starter-vs-veteran win band, two golden decks (torpedo-less TB, pure gunboat), heal-take rate, levels wasted, one-copy appearance rate re-measured at 50. **Balance cycle 1's class numbers are VOID once the Shift boost lands on every hull — rerun before tuning any tier.**
FR60: **Results and the record.** The results modal shows the **FINAL LOADOUT only** — the nine slots as they ended plus the five ship-ladder tiers; BOONS ACCRUED / LAST OFFER are RETIRED (Eric 2026-09-11, which also closes the deferred *"i'll need to think on that"* results-blocks question). **Brought / drawn / taken lives ONLY in match history (E9).** Enemy decks are shown to no player, ever, by any surface. **Every deck in every match is recorded server-side** as a `MatchRecord` (deck brought, cards drawn and taken with `T+` stamps, placement, kills, the whole pool) — E8 emits it; E9's store keeps it; and **`MatchRecord` is NEVER `ResultsMsg`** (a deck field on the broadcast results row would hand every enemy deck to every player).
FR61: **Anonymous and signed-in, match side.** Anonymous = today's game: open the URL, pick a hull, sail its starter deck; nothing is stored. Signing in changes what you KEEP, never what you can DO in a match — same decks, same rules, same doors, and **Epic 8 works in full anonymously on the starter decks**. Unlocks are variety, never power: a starter deck is authored to be viable against a veteran's, and the starter-vs-veteran win band is the measurement. **The dev/staging host is ACCOUNT-ONLY (Eric, FIRM). Anonymous play on PRODUCTION is OPEN — Eric is thinking it through; do not re-ask.**

**The account (E9)**

FR62: **Two states, no guest tier. OAuth only — Google (`openid`) and Discord (`identify`), no email scope on either.** The account holds a provider + an opaque subject id and never an email, name or password; 13+ by the providers' terms, not verified. A signed-in caller completing OAuth with a SECOND provider gets that identity LINKED to the same account (Eric 2026-09-09 — one account, two ways in, never merged by email, never a second account). The session is a JWT `{ id, tokenVersion }` valid 30 days, kept in localStorage across the return-to-port reload; sign-out and deletion bump the token version so every issued token dies at once. Anonymous is a first-class result of verification (`null`), never an error. The password/register/anonymous/forgot endpoints of the auth module are NEVER mounted (pinned by a test on the mounted path set).
FR63: **A deck enters a match only through the server.** Both doors (the standard queue for Standard, the arena for Solo vs AI) verify the Bearer token AFTER the `PROTOCOL_VERSION` gate and the staging gate, then load the deck through ONE shared `loadDeckFor(userId, deckId, hull)` that runs the shared legality function against caps and unlocks and places the 40 line ids in the seat. **The client sends a deck ID, never contents — a client-supplied `deck` key is REJECTED by the option sanitizer at both doors** (`deckOverride` under `HC_DEV_OPTIONS=1` is the sole exception). Anonymous → the hull's starter deck; signed in without a `deckId` → the account's LAST-USED deck for that hull, else the starter; an invalid deck → a refusal with a stable reason (`account.unavailable` / `deck.missing` / `deck.illegal { rule }`), **never a silent substitution**. **The legality check happens BEFORE queueing (Eric 2026-09-11): a captain cannot enter ANY mode — SOLO or SOLO VS AI — with an illegal deck selected.** The client runs the ONE shared legality function against the selected deck on the home screen and refuses the mode buttons while it fails (the reason word beside the deck), so the server's door check is the BACKSTOP for the rare mismatch (a copy revoked or a cap changed since the page loaded); when that backstop fires, the client shows the one-word reason on the home status line — the mode-agnostic channel connection failures already use — and offers **SAIL DEFAULT DECK** as an explicit act.
FR64: **The deck editor is the merged SHIP & DECK screen** (Eric 2026-09-11: Ship Select and the deckbuilder are ONE screen — no `/decks` route, no `DECKS` home link). A signed-in captain keeps **several NAMED decks per hull** (*"fine. Data is cheap"*; names allowed, reversing the forge/GDD rejection; 1–16 characters under the callsign's own filter; visible only to their author — the feed stays name-free), chosen from a dropdown; every account starts with the hull's starter as deck 1 (player-facing name `DEFAULT` — Eric 2026-09-11; "starter" stays the internal word), renameable and deletable, and a hull's LAST deck cannot be deleted (delete confirms). An author decides how many copies of each line to run (*"only taking up to Tier III… might be a planned strategic choice"*). Legality is evaluated client-side by the ONE shared function the server runs at the door (exactly 40 · ≤ 3 equipment lines · caps · owned copies); SAVE is unlit while illegal; CONFIRM SELECTION with unsaved edits saves if legal and otherwise blocks on the legality line. A deck is never a forced choice; the last-used deck per hull is the default.
FR65: **Unlocks and earning.** **COPIES are acquired per copy** (Eric 2026-09-11) — unlocking a line IS buying its copy 1, and every further copy up to the cap is bought individually at a **flat per-copy token price `[DRAFT]`**, in any order; one currency (unlock tokens), no copy packs, no tiered pricing, never bought with money; unlocked IN PLACE from the collection grid (Hearthstone / MTG Arena shape — no armory page). Account level derives from per-match XP (**placement-scaled, discounted in Solo vs AI**) and grants **one token per level**; level and available tokens are DERIVED (`level = f(xp)`, `available = level − tokens_spent`), never stored. Both XP dials derive from ONE intent number — **matches to unlock the launch catalog (`CONFIG.progression.matchesToCatalog`) — OPEN, shipped as a CONFIG dial with NO placeholder, now measured over 114 cards, tuned from live history. Eric's intent 2026-09-11: *"It should take a while, but not too long"* — the tokens/unlocks story proposes a candidate against that sentence for him to confirm; nothing invents it.** All three hulls and their starter decks are unlocked from day one (the starters' cards are the initial unlock list); the unlock surface beyond a starter is every unhomed line plus every line another hull's starter carries. Tokens are the only account currency; XP the only in-match currency. Deck slots may later be rewarded or monetized — a slot is not power; noted, not scoped.
FR66: **Match history.** A player reads back their OWN rows only — deck brought, cards drawn and taken with `T+mm:ss` stamps, placement, kills — cursor-paged, keyed on the caller's user id and never on match id, so **enemy decks are never returned by any route**. The hidden match pool appears only as the pool cards that player DREW (Eric 2026-09-11, closing GDD open note 20a; no post-match reveal). Every deck in every match is stored for Eric's metrics (*"the only way to see how things are performing in live play"*), read through the admin console (FR69). Bots, anonymous captains and mid-match-deleted captains are written with no account reference.
FR67: **Preferences and settings are TWO-STATE, nothing in between** (Eric 2026-09-09). Anonymous: the starter deck only, no progression, no deck editor, and `hullcracker.*` localStorage exactly as today — byte-identical. Signed in: the account profile is the source of truth for callsign, colour preference, last class AND the settings store; the first sign-in of a fresh account SEEDS the profile from local (never wiping a carried callsign), every later sign-in overwrites local, edits while signed in write both, and sign-out leaves local as it was — ONE store with two backends, flipped only by the account module. Settings gains **SIGN OUT** and **DELETE ACCOUNT (with a confirm)**; the binding reference lists the v3 scheme.
FR68: **Self-serve deletion.** An authenticated `DELETE /api/account` runs one transaction — bump the token version FIRST, delete identities, profile, decks, unlocks and progress, set every referencing match-participant row's account reference to NULL — and answers 204; the anonymised match rows are RETAINED (Eric's metrics survive as anonymous rows). Google's and Discord's developer terms expect this path; the privacy paragraph states it.
FR69: **The admin console.** `@colyseus/admin` is mounted at `/admin` in BOTH environments **for Eric only** (*"I DO want @colyseus/admin for ME"*), behind its own login (the bootstrap admin created once, out of band, never by a route) and, on the dev host, behind the staging gate; player accounts never hold an admin role; the account tables are exposed as its `tables`. **Knowing exception:** it stores ONE password hash (Eric's) — the forge's "no own email/password storage" rejection was about players and stands.
FR70: **The privacy policy grows by ONE paragraph on signed-in accounts**, every sentence a claim about shipped behaviour: what is held (provider name + opaque subject id; chosen callsign, colour preference, last class; the settings store; decks and **deck names as stored user-entered text**; unlocks and progress; per-match rows of own deck, draws and picks with placement and kills), the 30-day session token in localStorage, self-serve deletion leaving match rows with no account reference, that nothing is stored for a player who does not sign in, that the operator's admin console can read these tables, and Google and Discord named as providers. Third-person Hullcracker.io voice throughout; the absence of a named controller IS the standing ruling (cycle 108) and is not reopened here.
FR71: **How-to-Play is re-cut** (EQUIPMENT and UPGRADING) for decks, the nine slots, consumables, `Shift` and the countdown REDRAW — **the ONLY place a new feature is explained; no in-game copy is added unasked, still NO glossary.**
FR72: **Key rebinding (Eric 2026-09-11 — reversing the post-beta deferral in AR17 and UX-DR31).** A player can rebind the fixed v3 keys (helm W/S/A/D, Q/E/R, `Shift`, the belt `1`–`4`, Tab, F, M, zoom) from the Settings overlay; bindings are saved in localStorage when anonymous and as an ACCOUNT setting when signed in, riding the same two-state settings source as everything else in FR67 (seed-from-local on first sign-in, account wins after, edits write both). Defaults are the v3 scheme; a binding conflict is refused, never silently swapped; the binding reference the overlay already shows becomes the editor. Lands in Epic 9; needs a small design pass (UX-DR78).
...
NFR6: Matches complete in **~15:00 start-to-results (the estimate, and the design target)**. **RESTATED 2026-08-21 (Story 7.6; Eric: "~15 minutes is still accurate, that is the estimated game time. The theoretical max is a little over 17 right now I think.").** The original parenthetical ("ring fully closed at ~12:00") is stale: cycle 82's sudden death added a FOURTH ring group, so the ring reaches the 660u endgame radius at 12:00 and then collapses to zero between 15:00 and 16:00 — **full closure is 16:00** (`CONFIG.zone.suddenDeath`, `zoneClosedAtMs` 960000). Past 16:00 the whole map is storm at `stormDps` 4 hp/s with no ramp, so the **structural ceiling is ~17:30**: the game's largest hull (Battleship, 350 hp — maxHp does not scale with boons) sinks ~87 s after full closure. That ceiling bounds the hull, not the match: heals stay spendable while alive and `bankedLevels` is uncapped, so each hoarded level extends the tail ~25 s (`CONFIG.damageControl` = 50 instant + 50 pooled since cycle 122). Termination is still structural — the XP tick funds far less healing than the storm removes — and the measured worst case in the batch-sim harness was ~16:43, because pilots spend levels as they earn them — a cycle-82 measurement, taken before cycle 122 doubled hull HP and heal value in step, so it is indicative rather than current.
...
NFR18: **DEFERRED 2026-08-21 (Eric) — NOT SATISFIED FOR BETA, by decision rather than by omission.** Story 7.7 is deferred in full, so beta ships ONE deployable (the existing Render web service serving `client/dist` alongside the arena) carrying ONE version number on the `0.17.X` scheme. The requirement stands as written for whenever the split is revived; see `deferred-work.md` for the revival tripwires. Original text: Two deployables. The client ships as a Render Static Site (client/dist, CDN-served, no Node process) on the apex domain; the game server ships as a Render Web Service running only the Colyseus arena, on a subdomain (ad and analytics identity is per-domain and must not move). shared/ is a build-time library of both and is never deployed independently. Each deployable carries its own version number, cutting `0.1.0` at beta; the game-server URL is build-time client config; the server declares explicit CORS and WebSocket origin allowances. PROTOCOL_VERSION remains the runtime client↔server compatibility gate and is independent of every release version.
...
**Epic 8 · Epic 9 — added 2026-09-11**

NFR20: **Deck anti-cheat.** Deck state is a server-private multiset in insertion order, never on the wire; only the four drawn line ids of an offer ride to a client; the match pool's composition never leaves the server (not in any frame, not in `WelcomeMsg`, count-only in the log); **a client never supplies a deck** — the door rejects a `deck` option and accepts only a `deckId` the server resolves (`deckOverride` under `HC_DEV_OPTIONS=1` is the single dev exception); legality is server-checked once at the door; consumable-pick refusal, mulligan eligibility and denial are server rulings; and **`MatchRecord` is never `ResultsMsg`** (pinned: no deck key on the broadcast results row). NFR4 extends to all of it.
NFR21: **The master perception invariant keeps EXACTLY SIX declared exceptions (`sp`, `hc`, `mz`, `sunk`, `sm`, `fh`) through both epics**, and `frames.ts` stays the single spatial chokepoint. CHAFF is a second SOURCE of the existing jamming carve-out, not a new clause (a fake discloses nothing real; the tests' fourth `verifyBlip` arm recomputes fakes from seed + epoch + source). Smoke and decoys are registry pseudo-rows with their own `visible()`/`materialize()`. Two DECLARED disclosure widenings are ledgered so nobody rediscovers them as leaks: the ballistic reveal gains one field `w` (the weapon FAMILY needed to draw it — no range-derivable value, no identity), and the self-private draft scalar tells a client "a wake is under you" even when its layer is island-hidden (own-ship state, no position; kinematics disclose the same one frame later). The account store changes nothing spatial.
NFR22: **Determinism and parity widen to the new sim surfaces.** Wake drafting, the smoke puff's radius curve, the fractional-step floor and the reload composition all live in `shared/` and run identically on both sides at the same 50 ms dt; the kinematics fold `boosted → slowed → drafted → hooks` is asserted byte-identical at both call sites, with a parity pin at `draft = 0` proving the tree is byte-identical to `HEAD`; `effectiveStats(cls, cards)` is permutation-invariant over any legal pick list (pinned); the pool and the mulligan draw on seeded, room-private streams; no `Math.random()` or wall clock anywhere in it (NFR5 stands).
NFR23: **Performance pins owed at build**, under the unchanged NFR1 bar: perception at 20 observers × 200 live smoke puffs inside the 50 ms tick; `checkMineTriggers` + the ordnance target collector at **500 live mines** (no cap of any kind survives — Eric); the machine-gun stream's per-shell muzzle flash measured at the emitter before the 0.25 s cadence is trusted (20 streaming bots ≈ 80 `mz`/s — `flashBudget.ts` caps the render only; if it fails, the fix is the `[DRAFT]` cadence, not the per-shell rule); `/metrics` reports `minesLivePeak`, `smokeLivePeak`, `streamShellsPerTick` and the harness reports the live-mine peak per match. Uncapped mines, undying decoys, smoke puffs and chaff fakes all count against the 60 FPS / 20-hull frame budget.
NFR24: **Privacy and PII for accounts.** The store holds a provider name + opaque subject id, the player's callsign, colour preference, last class, settings, decks (names included), unlocks, progress and own per-match rows — never an email, name or password; no email scope is requested; nothing is stored for a player who does not sign in. **The log's FIRST PII rule:** never log tokens, provider subject ids, callsigns (stored personal data from this epic on) or deck contents — the opaque server-minted `userId` is the only identity a log line may carry; `match.end` stays byte-identical; the writer logs one line per match, never per row; the pool logs a count, never contents. NFR14's "no accounts, no server player DB" clause is SUPERSEDED (see the supersession register); NFR19's no-PII posture for analytics is unchanged.
NFR25: **The account layer never takes the game down** (a fourth error-handling zone beside NFR16's three). The module is absence-gated on `DATABASE_URL` exactly as GA/AdSense are: without it nothing mounts, the database client is never constructed (pinned), the home renders no SIGN IN, and the game is byte-identical to today. With it set but the database unreachable or a migration failing, the process STILL STARTS with the module OFF, logs `account.disabled { reason }` once and reports `account: false` on `/liveness`. **Nothing inside `Room.update()` / `World.step()` may ever await the database** — a query on the tick is a defect in the class of a throw in `shared/`; the match-end write is fire-and-forget through a PORT with an in-process FIFO that retries once and DRAINS on shutdown (Render sends SIGTERM on every deploy — twenty matches ending as the process dies must not lose twenty XP grants); a `NullWriter` is injected when the module is off so rooms carry no `if (accountEnabled)`. HTTP endpoints answer Result-shaped JSON (`{ ok, code, reason }` with stable dot.case codes; the status carries only the class); no stack trace, driver message or SQL ever leaves the process; anonymous joins are untouched by any account failure. The deck adds no error zone: every spend-path refusal is a silent no-op with the offer left byte-identical; `held` joins the finite-checked input set (a non-boolean drops the whole message).
NFR26: **Storage and durability.** Render managed Postgres, one instance per environment (`hullcracker-db`, `hullcracker-dev-db`; smallest paid tier, free tier rejected for staging too), declared under `databases:` in `render.yaml` with `DATABASE_URL` from `fromDatabase` — **the `databases:` entries land in the SAME PR as the code that reads them, never ahead** (the Blueprint auto-syncs on `main` and bills both instances on merge). **Never a persistent disk on either web service** (it disables zero-downtime deploys and drops every live match per push); Render managed backups only. Migrations are checked-in SQL generated by `drizzle-kit` and run at boot — never `"auto"` in production; the unused plugin tables are never wired. Level and available tokens are DERIVED, never stored, so the match-end write is one atomic increment with no read-modify-write race (one account in two browsers is Tuesday). Local dev and the server test suite run the REAL schema on PGlite (`HC_DEV_DB=pglite`) so `npm run check` exercises the real SQL with no Postgres installed. Staging carries its own database and OAuth apps so accounts ARE QA'd on the dev host — deliberately NOT the ads/analytics blind spot.
NFR27: **Ops, secrets and the deploy contract.** New server-side env: `DATABASE_URL` (via `fromDatabase`), `JWT_SECRET`, `SESSION_SECRET`, `HC_OAUTH_GOOGLE_ID/_SECRET`, `HC_OAUTH_DISCORD_ID/_SECRET` (all `sync: false`, one OAuth app per host) and `HC_SITE_ORIGIN` (in the clear — an origin is not a secret; never derived from the `Host` header). **No secret is ever a `VITE_` variable** — `client/.env.*` is not gitignored, which becomes urgent the day OAuth client secrets exist locally; the client learns "accounts exist" only from `/liveness` (`account: true`), never from build config. The six `sync: false` secrets must be set in the Render dashboard **BEFORE the account PR merges to `main`** (an unattended Blueprint sync reports `error` and skips each until set — cycle 127's `HC_STAGING_KEY` observation, now ×7). `@colyseus/database`'s `drizzle-orm` release candidate is pinned to an exact version. `/liveness` is the natural `healthCheckPath` now that a database dependency exists (ledgered, Eric's yes). Sign-in start and deck writes share `soloThrottle.ts`'s per-IP in-memory shape (no Redis). The admin console sits on the public host behind its own login (its "separate hostname" guidance is knowingly not followed at launch; revisit if it ever holds more than one admin).
NFR28: **Versioning and the wire gate.** The game keeps ONE version on the `0.17.X` cycle scheme — the Colyseus 0.18 upgrade is a `0.17.X` cycle, not a `0.1.0` cut; Story 7-7 stays DEFERRED IN FULL and the account store adds no deployable, no CORS and no cross-origin cookie (its Bearer-token API is what lets a future split change nothing here — appended to 7-7's revival brief: option A needs CORS on `/api/*` and `/auth/*`, options B/C need nothing). `PROTOCOL_VERSION` (49 at `HEAD`) bumps for the 0.18 schema encoder and once per wire-changing E8 story (catalog content, `InputMsg.held`, the nine-slot ammo array, the new `OwnShip` fields, the `missile`/`missileU`/`smoke`/`decoys` rows, the reveal's `w`) — **never for the account HTTP API, the harness or the bots**; the account API's contract is its endpoint path set, pinned by test, outside `PROTOCOL_VERSION`; a shared `CONFIG` block bumps PV only when the client READS it (the `CONFIG.fleet` precedent). Framework upgrade and feature never share a PR.
NFR29: **Accessibility scope for v3 surfaces.** **Keyboard-only access is OUT OF SCOPE** (Eric 2026-09-11: *"You need the mouse to target/fire, NO ONE is keyboard-only"*) — no surface (Ship & Deck, card tooltip, OAuth popup) specifies a keyboard-only path, hover-only tooltips stand, and every reviewer finding premised on a keyboard-only player is WAIVED; this is a scope statement, not a floor change — NFR13's dual-coding, twin and photosensitivity laws bind every new surface (wipe + numeral, frame + chip, rung fill + hue, word + dim). **The 9 px floor is RAISED INTO, never excepted** (every sub-9 px v3 register was raised). The Windows Sticky Keys hazard on the `Shift` tap is ACCEPTED and documented in the input-capture rule. **No explanatory wording on a card face or a HUD element** — a stat row or a glyph, never a sentence; explanation is hover-only or How-to-Play.
NFR30: **Load and the no-account promise.** NFR2 stands with one word changed: cold load → playable in under ~10 s with no install and **no account REQUIRED**; sign-in never gates play, never blocks first paint, and its scripts count against the same budget as ads and consent.

### Additional Requirements

**From Architecture — sequencing and platform**

AR1: Work item #0 (before E1): the Colyseus 0.16 → 0.17 upgrade — epic work builds on the stabilized 0.17 adapter. **The Track-2 hosting move is SUPERSEDED IN PART (Eric, 2026-08-18):** the trigger fired (open beta is the first public link) but the destination changed. The client/site → static hosting half was assigned to Story 7.7 (as a Render Static Site) and is **DEFERRED IN FULL (Eric, 2026-08-21)** — beta ships from the single Render web service, so NEITHER half is executed. The game server → Colyseus Cloud half was already NOT happening — Eric elected to stay on Render and scale vertically, retaining control of his own servers, so Redis-backed Presence/Driver is deferred with it (a single instance needs no shared registry; NFR10's injectability obligation keeps the door open). **Render still cannot host Colyseus horizontal scale-out (no WebSocket sticky sessions), and Render autoscaling must never be enabled.** Note also: matchmaking is NOT a separable service — in Colyseus 0.17 the matchmaker is a library every process shares through Presence/Driver, so the split is two deployables (frontend, backend), never three. The original trigger-based framing follows, retained as history: it happens before the first public/stranger link, at Eric's call — NOT as a prerequisite for epic work. Render remains fine for friends-scale playtests throughout (Eric, 2026-07-17: the game stays playable and hostable-as-is during the whole epic sequence). When the move happens, execute it as one motion so only one new deploy pipeline is ever built.
...
**From Architecture — the Account Store amendment (2026-09-09, D9–D18, Novel Patterns 6–9) and the Deck amendment (2026-09-10, D2 amended, D19–D31, Novel Patterns 10–13) — added 2026-09-11**

**Sequencing**

AR19: **E9 story 0 is the Colyseus 0.17 → 0.18 upgrade, and it comes FIRST** (Eric 2026-09-09: *"keep everything CURRENT"*; `@colyseus/database` and `@colyseus/admin` exist only on 0.18). Its own PR, `PROTOCOL_VERSION` bumped (the schema encoder changed: Schema 4 → 5 on both sides), a full headless-smoke pass BEFORE any account or deck code lands — a framework upgrade and a feature never share a PR. Pinned targets: `colyseus` 0.18.5, `@colyseus/core` 0.18.12, `@colyseus/schema` 5.0.27 (both sides), `@colyseus/tools` 0.18.3, `@colyseus/sdk` 0.18.2, `@colyseus/auth` 0.18.2, `@colyseus/database` 0.18.3, `@colyseus/admin` 0.18.5, `@colyseus/monitor` 0.18.3, `@colyseus/playground` 0.18.4, `postgres` 3.4.9, `drizzle-kit` 0.31.10; Node stays 22. Measured touchpoints: `setSimulationInterval` → `setTimestep` (one call in `ArenaRoom.ts`), `setMetadata` now REPLACES, `Client#id` is gone, schema field cap 63 (`ArenaState` has 23), `createRouter` survives, `static onAuth(token, options, context)` keeps `context.headers`; ~26 test files import Colyseus APIs. Story 0.1's ledger (PV join gate, JOINING guard, seat-reservation timing, `_enqueuedMessages`) is the checklist of what a Colyseus major must re-verify.
AR20: **Execution order: E9 story 0 → Epic 8 → Epic 9, shipped as the ratified ONE-UNIT release before the traffic push** ("go big or go home"). E8 builds on the 0.18 runtime and plays anonymously on the starter decks; E9 then adds what you keep. The shared modules the E8 mapping borrows from E9 (`deckRules.ts`, the door's `loadDeckFor`, `MatchRecord`) are BUILT in E8 with the starter-deck / null-writer path and CONSUMED by E9 — an ordering fact the story split must state, not assume.
AR21: **The damage gate (D28) and the ordnance target collector (D25) are prerequisites of E8 stories 5, 12, 13 and 15 but belong to none of them**: land them in E8's slot-rework story (which touches `fireControl` anyway) or as an **E8 story 0 — its own PR, no wire change**.

**Topology, storage, data layer (E9)**

AR22: **The account store is an in-process module `server/src/account/` mounted once in `app.config.ts`, same origin as the game — zero new deployables, no CORS, no cross-origin cookie; Story 7-7 stays deferred in full.** Its `index.ts` exposes exactly two things: `accountEndpoints()` and `createAccountWriter()`. **Nothing under `game/` imports it**; `world.ts` and `match.ts` keep zero persistence imports exactly as they keep zero Colyseus imports; `account/` imports `shared/` and Colyseus and never `game/`.
AR23: The module is **absence-gated on `DATABASE_URL`** like GA and AdSense (NFR25); staging gets its own database and OAuth apps so accounts are QA'd on the dev host.
AR24: **Storage:** Render managed Postgres per environment declared in `render.yaml` `databases:` (region `oregon`, smallest paid tier), `DATABASE_URL` via `fromDatabase.property: connectionString`, entries landing in the same PR as their readers; never a persistent disk (NFR26).
AR25: **Data layer:** `@colyseus/database` 0.18 (Drizzle ORM + `postgres` driver); checked-in SQL migrations under `server/drizzle/` generated by `drizzle-kit` and run at boot (never `"auto"` in production); **never `applyRouterDefaults` / the auto-mount** (it spreads the ENTIRE auth endpoint map) — the module builds its router itself; unused plugin tables not wired.
AR26: **Schema** (`server/src/account/schema.ts`): `account_identities (provider, subject) → user_id`; `account_profiles (user_id, callsign, color_pref, last_class, settings jsonb)` — `color_pref` is a PREFERENCE, never the server-assigned wheel index; `decks (id, user_id, hull, name, cards jsonb [{lineId, copies}], updated_at)`; `unlocks` **per copy** (UX 2026-09-11 corrects the per-line shape); `account_progress (user_id, xp, tokens_spent)` with level and available tokens DERIVED; `matches (id, mode, started_at, ended_at, roster_size, winner_class)`; `match_participants (match_id, user_id nullable FK ON DELETE SET NULL, role, class, placement, kills, deck_brought, deck_drawn [{lineId, atMs}], deck_taken [{lineId, atMs}])`; `colyseus_users.email` is NULL for every player permanently.

**Auth and the doors (E9)**

AR27: **Auth is `@colyseus/auth` 0.18, OAuth ONLY, on a CURATED endpoint subset** (userdata + OAuth start + OUR callback): Google `openid`, Discord `identify`, no email scope; `auth-login`, `auth-register`, `auth-anonymous` and forgot/reset/confirm are never mounted — a pin test asserts the mounted path set. **The popup callback is REPLACED by `oauthCallback.ts`, which `postMessage`s to `HC_SITE_ORIGIN`** (CONFIG, one per host, declared in the clear in `render.yaml`, never derived from the `Host` header) rather than `'*'`. Identity is keyed `(provider, subject)` through the module's own `onOAuthProviderCallback` upsert; **a second provider LINKS** (Eric 2026-09-09); the callback must write `anonymous: false` (the module's default is `true` — a trap); JWT payload `{ id, tokenVersion }`, `expiresIn: '30d'`, `revocationCheck` on the token version.
AR28: **Both doors verify the token — Novel Pattern 6 "verify, then load".** The Bearer token rides matchmaking; `StandardQueueRoom` (Standard) and `ArenaRoom` (Solo vs AI) `static onAuth` both call one `verifyToken(headers)` AFTER the PV gate and the staging gate — **the gate ORDER is fixed: cheapest and most disclosing-of-nothing first** — attaching `{ userId }` or `null` (anonymous, never an error). Then `onJoin` at BOTH doors calls the one shared `loadDeckFor(userId, deckId, hull)` (AR36).
AR29: **Card ids are camelCase LINE ids declared in `shared/src/sim/catalog.ts`** (`LINE_IDS`, 29 ids, one per catalog v3 line, with per-line caps); a deck row is `{ lineId, copies }`; store, `deckId` wire, editor and sim all speak `LineId`; display names live only in `client/src/ui/`; adding a line = one id + one cap; a rename = zero store migrations (FLAK GUN was SHRAPNEL GUN a week earlier).
AR30: **Deletion** (D15): `DELETE /api/account`, one transaction, token version bumped FIRST, match-participant rows set to NULL, 204; Settings gains DELETE ACCOUNT with a confirm. **Admin** (D16): `@colyseus/admin` at `/admin` + `/admin-api` in both environments, Eric only, own login, behind the staging gate on dev, bootstrap admin out of band; account tables via its `tables` option. **History** (D16): `GET /api/account/history?cursor=` returns ONLY the caller's `match_participants` rows, keyed on user id never match id.
AR31: **Prefs and settings — Novel Pattern 9, the two-state settings source**: `SettingsSource = 'local' | 'account'`, ONE store with two backends, `net/account.ts` the only module that flips it; anonymous byte-identical to today; signed in seeds-from-local on first sign-in, then account wins, edits write both, sign-out leaves local (Eric 2026-09-09 ruling verbatim in D17).

**The write path (both epics)**

AR32: **A deck enters a match ONLY through `loadDeckFor`** (placement rule 10 — never a client option, never schema, never `WelcomeMsg`): it runs `deckRules.check()` (pure, `shared/`) against caps and unlocks and returns the 40 line ids into the seat reservation; `sanitizeRoomOptions` REJECTS a client `deck` key at both doors and accepts `deckId`; anonymous → starter; signed in without `deckId` → last-used deck for that hull, else starter; invalid → refusal with a stable reason, never silent substitution; module disabled → `account.unavailable` for a token-bearer, silent anonymous for everyone else; `deckOverride` (line ids, `HC_DEV_OPTIONS=1` only, gated like `matchOverride`) is the sole exception to "a client never supplies a deck".
AR33: **`MatchRecord` → `AccountWriter` — Novel Pattern 7, a port with a queue.** At match end the room builds a server-only `MatchRecord` from `World` + `Match` (`game/matchRecord.ts`, including `pool`) and calls `accountWriter.recordMatch(record)` at ONE site (the results hook) — fire-and-forget, never awaited on the tick, logged on failure; the writer is a PORT the room is handed (not pub/sub — no other subscriber may ever exist, so the no-event-bus law is not bent), an in-process FIFO, retry once, `flush()` on shutdown; XP and tokens computed by pure `shared/sim/progression.ts` inside the same transaction as the history rows; bots, anonymous captains and mid-match-deleted captains are written with `user_id = NULL`; a `NullWriter` is injected when the module is off. **`MatchRecord` is NEVER `ResultsMsg`** (pin: no `deck*` key on `ResultsMsg`).
AR34: **Tick law extended and the three sites** (placement rule 9): nothing inside `Room.update()` / `World.step()` awaits the database; the writer is the ONLY runtime path from a room to the store; rooms touch `account/` at exactly three sites — `static onAuth` (verify), `onJoin` (load deck), the results hook (record) — and a fourth site needs an amendment.

**The card model and the fold (E8)**

AR35: **D2 boon effect model AMENDED.** Effect kinds are `stat` / `slotFill` / `doctrine` / `behavior` + NEW **`stock`**; **`slotReplace` is DELETED** (pinned; its two laws become "fitting a weapon never touches another slot's timer"); rarity is gone (`BoonRarity`, `CONFIG.deck.rareWeightBase` / `rareWeightPerDryLevel`, `DeckState.levelsSinceRare`, the `exclusive` validator rule); acquisition is gone (`acquire()`, `isAcquisitionDef`, `consumeAcquisition`, `EQUIPMENT_CATEGORY`, `UNIVERSAL_CATEGORIES`, `buildDeck`'s subdeck walk); **`HOOK_REGISTRY` ships empty and STAYS empty** — drafting and the boost are kinematics steps, not hooks, because a card-carried hook would be removable by a deck. AR4's hook purity law stands with no members.
AR36: **Card model (D19).** A line is `{ id, kind, cap, tiers[] }` with `tiers.length === cap` pinned; tier N is a bundle of D2 kinds; copy 1 of an equipment line = `[{ kind: 'slotFill', equipmentId }]`; add-ons carry `appliesTo: EquipmentId[]` generating one `doctrine` effect per listed equipment (SUPERCAVITATING excluded by not being listed); authored through `weapon()` / `ladder()` / `addon()` / `consumable()` helpers so `catalog.ts` reads like Eric's sheet, the generated bundles frozen and pinned. **Held-ahead add-ons need no mechanism**: a doctrine verb lives on `EffectiveStats`, not on a slot, so ACOUSTIC HOMING taken first is true the tick a torpedo is fitted.
AR37: **The fold is `effectiveStats(cls, cards: LineId[])`** (`OwnShip.boons` renamed `cards`; pick order; one id per copy). Order-independence is a PIN: every v3 stat effect is an `add`, or a `mult` on a path no other line adds to; `validateCatalog` refuses add + mult on one path across lines; a permutation property test asserts byte-identical stats. **Two derivations live in `clampStats`, never card-written**: the equipment reload step (`reloadMs = base × round3(1 − step × (tier − 1)) × cooldownScale` for weapon lines, `× (1 − step × tier)` for the deck gun's real tier I, `step = CONFIG.catalog.reloadStepPerTier` = 0.05, overridable per line) and the fractional-step floor (integer stats accumulate as floats and floor ONCE at the end). `EffectiveStats.equipment` becomes a TOTAL record keyed by `EquipmentId` (11 lines + `gun` + `boost`), each row carrying its `tier`; the three torpedoes are three ids over one shared stat shape; `BOON_STAT_PATHS` is GENERATED from the record; `sightRange`, the `rangeU` family and `mine.triggerRadius` stay DERIVED and off the whitelist; the seven `<equipment>.reloadMs` paths stay whitelisted-but-unwritten. **A card can never address a derived field, reload a consumable, fit a fourth weapon or exceed a cap** (refused at the door by `deckRules` and again by `drawOffer`).

**Loadout, activation, deck state (E8)**

AR38: **Loadout (D20):** one flat nine-slot array with roles `['gun','boost','weapon','weapon','weapon','consumable','consumable','consumable','consumable']` (`SLOT_GUN = 0`, `SLOT_BOOST = 1`, `WEAPON_SLOTS = [2,3,4]`, `CONSUMABLE_SLOTS = [5,6,7,8]`); one `LoadoutSlot { equipmentId, state: { n, reloadMsLeft } }` shape for all nine; `loadoutFor()` returns gun + boost fitted and seven empty, IDENTICAL for every hull (`specialsFor()` deleted); `slotAmmo` wire array length 9; a consumable stack is `{ n: copiesHeld, reloadMsLeft: 0 }` with `maxAmmo` = the line cap, `tick` a no-op, `tickReload` never called; a consumable slot at `n = 0` clears to `equipmentId: null` the same tick; a weapon slot never empties. **Fill rules** both on `applySlotEffect`: `slotFill` takes the first EMPTY weapon slot (cannot fail for a legal deck — property-tested over random legal decks); `stock` takes the slot already holding that line, else the first empty consumable slot, else REFUSES. The client replays `you.cards` through the same function to rebuild `equipmentId` per slot and reads `n` from `ammo` — no new wire field for slot contents. Keys: `Q`/`E`/`R` prime slots 2–4; `1`–`4` with the window CLOSED fire/prime slots 5–8, OPEN take cards (the digit's meaning is the window's state — a client concern); `Digit5 → HEAL_CHOICE` deleted with the sentinel; `Shift` is slot 1's `actSlot`.
AR39: **Consumable activation (D21)** rides the TWO existing input channels split by `isWeapon`: `hullRepair`, `shieldBlock`, `smokeScreen`, `chaff` → `false` (ability channel: `actSeq` + `actSlot ∈ CONSUMABLE_SLOTS`, no aim, no latency compensation); `decoyBuoy` → `true` (click channel, primed by its digit, `input.slot` carries the prime, `arcs.ts` gives it the mine's rear sector). One `activate()` on the unchanged interface decrements `n` and does its work; no new channel, no priming state, reconnect-safe by construction. **Full-row refusal is a SPEND rule**: `spendCard` runs `canStock(loadout, lineId)` BEFORE `consumeCard`; refused = silent server no-op, card stays in the offer, the client greys with the same shared predicate, no `denied` view. **Effect homes:** HULL REPAIR — `spendHeal`'s body moves into `hullRepair.activate` (instant + pooled; `tickRepairs` unchanged; afloat-only); SHIELD BLOCK — `ship.shield = { hpLeft: 100, until }` read by the damage gate; SMOKE SCREEN — D24; CHAFF — a `FakeSource` on the ship; DECOY BUOY — a `decoys` store placed by `dropDecoy`.
AR40: **Deck state, pool, draw, opening (D22).** `DeckState { cards: LineId[] }` server-private, built at the seat: authored 40 → `checkDeck` (exactly `CONFIG.deck.size`, ≤ `maxEquipmentLines` (3) lines whose copy 1 fits, every line ≤ cap, every line owned) → `+ room.pool` → 50, the pool appended ONLY to a deck that passed. **`shared/src/sim/pool.ts` `rollMatchPool(rng, catalog, CONFIG.pool)`** draws `size` (10) consumables uniformly, each ≤ its cap WITHIN THE POOL, on a room-private stream rolled ONCE at room creation (both doors, solo included). **`drawOffer(deck, rng, catalog, opts?)`** keeps its non-consuming shape, draws `CONFIG.offer.size` DIFFERENT lines, weight = copies, a line at cap never offered; `opts.guarantee` (level zero only) draws the first card uniformly among USABLE cards (a consumable, or a line whose next tier is I) and the rest from everything else — vacuous for a deck with no usable card, pinned. **Level zero**: `Match`'s countdown-entry hook calls `grantPoint` for every captain BEFORE `activate()`. **The mulligan** is `SpendMsg.choice === MULLIGAN_CHOICE` (`-2`, beside the retired `-1` in `offers.ts`), honoured iff `phase === 'countdown' ∧ !ship.mulliganed ∧ ship.offer !== null`, redrawing with the same guarantee and re-queuing `pt`; anything else a silent no-op (tests prove a second or live-phase mulligan leaves the offer byte-identical). Exhaustion is the existing degenerate path (empty draw banks silently, no `pt`), logged once as `deck.exhausted`.
AR41: **`OwnShip.deckLeft` — CORRECTION OWED TO THE ARCHITECTURE.** D22 puts `deckLeft` (= `deck.cards.length`) on `OwnShip` as the draw-pile counter's only wire number. The UX pass DELETED the counter (Eric 2026-09-10) and nothing renders it. Under the standing rule that **the wire may carry no finer resolution than the presentation actually consumes** (Story 4.9), a field with no consumer should NOT ride — **PROPOSED: drop `deckLeft` from the wire entirely** (the `deck.exhausted` log line and `/metrics` `deck.exhausted` stay); flagged to `gds-game-architecture`, Eric to confirm.

**Kinematics, occlusion, ordnance, damage (E8)**

AR42: **Wake drafting (D23):** `draftLift(ribbons, x, y, now, cfg)` in `shared/src/sim/wake.ts` = the MAX over every OTHER hull's ribbon (own and torpedo excluded) in `[0, cfg.lift]` within `cfg.halfWidthU`; `CONFIG.wake.draft.{lift, halfWidthU}` are `[DRAFT]` with no placeholder; `draftedKinematics(kin, lift, active)` in new `shared/src/sim/draft.ts` raises the forward cap by `lift × kin.maxSpeed`; the pinned fold `boosted → slowed → drafted → hookKinematics` asserted byte-identical at `world.ts stepShips` and `prediction.ts tickKin`; the server reads ribbons as they stood after LAST tick's `sampleWakes` (one tick old IS the definition, pinned); `OwnShip.draft?: number` self-private, omitted at 0; `prediction.setDraft(you.draft)` replays the latest scalar; parity pin at `draft = 0`; not a registry row, not an exception; bots get it passively.
AR43: **Smoke as a sight occluder (D24 / Novel Pattern 12).** `SmokePuff { id, ownerId, x, y, bornAt, until }` in a new `world.smoke` store; `puffRadius(puff, now)` in new `shared/src/sim/smoke.ts` = `r0 + (r1 − r0) × min(1, age / expandMs)`; NEW `CONFIG.smokeScreen.{r0: 40, r1: 60, lifeMs: 30000, layMs: 5000, puffIntervalMs, expandMs [DRAFT]}` — **the existing `CONFIG.smoke` is WOUNDED smoke and is untouched; the epics text must never conflate the two**; `smokeScreen.activate` stamps `ship.smokeUntil`; new `STEP_ORDER` row `stepSmoke` (after `sampleWakes`, before `applyStorm`). **ONE predicate `sightClear(a, b, islands, puffs, now)`** = `losClear && !puffs.some(segCircleHit(…, puffRadius))` at the SIX call sites of the five sight-tier sensors (`shipSees`, `pointSighted`, `pointDetected`, the `mz` halo, the `sm` halo, the foghorn's one-step muffle); `blipGate` and `buoyGate` untouched BY CONSTRUCTION (they never called `losClear`) — smoke is an island for every sensor but radar, one occlusion rule not two; `ownZoneCovers` gains no smoke term (a lit zone ignores smoke as it ignores islands — pinned, ledgered); a second occluder kind is one more term, never a second predicate. Wire: `SmokeView { id, x, y, t0 }` (no radius; the client runs shared `puffRadius`), a `smoke` pseudo-row visible iff the CENTRE is within `sight + puffRadius` with ISLAND-ONLY LOS or `ownerId === me` (a puff must stay visible from inside another puff), `FrameMsg.smoke?`; bots receive it via `observe()`; cost bound: live puffs ≤ stacks × `layMs / puffIntervalMs`, one `segCircleHit` per (observer, subject, puff).
AR44: **The ordnance target collector (D25 / Novel Pattern 11).** One `hitTargets(mask: TargetKind[])` in `world.ts` (`TargetKind = 'hull' | 'mine' | 'decoy' | 'ordnance'`; `Target { id, kind, poly }` replaces `shell.ts`'s `HullTarget`; memoized per tick per mask; decoys 12 u squares, mines + ordnance small polys); every ordnance row declares its mask in `CONFIG.<ordnance>.hits`: deck gun / machine gun / monitor → `hull | mine | decoy`; FLAK → `hull | mine | decoy | ordnance`; HORIZONTAL MISSILE, all three torpedoes and the captive's fish → `hull | decoy`; `stepShell`'s signature unchanged; `burstVictims` widens to the same kinds; **an ordnance step never enumerates world entities** (placement rule 13). **Outcomes by kind:** `mine` → `detonateMine` at its own position with its own blast, PERCEPTION-BLIND (Eric), chains same tick with a visited set; `decoy` → a shell damages it, a torpedo or missile DETONATES on it; `ordnance` → the struck fish/missile is removed, no boom (ledgered presentation gap); owner immunity for HULLS is absolute (NO FRIENDLY FIRE, Eric 2026-09-11) and own mines ARE hit by own shells (the one deliberate exception); **the owner's own fish PASS THROUGH the owner's own decoy and the owner's own flak never destroys the owner's own ordnance (Eric 2026-09-11 — the D25 `[DRAFT]` non-exempt readings are OVERRULED; one mask/owner check each); homing acquire set = `hitTargets(['hull','decoy'])` minus the owner's hull, `lead.ts` untouched; the monitor's plunging shell = `stepShell` with an `arcing` flag skipping terrain, shell radius vs target at the click; a missile's en-route direct hit uses `CONFIG.missile.contactDamage`. **This rewrite is also where three ledgered `hc`/`sp`/wreck-consumption defects (`deferred-work.md:249, 590, 594`) are decided, not inherited.**
AR45: **The held-fire stream (D26 / Novel Pattern 13).** `InputMsg.held: boolean` — REQUIRED, a LEVEL riding `InputStore.latest` (no intent-queue slots, not rate-capped beyond the message cap); a non-boolean drops the whole message; in `fireControl` after the click pass, if `held ∧ EQUIPMENT[primed].stream ∧ inArc(aim) ∧ n > 0`, `ship.streamAcc += dt` fires one shell per `CONFIG.machineGun.rateMs` at `now` with NO `fireT` and NO D1 back-date; release, arc exit or empty pool stops it; arc exit emits ONE `out-of-arc` denial then silence; `held` with a non-stream prime is ignored (pinned); client `mouse.ts` sets `held` = pointer down inside the primed stream's arc; `ownFire.ts` untouched; no prediction; the ammo model stays round-by-round with ONE optional switch `ammo.refill: 'round' | 'magazine'` RESERVED, not built, unless the `[DRAFT]` numbers demand it. This is the multi-round click-fire weapon Story 2.1's coalescing note said to *"revisit if one ever ships"* (`deferred-work.md:198`).
AR46: **Signals for the new lines (D27).** Wire kinds (`BallisticEvent.k`): `shell` (gun family — deck gun, machine gun, monitor, flak; sight-gated), `torp` (all three torpedoes, one kind, detect-gated, wake paints as today), NEW `missile` (sight-gated) with `missileU` (the `torpU` shape); every new ordnance kind gets its own wire kind and registry rows revealed at the sight boundary with position + velocity only; the reveal gains ONE field `w` (weapon FAMILY needed to draw it). **Gun-shell rules everywhere (Eric):** `sp` for the gun family AND missile AND monitor (never torpedoes); `hc` exactly one per shell resolution (per machine-gun shell, flak shell, missile, monitor shell); **`mz` PER SHELL for every gun-family weapon and the missile INCLUDING THE STREAM** — the broadside's `perShellFlash` opt-out becomes the rule for `stream` equipment while the multi-barrel deck gun's one-click salvo still collapses to one flash (epic-4 amendments 19/20 intact). **CHAFF** is a `FakeSource { x, y, radius, count, until, seed }` on the ship — `scatterJamFakes` generalized into `game/fakes.ts` `scatterFakes(seed, epoch, cx, cy, radius, count)`, emitted through the observer's own `blipGate` + `blipShape`, owner-exempt, epoch-rescattered per sweep; **the fakes must be WATER-FILTERED** (rejection-sampled against island geometry) — the ledgered jamming defect (`deferred-work.md:1434`) carries into chaff and must close here. **DECOY and SMOKE are frame CHANNELS** (`decoys`, `smoke`), not events; the shield is `OwnShip.shield?` and emits nothing.
AR47: **The damage gate (D28 / Novel Pattern 10).** ONE `applyDamage(victim, amount, src: DamageSource, byId)` in `world.ts` replaces the THREE hull-hp writers at `HEAD` (`applyStorm`, `hitShip`, `burnShip` — verified; + decoy contact = four callers); `DamageSource = 'shell' | 'burst' | 'torpedo' | 'missile' | 'mine' | 'burn' | 'storm' | 'contact'`; fixed order inside: shield → hp → assist ledger → sink check → `dmg`; SHIELD BLOCK absorbs there FROM EVERY SOURCE, storm and burn included (Eric); callers emit `hc`/`boom` on the HIT before calling (a fully-absorbed hit still calls); `ship.shield = { hpLeft, until }` expires at `until` or `hpLeft === 0`, a second shield REPLACES (Eric 2026-09-11); `payRepair` is the only hp-INCREASE path and never runs inside the gate; a lint restriction + grep pin assert exactly one `victim.hp -=` in `world.ts` (placement rule 12: *"a `ship.hp -=` outside it is a defect of the same class as a spatial emit outside `frames.ts`"*). Whether `tickRepairs` is gated with damage (it heals in the weapons-safe room today — `deferred-work.md:564`) is decided at this gate.
AR48: **Mines without caps, and the decoy (D29).** `CONFIG.mine.maxLive` (and its stat path), `CONFIG.mine.globalCap`, both eviction branches in `addMine` and the `maxLive` parameter are DELETED; the only bound is pools × reloads (20 hulls × (6 naval + 3 captive) per reload window); perf pin at 500 live mines; the harness reports the live-mine peak. A `decoys` store replaces the buoy store: `DecoyState { id, ownerId, x, y, hp }` dropped by `dropDecoy` into the mine's rear arc, no lifetime; `DecoyView { id, x, y, own, hp? }` with **`hp` present ONLY when `own` (Eric)**, stripped by `materialize()` for everyone else (the `sunk.seen` idiom); a decoy PAINTS on radar (Eric 2026-09-11) via the buoy's footprint path. **The radar buoy is DELETED** — equipment, `BuoyView`, `buoyGate`, the `gun`/`jamming` doctrines, `ownBuoyScopeBlips`, the `src` blip tag, `FrameMsg.buoys`, `CONFIG.radarBuoy`, `radarBuoy.ts`; `scatterJamFakes` survives as CHAFF's generator. The ledgered *"the decoy lays no wake — reopen WITH the decoy rework"* thread (`deferred-work.md:728`) and the 30 s owner-death persistence tell (`:98`) reopen HERE.
AR49: **The Shift boost (D30):** a PERMANENT slot (`boost`, slot 1) through the unchanged interface (`isWeapon: false`, `actSeq` channel, `Shift` → `actSlot: 1`); `boostUntil` remains its only writer; `bonus = CONFIG.boost.factor (0.25 [DRAFT]) × kin.maxSpeed` from POST-FOLD kinematics so the SPEED ladder is inside it (Eric: ladder-raised max — a capped Torpedo Boat boosts to 68.75 u/s); `boost.reloadMs` (20 s `[DRAFT]`) takes `cooldownScale`; the card-fitted speed-boost EQUIPMENT and `CONFIG.speedBoost` deleted; the boost stays outside `EffectiveStats`. **GDD open note 19 is CLOSED (Eric 2026-09-11):** the LIGHT TORPEDO at 45–55 u/s no longer outruns a boosted Torpedo Boat, and that is fine — there is NO FRIENDLY FIRE, so own ordnance can never damage the owner's hull; FR7's outrun law and its pin are RETIRED as requirements (the owner-grace mechanism is subsumed by absolute owner immunity), the catalog speeds stand and the boost stands.
AR50: **Bot decks and consumable tactics (D31).** `ai/decks.ts`: `BOT_DECKS: Record<BotProfileId, LineId[]>` (40 each) legality-checked by `checkDeck` in a BOOT-TIME test; a bot is seated through the same `addShip(deck)` path and receives the room's pool; `ai/consumables.ts`: `CONSUMABLE_TACTICS: Record<ConsumableId, ConsumableTactic>` TOTAL, deep-frozen, `{ want(ctx), solve?(ctx) }` (only the decoy solves a placement); `EQUIPMENT_TACTICS` stays total over the WIDENED `EquipmentId`; `spending.ts` drops `HEAL_CHOICE`, weights LINE ids including consumables, consults `canStock`, mulligan policy never; the harness gains `--deck authored | random-legal` arms (replacing `--bots N`'s implicit fixed fit) with the pool rolled per match; the `pacifist` storm control keeps its gun-only posture as a deck of zero equipment lines (legal by design); `ai/` still never imports `world.js`. The `siege` weight table is re-authored, not ported (it was never retuned for the 5/8-rung broadside — `deferred-work.md:1454`), and the ML tables' dead `buoyGun` override goes with the buoy.

**Cross-cutting (both epics)**

AR51: **Logging.** New dot.case events — `account.signin { provider, userId }`, `account.link`, `account.signout`, `account.delete`, `deck.rejected { userId, code }`, `account.write.ok { matchId, rows }`, `account.write.failed`, `account.disabled { reason }`, `match.pool { matchId, count }` (once per room, count only), `deck.exhausted { matchId, shipId }` (once per ship) — under the PII rule of NFR24.
AR52: **Configuration.** Shared `CONFIG` gains `deck` (`size: 40`, `maxEquipmentLines: 3`), `progression` (placement XP curve, `soloXpFactor`, `matchesToCatalog` — OPEN, no placeholder — and the flat per-copy `tokenPrice [DRAFT]`), `pool` (`size: 10`), `catalog` (`reloadStepPerTier: 0.05`), `wake.draft`, `smokeScreen`, `shield`, `chaff`, `decoy`, and one block per new equipment (`lightTorpedo`, `heavyTorpedo`, `supercavTorpedo` — one torpedo shape — `machineGun` (`rateMs`, `stream: true`), `flak`, `monitor` (`arcing: true`), `missile` (`contactDamage`), `captiveMines`) plus every ordnance row's `hits` mask and `boost.factor`. **`shared/src/sim/catalog.ts` IS the catalog** (29 lines, tiers, caps, `appliesTo`, the three starters, `FLEET_FIT`) and `BOON_CATALOG` in `boons.ts` is deleted; catalog content is wire contract. DELETED: `CONFIG.radarBuoy`, `CONFIG.speedBoost` (as equipment), `CONFIG.mine.maxLive`, `CONFIG.mine.globalCap`, `CONFIG.deck.rareWeightBase` / `rareWeightPerDryLevel`, `HEAL_CHOICE`. `CONFIG.damageControl` SPLITS: the paid heal's two numbers move to `hullRepair`; the FREE per-level auto-heal's fields (`levelMissingPct`, `levelRegenMs`) STAY (GDD note 14 unruled). **Every `[DRAFT]` is a CONFIG dial the harness tunes; none is invented and none is given a value the catalog did not state.**
AR53: **Events and frame channels.** Still no event bus (AR13 reaffirmed). `FrameMsg.buoys` DELETED; `FrameMsg.decoys?` and `FrameMsg.smoke?` added as registry pseudo-rows; `OwnShip` gains `draft?`, `shield?` and renames `boons` → `cards` (and `deckLeft` per AR41's proposal); `InputMsg` gains `held`; `SpendMsg` gains `MULLIGAN_CHOICE` and loses `HEAL_CHOICE`; `PerceptionView.{smoke, decoys}` replace `buoys`. Client-side: one `account` slice in `state.ts` (`{ status: 'anonymous' | 'signedIn', userId?, profile?, decks?, progress? }`) written ONLY by `net/account.ts` from HTTP responses and read by `ui/`; the deck editor is DOM chrome that never touches the sim; one-way flow stands.
AR54: **Observability.** `/metrics` gains `account: { signIns, writeOk, writeFailed, queueDepth, deckRejected }`, `deck: { picks, mulligans, stockRefused, exhausted }` and `world: { minesLivePeak, smokeLivePeak, streamShellsPerTick }`; `/liveness` gains `account: boolean` (the client's only signal to render SIGN IN); `@colyseus/admin` is the read surface for history — an ops console, not a metric.
AR55: **Debug and development tools** (all `HC_DEV_OPTIONS` / dev-gated): `HC_DEV_DB=pglite` (real schema in-process, dev/test only), a `devSignIn` endpoint minting a token for a named local user (never mounted in production), `deckOverride` and its sibling `poolOverride` (line ids) so a headless smoke asserts a deterministic pool, `matchOverride` learning a `mulligan` beat so the countdown path is smoke-testable, the batch-sim `--deck` arms. Setup: `HC_DEV_DB=pglite HC_DEV_OPTIONS=1 npm run dev` (accounts ON locally) vs plain `npm run dev` (accounts OFF, byte-identical); `npx drizzle-kit generate --config server/drizzle.config.ts` after any schema change, SQL committed; E8 adds no new env var, service or package.
AR56: **New homes.** Shared: `sim/catalog.ts` (THE catalog + helpers), `boons.ts` (engine only), `deckRules.ts`, `progression.ts`, `pool.ts`, `draft.ts`, `smoke.ts`, `wake.ts` (+ `draftLift`), `arcs.ts` (+ six arcs: light twin-sector; heavy / supercav / monitor / missile / machine-gun bow sectors; decoy = the mine's rear), `aim.ts` (+ missile burst point, monitor landing point, machine-gun range clamp), `hooks.ts` unchanged and STILL EMPTY. Server: `account/` (`index.ts`, `db.ts`, `schema.ts`, `auth.ts`, `oauthCallback.ts`, `session.ts`, `decks.ts`, `progress.ts`, `history.ts`, `writer.ts`, `deletion.ts`, `endpoints.ts`, `throttle.ts`, `devSignIn.ts`), `drizzle/`, `game/matchRecord.ts`, `game/fakes.ts`, `equipment/index.ts` (a TOTAL registry of 18 rows: gun, boost, lightTorpedo, heavyTorpedo, supercavTorpedo, navalMines, captiveMines, missile, machineGun, flak, monitor, broadside, starShells, hullRepair, shieldBlock, smokeScreen, chaff, decoyBuoy), `torpedoCore.ts` + three torpedo files, `captiveMines.ts`, `missile.ts` / `machineGun.ts` / `flak.ts` / `monitor.ts`, `equipment/consumables/{hullRepair, shieldBlock, smokeScreen, chaff, decoyBuoy}.ts`, `ai/decks.ts`, `ai/consumables.ts`; `radarBuoy.ts` DELETED; `roomOptions.ts` (+ `deckId`, rejects `deck`, `deckOverride`, `poolOverride`); both rooms (+ `verifyToken`, `loadDeckFor`; arena + `MatchRecord → writer`); `app.config.ts` (+ ONE spread of `accountEndpoints()`, the admin mount, the writer to both rooms, shutdown drain); `liveness.ts` / `metrics.ts` additions. Client: `state.ts` (+ account slice), `net/account.ts`, `settings/store.ts` (+ account source), `ui/signIn.ts`, `ui/history.ts`, `ui/settings.ts` (+ SIGN OUT / DELETE ACCOUNT), **the merged Ship & Deck screen (`ui/deckEditor.ts` + `classSelect.ts` MERGE — UX 2026-09-11 corrects the architecture's separate `deckEditor.ts`)**, `render/smokeScreen.ts` (NEW; `render/smoke.ts` stays WOUNDED smoke), `render/decoys.ts` (replaces `buoys.ts`), `hotbar.ts` (9 slots on the bottom-centre bar), `hud.ts` (− DAMAGE CONTROL rail; the HP/helm globes and XP strip per UX-DR40+), `weaponArc.ts` / `aimPreview.ts`, `ui/upgradeMenu.ts` (mulligan, greyed cards, − key 5), `net/snapshots.ts` (+ missile interpolation), `render/projectiles.ts`. No new workspace, no new deployable.
AR57: **Naming.** DB tables/columns snake_case plural with an `account_` prefix on account-owned tables; migrations `NNNN_<slug>.sql`; line ids camelCase = the equipment id; HTTP paths `/api/account/<noun>`, auth at the module's `/auth/*`; error codes dot.case `<domain>.<reason>`; env vars `HC_`-prefixed for ours, the module's own names verbatim (`JWT_SECRET`); consumable modules `equipment/consumables/<id>.ts`; ordnance masks `CONFIG.<ordnance>.hits: TargetKind[]`; frame channels plural nouns matching the registry pseudo-row; spend sentinels `SCREAMING_CHOICE` negative integers in `offers.ts`; tick rows verb-first camelCase in `STEP_ORDER`.
AR58: **Five new placement rules (9–13):** (9) persistence lives in `server/src/account/`, never reaches the sim, three room sites only; (10) a deck enters a match only through `loadDeckFor`; (11) every catalog fact lives in `shared/src/sim/catalog.ts` (display names excepted); (12) hull damage enters through `applyDamage` and nowhere else; (13) an ordnance step never enumerates world entities.

**From the deferred-work ledger — threads E8 and E9 must close, carry, or explicitly park (bind story scope; line refs are into `deferred-work.md`)**

AR59: **Threads E8 CLOSES BY DELETION** (confirm in the story, then stamp the ledger): every radar-buoy thread (`:1409` unmeasured, `:1424` relay along the owner's ray, `:1429` `BuoyView` without hp, `:1449` gun-buoy behaviours); the doctrine-rival ping-pong and rivals-only exhaustion definition (`:302`); the rarity pity inversion at deck 41 (`:1459`) and the drifted rare draw rate (`:459`); the thinned `speedBoost` / `decoyBuoy` subdecks awaiting replacement lines (`:445` — catalog v3 IS the ruling); the exclusive swap-and-refund laundering (`:262`, second half); the smoke-screen deferral (`:262`, first half — smoke returns as a consumable); the refit band's five-pixels-of-slack and overlap pins (`:571`, `:575`, `:244`); the digit-5 ratified binding (`:582`); the `weaponsSmoke.mjs` dead mine phases (`:92`); the "1 of 6 acquisition cards can ever fire" finding (CLAUDE.md cycle 93 — never ledgered, closed here by name).
AR60: **Threads E8 CARRIES** (each must land in a story's ACs or be explicitly parked): the jamming fakes not water-filtered → CHAFF (`:1434`); the owner-exemption and own-shells-intercept-own-buoy readings → DECOY BUOY (`:1419` → `:1861`); the decoy's missing wake and 30 s owner-death tell (`:728`, `:98`); the click-coalescing revisit clause tripped by the machine gun (`:198`); the offer-less terminal level re-derived for 40-card decks + mulligan + the consumable heal (`:982`); the mid-click-kill `hc`/`sp` grammar and same-tick wreck consumption at the collector rewrite (`:590`, `:594`, `:249`); the same-owner mine cascade, which uncapped mines WIDEN (`:496`); the captive fish's no-max-range tripwire if any torpedo line gains a range (`:1444`); the reload-timer proportional rescale rule surviving into tier grants (`:463`); the refit band's physical-vs-CSS scaling defect, which the replacement must not inherit (`:579`); the `isOverlayFocused` convention for any new focusable in-match DOM (`:201`); the ability-FIFO cap and transport-coalescing press swallows under nine slots (`:119`, `:116`); the ACTIVE hotbar state for timed consumables (`:206`); `tickRepairs` un-gated in the weapons-safe room, decided at the damage gate (`:564`); heal observability inside truesight (`:555`) and the heal tone + the 4-7 listening pass for every new cue (`:559`, `:792`); the `encounterSpan.ts` killing-blow bias the v3 balance pass inherits (`:1744`); the "blind-vacuum control is not valid across cycles" method rule (`:1630`); `siege` never retuned (`:1454`) and the ML profile findings (`:1541–1640`); the standing "torpedo speed 60 is FIXED" record, superseded by cycle 122 and v3 and needing a stamp (`:417`); the "i have enough systems" phase ruling, which E8's new lines and smoke satisfy only by citing Eric's 2026-09-10 rulings as the fresh authority (`:1064`).
AR61: **Threads E9 CLOSES OR CARRIES:** `hullcracker.mode` written with no reachable reader — the two-state prefs source must give it a reader or delete it (`:1193`); `LivenessPayload.queue` with no consumer + `parseLiveness` failing the whole payload closed on it — resolve together when `/liveness` gains `account` (`:1359`, `:1369`; a wire ruling); `/liveness` negative cache / client backoff (`:1198`); the cycle-105 inline-`visibility` defect class on `queueModal.ts` and `showHome()` mounting unyielded — every new home ancestor (sign-in row, Ship & Deck) follows the structural pin (`:1374`, `:1379`); the settings ANALYTICS row's EEA truth and the consent-storage fail-open (`:1389`, `:1404`) — not papered over by the settings migration; the half-resume double-fault window and the SDK token-rotation line the 0.18 upgrade may move (`:29`); the reconnect `sunk` catch-up that `MatchRecord` could serve (`:544`); `endedBy: 'lastHumanLeft'` unreachable, revisit before history persists the enum (`:921`); the privacy policy's plain-text links (`:1234`); `client/.env.*` not gitignored (`:1789`); the Story 7-7 revival brief's account-API addendum (`:1817`).
AR62: **Open Eric questions E8/E9 would ANSWER — or must explicitly PARK, never pre-empt. RESOLVED at the 2026-09-11 step-1 walk (stamp the ledger when the stories land):** heals during the collapse — allowed, closed (`:1030`, `:1684`); the results boons/last-offer blocks — closed by the LOADOUT ruling (`:964`); LIGHT TORPEDO vs the boosted hull — closed by NO FRIENDLY FIRE (`:1836`); SHIELD BLOCK replaces + no shooter tell (`:1856`); the decoy paints on radar, own ordnance passes through it, own flak spares own ordnance (`:1861`); the free auto-heal stays as built for now (`:1886`); BARREL's 12 u spacing stays as shipped for now (`:1439`); history shows drawn pool cards only (`:1871`). **STILL OPEN:** sudden death's draw-not-winner tiebreak (`:1030`); heal observability inside truesight (`:555`); the DECOY SHIP expansion class, whose premise the decoy consumable re-supplies (`:1677`); duplicate-equipment fits, now legal by the ≤ 3-lines rule (`:234`); wounded smoke stopping at sink-entry (`:938` — adjacent, exclude explicitly); can a quitter win (`:1212`); live spectate (`:1208`); the D6 steer copy (`:1364`); GPC settings-row copy (`:1779`); `healthCheckPath`, dead prod env vars, `www` DNS, a `dev.hullcracker.io` name (which OAuth redirect URIs make load-bearing), `domains` in `render.yaml`, branch protection (`:1705–1738`); `/admin` on the public host (`:1812`); the results modal reopen path (`:228`); the home scene showing combat (`:994`). **Anonymous play on production and the GDPR named controller appear nowhere in the ledger** — the former is Eric's open question (FR61), the latter's absence IS the ruling (cycle 108).
AR63: **The corrections owed upstream get a home (Eric 2026-09-11, party-mode review).** The UX pass's list — draw-pile counter deleted, results = final LOADOUT, ship ladders start at Tier I equipped, deck names allowed, copies acquired per copy, the `/decks` route gone (Ship Select + deckbuilder merged), the dev host account-only, keyboard-only out of scope, Sticky Keys accepted — plus this run's own (no friendly fire retires the outrun law; `OwnShip.deckLeft` off the wire; the pre-queue legality gate; "starter" → `DEFAULT` in copy; key rebinding in) are RECORDED here but still contradicted by `gdd.md`, `game-architecture.md` and older ledger entries. **Epic 9's final story is a design & doc reconciliation (the Story 7.6 pattern)** that writes every one of them back, stamps the resolved ledger threads named in AR59–AR62, retires the unused `shield-ring` token from DESIGN.md, and updates both tracker files — because the implementing agent reads those documents first, and a retired sentence left standing is a landmine, not laundry.
...
UX-DR5: **[EXCLUSIVITY CLAUSE SUPERSEDED 2026-08-21 (Story 7.6) — epic-2 amendment 49, then epic-3 amendment 1.]** The storm's own rendering stands exactly as written. What is retired is the *"purple is never used for anything else"* clause: **purple exclusivity is dead; the binding requirement is ring-edge legibility** (epic-3 amendment 1: *"purple exclusivity is dead; what matters is that players can easily see the edge of the ring against everything else on the water"*). The rule is now scoped to the WATER — don't use purple on the ocean for anything but the storm — with **two ratified carve-outs, both in port/modal chrome rather than on the ocean**: the refit card's `storm-readout` rung III of the loot-tier lineage ramp (live today), and the EXCLUSIVE rarity register that first opened the exception (epic-2 amendment 49, Eric choosing it over the recommended white-hot register with the violation squarely presented) — **that tier is now extinct, deleted with exclusivity at Story 7-5**, so the ladder rung is the only live instance. Anything new that wants purple needs the same explicit call. Original: The storm renders in violet exclusively (#7B2FBE fill + #B06EE8 readout text AND on-water edge stroke — the fill alone fails the 3:1 graphics threshold); purple is never used for anything else.

**Regatta Hoist & silhouettes**

UX-DR6: Personal combatant colors: every human gets a unique hue from the 20-hue wheel, server-assigned match-consistently first-come-first-served at join (color index rides the roster); players pick a preference on home (granted if free; if taken, nearest free hue clockwise, ties seeded-random among free; colors never change mid-match; UI must never imply claiming/locking) (Eric ruling 2026-07-23, Story 1.12 — supersedes match-start fair-random draw); nameplate reveal in the waiting room (contested-hoist toast REJECTED, Eric 2026-07-23).
UX-DR7: **[SUPERSEDED IN PART 2026-08-21 (Story 7.6) on two clauses.]** **(1) The decoy-buoy truth-marker is gone**: the DECOY ROLE was deleted entirely at Story 7-5 (the RADAR BUOY replaced it — flat 330u own radar relaying returns only), so **nothing fakes a ship contact any more** and there is no decoy to render in the firer's hue. Mines and lit zones keep the truth-marker grammar unchanged. **(2) The Variant C / Variant P fork for BLIPS is retired** — Variant P (the phosphor-anonymous build flag) went at cycle 51 with the monochrome return grammar, and Variant C (per-hue blips) at cycle 105: radar returns now take **heatmap band colours by return strength**, never a personal hue, so hue no longer propagates to the scope at all. Hue propagation to own hull, nameplate, own-blip ring and kill-feed names is unchanged, as are the phosphor-functional HUD chrome and greyscale-drone laws. Original: Hull treatment: bright personal hue on the outline, same hue at ~45% value as interior fill; propagation = own hull, nameplate, own-blip ring, radar blips + kill-feed names (Variant C default; Variant P phosphor-anonymous build flag kept for playtest swap) plus ordnance truth-markers (mines, lit zones, decoy buoys) in the FIRER's hue for ALL observers via wire attribution — a deliberate intel grant (Eric ruling 2026-07-23, Story 1.12); ALL HUD chrome stays phosphor-functional; drones always greyscale.
UX-DR8: **[RESERVE NARROWED AND THE OPEN QUESTION CLOSED — Eric ruling 2026-08-21, question gate B1.]** **Reserved, never a combatant hue: amber (~25–52°) and the red family (~345–25°). That is the whole reserve.** The **storm-violet (~266–286°) and phosphor-green (≈132–172°) reservations are RETIRED** (Eric: *"ima be real, i see myself probably expanding the colors and some other shit in the future. Lets maybe just get rid of the 'law' here."*). The cycle-125 wheel ships exactly as generated — **no hue moved** — so `player-green` (149.8°), `player-spring` (164.3°) and `player-jade` (171.6°) now sit inside the old phosphor band and `player-orchid` (270.9°) inside the old storm-violet band. **Recorded honestly: the phosphor reservation existed so a captain's hull could never be mistaken for a radar return, and it is retired BY PREFERENCE, to keep the palette expandable — not because the concern was found to be wrong.** The surviving amber/red reserve is pinned against the live wheel by `client/src/__tests__/tokens.test.ts`. The text clause stands and its open question is **RESOLVED as NO TABLE — decided, not open**: `textSafe(rgb, bg)` (`client/src/util/color.ts:93`) derives the lift ALGORITHMICALLY, returning the hue unchanged when it already clears 4.5:1 against `void` and otherwise lightening toward white in uniform steps until it does. Since the cycle-125 regeneration every hue clears 4.5:1 unaided (min 5.10), so the lift is currently **inert for all twenty**; the mechanism is kept because it is what guarantees the bar.
...
UX-DR10: Blip rendering: outline-only 1px non-scaling stroke at true heading with an arrowhead heading vector; class blips never below 11 px (Battleship 14, Mine Layer 12, boats 11 floor-clamped); Mine Layer notch cut ~3× deep in the blip path only; ≤ 3 decay ghosts per contact (TTL-based); per-hue luminance floor so dark hues render a lightened variant at blip/ghost scale. **[SUPERSEDED IN PART 2026-08-21 (Story 7.6)]** — the per-hue blip clause is MOOT: radar returns take heatmap band colours by return strength (Story 4.10), never a personal hue, so there is no dark-hue blip to lighten. The stroke, heading-vector, minimum-size and ≤3-ghost clauses stand.

**HUD components (Pixi)**

UX-DR11: **[CONTRADICTED ON ITS KEY MAP 2026-08-21 (Story 7.6)]** — the shipped scheme is `SLOT_KEY_CODES` in `client/src/input/keyboard.ts:77-84`: **the gun is slot 0 and has NO key** (it is the always-selected default), **Q/E are the two class-special slots**, **R is the pickup/extra slot**, and **F is the FOGHORN** (Story 4.5, epic-4 amendment 56; `keyboard.ts:349`), not an offer slot. The state grammar below is unaffected and stands. Original: Hotbar: four 54 px slots in a vertical bottom-left stack, keys Q/E/R/F mapped top-to-bottom (Q gun, E/R specials, F offer slot), with the full documented state grammar — ready weapon / ready ability (+chamfer) / selected (amber outline + inset wash + filled key chip — dual-coded) / cooling (conic perimeter track + seconds) / activated flash (≤ 80 ms) / empty ("— awaiting refit —" dashed) / denied (80 ms red edge pulse + icon flash).
UX-DR12: Hotbar satellites: ammo badge (mono count, top-right overhang, only on systems storing > 1 round), XP rail (3px vertical phosphor fill toward next level, LV tag), banked-level chip (30 px, count, 2.4 s breathing glow decaying to static after ~10 s unspent, re-arming on new bank or Space touch; hidden at zero), "HOLD SPACE TO REFIT" cue line. **[SUPERSEDED 2026-08-21 (Story 7.6) on its Space clauses — Eric ruling D1: Tab is ratified.]** The shipped cue reads **`▲ LEVEL UP — TAB TO REFIT`** (`client/src/ui/upgradeToast.ts:22`, `render/xpRail.ts`), and the chip re-arms on a new bank or a Tab touch. Space is bound-inert (prevented so the page cannot scroll, no action). The ammo badge, XP rail and chip behaviour stand.
...
UX-DR14: **[SUPERSEDED IN PART 2026-08-21 (Story 7.6) — Eric ruling D1.]** The "SPACE is hold-not-toggle (absolute)" clause is **RETIRED**: the ratified binding is **Tab to open/close (a toggle, not a hold)**, `1`–`4` to pick a card and `5` for the DAMAGE CONTROL heal (`HEAL_CHOICE`) — `client/src/input/keyboard.ts`. The four-card / spatial-1–4 / spend-latch / queue-pip / denied-pulse clauses are shipped and STAND, and "while held" reads as "while open". Original: Refit window: SPACE is hold-not-toggle (absolute); four 216 px cards side by side in the below-center band (never occluding listening ring or own hull; never wrapping — 1–4 keys map spatially); pick with 1–4 while held; queue pips + dashed ghost edge show waiting offers; hotbar dims to 38% and Q/E/R/F suspend while open, helm stays live; spend-in-flight latch; server rejection = denied pulse on the card, level stays banked; release or last-spend dismisses.
...
UX-DR16: **[AMENDED 2026-08-21 (Story 7.6) — one register added, one word redefined.]** The shipped register is `n AFLOAT · n KILLS · T+mm:ss · <ring readout> · ☠︎ <NAME>` (`client/src/ui/chromeBar.ts`): the **KILL LEADER register** (Story 4.6) rides at the TAIL, deliberately, as the bar's only OPTIONAL segment — omitted entirely while the throne is vacant rather than printed empty — and it is the first PER-PLAYER HUE the bar has ever carried. And **`AFLOAT` counts PARTICIPANTS, not humans**: every contesting hull including AI captains and bots, the local player included (epic-6 amendment 30, superseding the epic-4 humans-only wording; the KILLS segment beside it deliberately differs, counting ALL of the local player's kills including drones). The ring readout, the 1 Hz amber final-10s pulse and the no-supply-drop clause are unchanged. Original: BR Chrome Bar (top-center): `n AFLOAT · n KILLS · T+mm:ss · RING CLOSES m:ss` in restrained mono; ring readout pulses amber at 1 Hz in the final 10 s; no supply-drop reservation.
UX-DR17: **[FIGURES CORRECTED 2026-08-21 (Story 7.6)]** — shipped capacity is **6 lines / 8000 ms TTL** (`client/src/ui/killFeed.ts:23-24`), set by the Public Register (cycle 45), not 5 lines / 6 s. The same stale figure appeared in Story 1.12's acceptance text below and is corrected in the same pass. Kill feed (top-right, DOM): mono 14 px uppercase, max 6 lines, 8 s TTL, newest on top; vessel names 600-weight in text-safe personal variants, drones grey, connective text secondary; 14-char callsign cap at entry, longer legacy names mid-ellipsize; per-line dark-glass scrim is the sanctioned fallback if feed-vs-blip confusion is confirmed.
UX-DR18: **[DORMANT 2026-08-21 (Story 7.6) — deferred with Story 4-1, Eric 2026-08-04.]** The Listening Ring was never built and this spec is not live guidance; it is retained for whenever sonar returns (Eric, 2026-08-21: *"very deferred. sonar might come back in the future, but radar is plenty deep enough"*). The shipped bearing-grade surface is the **foghorn chevron** (Story 4.5). Original: Listening ring: dashed 48-pip compass rose around own ship (~half truesight radius visually); segments light toward noise with brightness ∝ loudness/closeness — pure intensity grammar, deliberately source-ambiguous (never encodes what, only where and how loud); sight is the confirmation channel; renders on the fog-immune chart layer above refit cards.
UX-DR19: **[SUPERSEDED / STRUCK 2026-08-21 (Story 7.6) — Eric rulings 2026-08-10, epic-4 amendments 216-227.]** **The Bounty Bloom was DELETED end to end**: no radar paint, bloom, ring, bearing, range or area disclosure of the holder, ever. The throne is **IDENTITY ONLY** (`ArenaState.bountyId`), which discloses nothing a client could not already derive from the roster's per-player kill counts. The player-facing name is **KILL LEADER**; the `BOUNTY: ` label is retired. Shipped presentation is three surfaces: a persistent chrome-bar `☠︎ <NAME>` register in the leader's text-safe hue with a static faint glow, a `☠︎` mark riding the leader's name in any feed line (as killer OR victim) with the claim register `☠︎ <NAME> IS THE NEW KILL LEADER`, and a `YOU ARE THE KILL LEADER` toast plus the `bounty` audio cue when the throne lands on you. Original, retained as the record: Bounty bloom visual: an expanding ring (1 px → 3 px, ~2 s decay) in the leader's personal color around their class blip at true position — visually distinct from sweep paints; feed line "BOUNTY: <NAME>" on activation.
UX-DR20: Torpedo/mine on-water rendering: torpedo hull dash + wake astern in {torpedo}; materialization = pale boundary rings at the sighting point so listening-pips → sight reads as one continuous event; mines render in the same register at truesight. **[RUNG CORRECTED 2026-08-21 (Story 7.6) — Story 4.9.]** Mines confirm at the **3/8 DETECT rung** (`pointDetected`, `server/src/game/signals.ts:555` — 247.5u base), **not at truesight**: Eric asked for the mine/torpedo detect gate by name and it is a real combat buff. `pointDetected` is a SIBLING of `pointSighted`, never a narrowing of it — only three call sites moved (mine, the `torp` branch, `torpU`), and **shells did not** (a shell is in the air; a torpedo is a wake just under the surface). The rung is OBSERVER-SCALED. The torpedo hull dash, wake and pale-boundary-ring materialization grammar are unchanged.
...
UX-DR23: Toasts: top-center phosphor mono 16 px, 3 s TTL, max 3 stacked, self-events only (level banked, boon fitted) — never enemy information. (Hoist-fallback toast REJECTED, Eric 2026-07-23.)
...
UX-DR25: Home page over a live ambient CIC canvas (never blank): wordmark (`.io` in phosphor-bright), callsign field (14-char cap), Class Chip (current pick at a glance), Color Hoist (20 swatches + preference caption), amber Primary Button ("SET SAIL" with mode/class sub-line, Enter equivalent, defers to status line while connecting; failures report plainly on the status line), mode pick (Solo · Solo vs AI), How-to-Play link, server status, settings gear. **[SUPERSEDED IN PART 2026-08-21 (Story 7.6) — epic-6 amendments 31 and 50.]** The single `SET SAIL` button plus its mode pick is replaced by a **MODE ROW**, built to take DUO and TRIO as in-line siblings later: **`SOLO`** (the primary — amber outline + glow, never a filled slab; relabelled from `PLAY` by Eric, *"I want the current 'PLAY' button to say 'SOLO' and nothing else"*) over **`SOLO VS AI`** (the second door, the same shape UNLIT in phosphor, taking its non-dominance from the results modal's SPECTATE precedent). **Both mode sub-lines are DELETED** (Eric, amendment 50: *"Just get rid of that text entirely instead of fixing it… it doesn't need to be there"*), which also **retires this row's mode/class sub-line clause and overrides EXPERIENCE.md:67**. Two shipped registers this entry never listed: the **bottom-left liveness register** (`PLAYERS ONLINE` / `LIVE GAMES`, Story 6.6 — rendered only when the `/liveness` read is available, absent rather than zeroed when it is not) and the **version register** on the wordmark (`v<VERSION>`). The Color Hoist has since moved off home — see UX-DR26.
UX-DR26: **[THREE CORRECTIONS 2026-08-21 (Story 7.6).]** **(1) The middle pip row is `ARMOR`, not `TOUGHNESS`** — a COPY ruling only (epic-7 amendment 35, Eric 2026-08-21); the internal key is unchanged, and the change was forced by fit: `TOUGHNESS` measured 96.4px into an 88px cell and overflowed it, while `ARMOR` is 53.6px. **(2) Enter ≡ CONFIRM SELECTION** — the layer carries an explicit CONFIRM SELECTION action in its footer which saves the class and returns to port; arrows and 1–3 move the highlight only, and ESC still closes without change. **(3) This layer is the SOLE home of the Color Hoist** — the duplicate home picker is retired, so UX-DR25's home-mounted Color Hoist has moved here. Everything else stands. Original: Class-select layer: Class Cards (356 px: name+key, silhouette box, SPEED/TOUGHNESS/TURNING pip scales (real values on absolute anchors, Eric 2026-07-24), two special-slot rows keyed Q/E, pick button — no fantasy line, no GUN row (Eric post-merge ruling 2026-07-24); selected = personal-color border/glow) on a horizontal rail with a dashed ghost card ("MORE CLASSES IN DEVELOPMENT") clipped at the edge; keys 1–3/arrows highlight, Enter picks, ESC closes without change; first-run class select pushes no default — three cards, forced meaningful choice, Torpedo Boat pre-focused for keyboard flow (ruled 2026-07-19; closes the old "default Gunboat" proposal).
UX-DR27: Results modal: kills, placement, time afloat, accrued boons + last offer reviewable; **[SUPERSEDED IN PART 2026-08-21 (Story 7.6) — epic-2 amendments 22/23, epic-7 amendment 17.]** The "single action / no dead spectate button" clause is retired: the modal renders **TWO** actions, **SPECTATE** and **RETURN TO PORT**, and **ESC means SPECTATE** (Enter confirms RETURN TO PORT). Spectate is a real destination, not a placeholder, so the clause's actual intent — no dead button — is discharged rather than overruled; ESC from spectate reopens the score screen (epic-7 amendment 17). The **no-re-queue-from-the-modal** clause STANDS and is pinned by test (Eric: you MUST return to the home screen to requeue). Original: single action RETURN TO PORT (amber Primary Button; Enter or ESC) — no re-queue from the modal, no dead spectate button; victory banner phosphor / defeat amber; fullscreen dim behind results only.
...
UX-DR29: **[GLOSSARY CLAUSE STRUCK 2026-08-21 (Story 7.6) — Eric ruling, epic-7 amendment 15 (R1).]** **The shipped How-to-Play page hosts NO boon glossary and never will** — Eric: *"No need for a boon glossary"*, restated when the structure question came back as *"NO FUCKING GLOSSARY. One page."* **Struck, not deferred**, here and in FR39. It was also unbuildable as specified: any hand-copied glossary would have gone stale silently within two stories. The content that clause wanted lives in two places that cannot drift — the **refit card's hover tooltip** (Story 7-5, generated from the catalog) and the page's own **`UPGRADING`** section. Shipped sections are THE OBJECTIVE · STEERING · SHOOTING · EQUIPMENT · UPGRADING (`client/src/how-to-play/copy.ts`); the win condition survives from FR39 because it is stated nowhere else a new player can read. The static-DOM, ESC-returns-home, Solo-vs-AI-as-live-tutorial and coach-mark clauses all stand.
UX-DR30: **[SUPERSEDED 2026-08-21 (Story 7.6) on its waiting-room clause — epic-6 amendments 1/8; and on its disconnect clause — Story 0.2 / Story 6.7 (FR38).]** **The sailable weapons-safe waiting room NO LONGER EXISTS in matched play.** It is replaced by a **frozen held start line**: movement locked, weapons locked, radar off, spawn position disclosed. The sailable, genuinely-firing weapons-safe grammar survives ONLY behind the dev/sandbox door. The disconnect clause is likewise superseded: a disconnected ship keeps simulating under its last input and stays a huntable participant, and the player resumes the SAME ship by token-authenticated reconnection while it is alive or sinking (FR38) — it is not a banner-and-go-home. The countdown and failed-connection clauses stand. Original: Match-flow surfaces: waiting room shows the full live HUD with "AWAITING CAPTAINS n/2" + "WEAPONS SAFE" (weapons genuinely fire, damage suppressed — not a denied state); countdown = "MATCH STARTING" + big center count; disconnect mid-match = banner + return home; failed connection surfaces on the home status line, never a dead screen.

**Input scheme (EXPERIENCE.md — supersedes current bindings)**

UX-DR31: **[CORRECTED 2026-08-21 (Story 7.6)]** — three clauses below are stale against the shipped `client/src/input/keyboard.ts`: the gun is slot 0 with **no key** (Q/E are the class specials, R the pickup/extra slot); refit is **Tab-toggle**, not SPACE-hold, with `1`–`4` picks and `5` for the DAMAGE CONTROL heal, and Space is bound-inert (Eric ruling D1); and the **foghorn is bound to F** (epic-4 amendment 56, `keyboard.ts:349`) — the "specced but unbound (open question)" clause is CLOSED. Everything else here is shipped as written. Original: Fixed v1 bindings: Q/E/R/F slots (weapons switch-to, abilities activate immediately), SPACE-hold refit, 1–4 refit picks (a number key's meaning is evaluated against Space's state at its own keydown; for ~150–200 ms after Space release, number keys resolve as refit-or-nothing — never as a future consumable; closed-window spend behavior dies), W/S telegraph taps (±1 detent, hold does not repeat), A/D rudder hold, Z/X + mouse wheel camera zoom (provisional; fog stays server-authoritative — zoom is never an information exploit), mouse aim/click fire, ESC (closes topmost surface, else opens non-pausing settings), Enter (contextual confirm), M mute; foghorn emote specced but unbound (open question). Key remapping deferred post-beta.
...
UX-DR35: **[TIER-1 SET CORRECTED 2026-08-21 (Story 7.6).]** **"Pip surges" depends on the Listening Ring, which was DEFERRED (Story 4-1, epic-4 amendment 1) and never built**, so it is not a channel: `render/attention.ts` deliberately records that **nothing may list it as a Tier-1 input until it exists**. The shipped Tier 1 is **denied pulses (ALL of them — the on-water arc/marker pulse AND the per-slot hotbar pulses, the table's "denied pulses" read as the plural it is) plus the HP rail's pulse in its CRIMSON band (< 25%)**. One further clarification from the same module: a Tier-2 channel does NOT hold under another Tier 2 (the hold rule is scoped to Tier 1), while Tier 3 freezes under ANY higher tier — the asymmetry is deliberate. Original: Attention-priority arbitration: Tier 1 threat (pip surges, denied pulses, HP pulse < 25%) always animates; Tier 2 match-state (final-10s ring pulse, storm vignette) holds at lit keyframe while Tier 1 is active; Tier 3 economy (chip breathing, toasts, XP wrap) freezes at dim keyframe while any higher tier is active; only the highest-tier active amber channel pulses.
UX-DR36: **[DELIVERED, WITH TWO CORRECTIONS 2026-08-21 (Story 7.6)]** — the sound map **SHIPPED** as `client/src/audio/twinMap.ts`: `TONE_TWINS` is a `Record<AudioCueId, string>`, so the two-column audio-event ↔ visual-twin table is **exhaustive at the type level** — a new cue cannot compile without its row, which is a stronger guarantee than the requirement asked for. Two figures are stale: the set is **33 tones / 34 audio cues**, not 13 — `ToneId` (`client/src/audio/tones.ts:20-53`) is a 33-member union, and `AudioCueId` is `ToneId | 'foghorn'` (`twinMap.ts:29`), so the **type-level exhaustiveness pin covers 34 cues while the TONE set is 33**. The distinction is load-bearing, not pedantic: the foghorn is a cue with a visual twin but is not a tone — it is not synthesized through the tone system. And **the Listening Ring named here as the mono-audio backstop was DEFERRED and never built** (Eric 2026-08-04, re-affirmed 2026-08-21: *"very deferred. sonar might come back in the future, but radar is plenty deep enough"*), so the shipped bearing surface is the **foghorn chevron**, and the mono-audio backstop is each cue's own visual twin. Original: The audio system's sound-event map (open deliverable) must ship with a two-column audio-event ↔ visual-twin table; no audio event ships without its row; the 13 existing tones are the template; mono-audio users rely on the listening ring as the visual backstop.

**Game feel (carried + added)**

UX-DR37: Carried juice inventory: directional screen shake on own damage (4→16 px by hit size, exponential decay), 130 ms white hull flash on struck contacts (300 ms same-source floor), amber hit spark vs miss splash, crimson sink ring, denied 80 ms pulse, 13-tone WebAudio set — all mute-aware, all governed by the motion setting. **[FIGURE CORRECTED 2026-08-21 (Story 7.6)]** — the WebAudio tone set is now **33 tones**, not 13 (`ToneId`, `client/src/audio/tones.ts:20-53`). The audio system carries **34 cues** in total: `AudioCueId` is `ToneId | 'foghorn'` (`twinMap.ts:29`), and the foghorn is a cue with a visual twin but is not a tone — it is not synthesized through the tone system, so it sits outside the tone set this inventory counts while still being covered by UX-DR36's type-level twin pin. The mute-awareness and motion-governance laws are unchanged.
UX-DR38: Death sequence presentation: ~5 s sinking ritardando (helm accepted but decaying, guns live) → omniscient reveal (fog drops, camera zooms out, nameplates on all; only Enter/click proceeds; BR chrome + kill feed persist, hotbar/XP/chip/vitals/ring die with the hull) → results modal. **[OPEN QUESTION RESOLVED 2026-08-21 (Story 7.6) — Eric ruling 2026-08-13, epic-5 amendment 26, closing UX open question #25.]** The reveal zoom is **NOT EXEMPT** from the motion setting. **Motion scales the DURATION, never the destination**: at `motion: off` the camera SNAPS to the identical whole-map framing on the frame the animation would have started, so nothing about the reveal is lost — only the travel. Three facts decided it: nothing in the game is exempt from `motion` (an exemption here would have been the first of its kind — the two exemptions that DO exist are from the flash budget and from attention tiering, not from `motion`); the setting's own standing law is *"`off` removes motion, never information"*; and exempting it buys nothing.
...
**Epic 8 · Epic 9 — the v3 surfaces (DESIGN.md + EXPERIENCE.md, updated 2026-09-10/11; mocks `hud-composite-3`, `countdown-results-1`, `shipdeck-1`; ~55 Eric rulings in `.decision-log.md`) — added 2026-09-11.** Where the re-rendered mock and the written spine disagree (helm globe 104 vs 132 px; compact tile 138×84 no-scroll vs 130×96 + scroll), the SPINE wins. Every `[ASSUMPTION]` in the docs is a facilitator draft for Eric's redline, not a decision.

**In-match HUD — the bottom-centre bar (E8)**

UX-DR40: **The HUD Bar (Direction B "TRISTRAM", ratified 2026-09-11: *"YES THIS IS PERFECT!"*).** ONE bottom-centre Pixi cluster replaces the bottom-left hotbar stack AND the bottom-right own-vitals cluster; nothing renders bottom-left, nothing on the right edge; the BR chrome bar (top-centre) and kill feed (top-right) are UNCHANGED. 18 px above the viewport floor; ~790 px wide `[ASSUMPTION, follows the grown helm globe]`; gaps 16 px globe↔slots, 8 px slot↔slot. Order left→right: **HP globe · gun slot (keyless) · `Shift` slot · Q E R · framed belt 1–4 · helm globe**, with the **XP strip full-width beneath**. Afterimage register — no shared panel; only the two globes carry their own `rgba(3,6,5,.7)` bed. **The whole bar — slots, belt, XP strip, both globes — dies with the hull at the reveal** (survivor set: chrome bar + kill feed). UI scale applies to it; corner anatomy never rearranges. **Bar occlusion astern is ACCEPTED, not a defect** (~240 px of water directly astern covered only at maximum zoom-in; zoom is the remedy; no rule, no token).
UX-DR41: **Hotbar Slot anatomy and state grammar.** 54×54, 1 px outline `rgba(192,192,192,.28)`, transparent fill, sharp corners, icon in `currentColor` inline vector; **key chip centred BELOW the square** (no label column); **tier numeral 9 px/600 mono bottom-right on the absolute loot ramp, drawn ABOVE the cooldown wipe**. States: Idle silver `.28` · Ready weapon phosphor `.4` outline + 10 px glow · Ready Shift phosphor `.65` + 14 px glow `[ASSUMPTION]` · Selected `{colors.amber}` outline + `0 0 16px` glow + inset wash + filled amber key chip (wash + chip are the channel, hue secondary) · Cooling = the cooldown wipe (UX-DR44) · Held fire (UX-DR52) · Activated flash ≤ 80 ms (`.2` wash, full outline, 22 px bloom) · **Empty: 1 px dashed slate `.45` + `—` glyph, NO words** ("— awaiting refit —" is retired; the empty IS the state) — the 0:00 state of every weapon slot and of a depleted belt slot · Denied 1 px→2 px `{colors.denied}` edge pulse + red icon flash. **The activated-ability CHAMFER is RETIRED** (the 9 px cut token goes; belt framing + key chips carry the weapon/consumable distinction). Reloads tick on every slot regardless of selection.
UX-DR42: **The deck gun is SHOWN, keyless, at the head of the row** — its chip renders as a GHOST that keeps the row's alignment; its 5 s reload wears the cooldown wipe like every other slot; it stays the permanently-selected default. **The `Shift` boost is a hotbar SLOT with a `Shift` key chip** in second position, wearing the ready outline and the cooldown wipe — NOT in the helm globe. **Q/E/R are three GENERIC weapon slots, all EMPTY at 0:00** (the per-class fit is retired), filled by drawing a weapon's tier I; a weapon key primes its slot and the same key reverts to the gun; **firing auto-reverts on RELEASE, never on press**; a weapon key mid-stream switches the prime and ends the stream without a denial; a key on an EMPTY slot is denied (suppressed under the held start line, where the lock wins); a click on any slot is that slot's key; Q/E/R/`Shift` are suspended while the refit window is open.
UX-DR43: **Belt Slot (consumables, keys 1–4).** Four 44×44 squares inside ONE 1 px silver `.2` frame (padding 8/8/6, 6 px gaps) — **the frame + key chips ARE the weapon/consumable distinction**. Stocked = ready-weapon phosphor outline + ammo badge `×n` at −6 px overhang; a second copy of a stocked line increments `×n`. **No cooldown, ever.** Empty and depleted are the SAME dashed state — *"empty is empty"* (Eric), no ghost glyph. HULL REPAIR / SHIELD BLOCK / SMOKE SCREEN / CHAFF fire on their key (one physical press = one use; key repeat fires once `[ASSUMPTION]`); DECOY BUOY primes on its key and fires on click into the mine's rear arc (prime replaced by any other prime; cleared by a click outside the arc `[ASSUMPTION]`). **Ammo Badge v3:** 16 px phosphor digit on `{colors.card-scrim}`, 1 px `rgba(0,255,136,.5)` border, top-right −7 px overhang on weapon slots storing > 1; on a belt slot it is the STOCK COUNT — down on use, up when a card stocks it, never on reload; the machine-gun pool reads as the held-fire floor fill, NOT a badge. Which consumable fires vs primes is learned from the hover tooltip and How-to-Play ONLY — no glyph on the face or the slot (Eric 2026-09-11).
UX-DR44: **Cooldown Wipe on every cooling slot** (gun, Shift, Q E R): a dark conic overlay `rgba(3,6,5,.86)` uncovering CLOCKWISE from 12 o'clock as the reload elapses (transparent = elapsed) over a `rgba(3,6,5,.55)` interior with the icon at 40 %; a large centred seconds numeral 18 px/600 mono tabular with a 6 px black shadow, **in tenths under 2 s**. Direction B's dimmed-refill and the shipped 2 px conic perimeter track + quick-info readout are dropped. Dual-coded (wipe + numeral); tier numeral and ammo badge render ABOVE the wipe; `[ASSUMPTION: progress readouts take no attention tier]`.
UX-DR45: **XP Strip (replaces the XP Rail) + Banked-Level Chip re-seated.** A full-width strip 8 px under the slot row: 4 px tall, phosphor `.06` track, 1 px `rgba(0,255,136,.25)` border, `{colors.phosphor}` fill + 6 px glow, filling left→right toward the next level; `LV n` (11 px/600 mono) at its head — **`LV 0` is a rendered state (the countdown)**; the bank chip + `TAB TO REFIT` cue at its tail. The chip is 24×24 at the strip's TAIL (was 30×30 at the hotbar head): 1 px `rgba(0,255,136,.65)` outline, mono count, 2.4 s breathing glow decaying to static after ~10 s unspent, re-arming on a new bank or Tab; hidden at zero; its FIRST appearance is the countdown; the cue reads **`TAB TO REFIT`** (drops `LEVEL UP —`; the toast twin `▲ LEVEL UP — TAB TO REFIT` is untouched) and is suppressed whenever the bank cannot be acted on. **NO draw-pile counter anywhere** — not on the HUD, the chrome bar or the refit window (Eric 2026-09-10: *"You have a tendency to add info no one needs."*).
UX-DR46: **HP Globe (replaces the HP Rail) + the SHIELD BLOCK owner readout.** A 96 px globe at the LEFT end: 1 px silver `.28` ring over a `rgba(3,6,5,.7)` bed, phosphor `.28` fill rising from the floor with a 1.5 px phosphor waterline, phosphor `.35` outer ring; `HULL 212 /250` mono readout centred (18 px/600 + 10 px). `[ASSUMPTION: the rail's ramp — phosphor ≥ 50 % → amber < 50 % → damage-marker < 25 % — and its pulse (~0.5 Hz below 50 %, hard cap 1.1 Hz at ≤ 10 %, opacity-breathing never strobing) carry to the globe's fill unchanged]`; its crimson-band pulse is the Tier-1 channel the rail's was; the low-HP sting carries. **While shielded the HP register reads hull + shield, allowed PAST max — `HULL 312/250` — in a changed colour** (Eric 2026-09-11; `[ASSUMPTION: {colors.info}; amber and the red family may NOT carry it]`), reverting number and threshold colour on deplete or expire; enemies see nothing. **RULED 2026-09-11: the recoloured NUMBER ONLY — no globe ring, no hull ring** (the `shield-ring` component token is therefore unused and should be retired from DESIGN.md). Whether an absorbed hit plays its own cue + twin for the VICTIM stays `[OPEN — Eric]` (today it fires the shooter's Hit Call and the victim reads 0 damage, so no shake/thud/vignette).
UX-DR47: **Helm Globe (replaces the Telegraph Cluster).** RIGHT end, same ring/bed as the HP globe, grown past 96 px to seat its 9 px registers (`[ASSUMPTION: 132 px]` — the spine wins over the mock's 104). Inside: **HDG** 15 px/600 mono (label 9 px muted), **KTS** 9 px (suffix 9 px muted); the **9-detent telegraph as a TICK ARC over the crown** (±70° at 17.5° steps, silver `.45` ticks, STOP tick at 12 o'clock brighter) with the ORDERED rung a **hollow 7×10 phosphor outline** and ACTUAL speed a **solid amber 2 px needle + arrowhead** (ordered vs actual shape-coded, never colour alone); **rudder track along the floor** (48 px silver hairline, 2×8 amber tick). W/S chips at the crown sides, A/D at the track ends, 9 px mono muted, visible at the start line and fading permanently after the first few successful inputs.
UX-DR48: **Slot Tooltip v3** (236 px, `rgba(10,15,12,.97)`, 1 px silver `.4` border, pointer notch): a weapon slot lists its drawn line + tier, a belt slot its consumable + stock; the `BOONS ACCRUED` divider copy is `[OPEN — Eric]`; it is also the refit card's hover-only explanation panel `[ASSUMPTION: same panel]`. Hover-only stands (keyboard-only is out of scope). **Toast copy:** `card fitted` / `consumable stocked` replace "boon fitted"; self-events only.
UX-DR49: **Retirements the bar carries — delete the machinery, not just the text:** XP Rail → XP Strip; HP Rail → HP Globe; Telegraph Cluster → Helm Globe; **the DAMAGE CONTROL rail, the digit `5` chip and `HEAL_CHOICE`**; the activated-ability chamfer (DESIGN.md Shapes + Do's/Don'ts struck); Composition-2's bottom-left stack. Component tokens RETIRED: `xp-rail`, `hp-rail`, `damage-control-rail`, `telegraph-cluster`, the 9 px chamfer cut. **Audio-visual twins move with them**: `TONE_TWINS` stays exhaustive at the type level over every new cue; E8 re-points rows whose surface it retires (`heal` → HP globe + belt count; `point` → `TAB TO REFIT`; `placeBuoy` → the decoy); a shield absorb/expire cue and twin are `[OPEN — Eric]`. **Attention tiers v3:** Tier 1 = denied pulses + the HP GLOBE pulse in its crimson band; Tier 3 = bank-chip breathing, toasts, XP-STRIP wrap; wipe, held-fire drain and shield ring are progress readouts taking no tier `[ASSUMPTION]`.

**Refit card, refit window, countdown offer (E8)**

UX-DR50: **Refit Card face — FIXED 216×226, FIVE stat rows, NO prose (ratified 2026-09-11: *"Yes, that's the card"*).** 1 px outline, `{colors.panel}` bed, 20 px gaps, no backdrop dim. Anatomy top-down: key chip 22×22 overhanging top-left · **ICON** 40 px outlined box, phosphor · **NAME** 15 px/600 sans uppercase · **TIER** five 14×7 rungs lit to current with the next rung glowing + `II → III` numerals 12 px/600 mono on the absolute loot ramp, **the slot BLANK on consumables and add-ons** · **KIND word** 10 px mono `.2em` (WEAPON · UPGRADE · ADD-ON · CONSUMABLE) · **FIVE fixed 17 px rows `STAT current → next`** (label 9 px mono, value 11 px mono tabular, next value phosphor; one row per affected stat; NAVAL MINES' blast and trigger radii on SEPARATE rows; unused rows blank) · a **fixed 14 px reason-word foot** (blank except e.g. `SLOTS FULL`). **NO activation wording ("KEY FIRES / KEY PRIMES · CLICK FIRES") and NO explanatory clause of any kind on the face** (Eric: *"its WAY TOO MUCH INFO"*) — explanation is HOVER-ONLY. Armed: amber outline + glow, chip/name/icon flip amber. Cards never wrap in the refit row (1–4 map spatially). New glyphs (ARMOR shield plate, ACOUSTIC HOMING torpedo + arcs) stand until an icon pass. **This RE-AIMS epic-4 amendment 47's container-fit law at the card face — which it now satisfies by construction — while the tooltip's escape from it is the point.**
UX-DR51: **Loot-tier colour ramp and the Tier-I base.** I `{phosphor}` · II `{info}` · III `{storm-readout}` · IV `{denied}` · V `{amber}` — ABSOLUTE, never normalised; no token minted; a sixth rung is forbidden. It rides the TIER numeral on the refit card, the deck-card tile, the results LOADOUT block and the hotbar slot's corner numeral. **ARMOR / SPEED / TURNING / DECK GUN start at Tier I EQUIPPED** (first card reads `I → II`; four copies → II–V); RADAR SWEEP and RELOAD have no base tier — cards I–V; every ladder tops at V (Eric 2026-09-11). Numerals as text on `panel` clear 4.5:1 at every rung (III ≈ 5.7, IV ≈ 5.5 — darken neither). The UX-DR5 purple carve-out widens accordingly (still chrome only): rung III now also rides the deck-card, LOADOUT and slot numerals.
UX-DR52: **Refit Card GREYED state, and held fire.** Greyed (the server would refuse the pick): face at opacity `.55`; the reason word in the foot boxed, `{colors.silver}`, full alpha (≥ 4.5:1 `[ASSUMPTION]`); a dashed hollow key chip at full alpha — dual-coded, the word is the non-colour channel; **its digit or click SENDS NOTHING** — no spend, no latch, no denied pulse; the card stays in the offer; greyed BEFORE the press; distinct from the spend-latch dim. **Held fire (MACHINE GUN click-and-HOLD) on the slot:** amber outline + amber fill `rgba(255,184,0,.16)` with a 1 px amber top edge **draining along the slot floor** as the pool empties (drawn only while a stream kind is primed and draining — never from pointer level alone); the stream ends on pointer-up (listened on the window, not the canvas), arc exit (ONE out-of-arc denial then silence), an empty pool, Tab opening, or the window losing focus (which also clears held keys); never resumes on close without a fresh press; the prime reverts to the gun on release.
UX-DR53: **Refit window behaviour v3.** Tab toggles; `1`–`4` pick; `5` unbound. **Placement is BAR-RELATIVE:** row bottom = hud-bar top − 8 px → row top **388 px** at 1366×768 with a 96 px globe (a taller helm globe lifts it equally), **344 px during the countdown** (a 44 px lift seats REDRAW); 216×226 cards, 20 px gaps; **the own-hull keep-out is WAIVED live and countdown — cards may cover a hull** (Eric). **Only the SLOT GROUPS (gun · Shift · Q E R · belt) dim to 38 %; globes, helm and XP strip stay at full.** Combat lockout while open: mouse fire suppressed, a held stream ends, Q/E/R/F suspended, `1`–`4` become card keys so the belt cannot fire; helm stays live. A pick LATCHES; the next offer renders in place; rejection = denied pulse, level stays banked, window stays open. Queue pips (8 px squares filled/hollow) + a dashed ghost edge show waiting offers. **On ANY close (Tab, ESC, last spend) the digits are INERT for a short grace (implementer dial)** so a mashed pick cannot spend a belt slot. Banked offers never expire; never-reroll has ONE exception — the countdown REDRAW. **Digits carry TWO meanings by window state, read at keydown:** OPEN = pick that card (a greyed digit sends nothing); CLOSED = fire that belt slot or prime the decoy; `preventDefault` stands in both.
UX-DR54: **Countdown level-zero offer + REDRAW (the mulligan; ratified as rendered 2026-09-11).** Level ZERO is granted at countdown start and the refit window **AUTO-OPENS on its offer** (an edge on the grant, never re-opened per frame; the weighted first draw guarantees ≥ 1 usable card). **ONE `REDRAW` button** (ratified copy) centred under the lifted row: the Primary Button register (amber outline + glow), 30 px tall, 0 18 px padding, mono 12 px/600 `.18em`, `0 0 18px rgba(255,184,0,.28)` glow, **one hollow 8 px pip beside the word (hollow = unspent)** — one-time. Countdown only — gone when the water goes live; Tab still closes/reopens, taking REDRAW with it; bots never redraw. The countdown tag stays `ALL STATIONS LOCKED`; `LV 0`, an empty strip, bank chip 1, Q E R + belt dashed; the bar reads Gun · Shift · seven dashed squares. **Boarding / held start line v3:** `CAPTAINS BOARDING — n ABOARD` + `ALL STATIONS LOCKED`; the locked weapons are the deck gun, Shift and seven EMPTY slots — nothing from the deck is aboard yet.

**Results, on-water, feel (E8)**

UX-DR55: **Results modal LOADOUT block (replaces BOONS ACCRUED / LAST OFFER; ratified 2026-09-11).** Section head `LOADOUT` (ratified copy); the hud-bar's own slot row **as it ended, at scale .72** (54→39 px squares, belt 44→32) with tier numerals on the absolute ramp in the slot corners and stock counts on badges — no names, the glyph is the name; a depleted consumable (`×0`) reads as plain EMPTY; beneath, one line of the five ship ladders `ARMOR III · SPEED IV · TURNING II · RADAR SWEEP II · RELOAD III` (10 px mono, numerals in ramp colours). Brought / drawn / taken lives ONLY in match history (E9). Modal ≈ 735 px tall at 1366×768 — accepted. Everything else on the modal (SUNK banner, `9TH OF 14`, identity line, three tiles, MATCH LOG, SHIPS YOU SANK, SPECTATE + RETURN TO PORT, no re-queue) stands.
UX-DR56: **Hue and shape rules for the new consumables (Eric 2026-09-11).** **DECOY BUOY in the OWNER's hue** for all observers (the Regatta propagation row extends to it; the deleted radar buoy leaves the row — this REVERSES UX-DR7's "the decoy-buoy truth-marker is gone" clause); 50 hp, undying until destroyed; owner-only hp readout `[OPEN — not mocked]`; **radar PAINTS it (Eric 2026-09-11)** through the buoy's footprint path; `[ASSUMPTION: a hit on a decoy fires no Hit Call — unruled]`. **SMOKE SCREEN** puffs are grey (`{colors.wounded-smoke}` family) and differ from wounded smoke by **SHAPE** — a trail of discrete expanding puffs laid astern (5 s; each r40 → 60 u for 30 s), never a plume off a hull, never by colour; occludes SIGHT only; own puffs always visible to the owner. **CHAFF** is neutral — no on-water effect; its 10 fakes are ordinary phosphor blips the owner never sees; **no "chaff active" HUD readout** — the owner-side twin is only the `×n` decrement. CAPTIVE MINES in the owner's hue like mines. All three torpedo lines share `{colors.torpedo}`; the per-line tell, missile trail, flak air-burst, machine-gun tracer and monitor arcing shell are `[OPEN — icon/effects pass]`.
UX-DR57: **Feel obligations and cold states.** A weapon's tier I landing in an EMPTY slot is the biggest "build must be felt" moment of v3: the square fills, its arc exists, the key means something. Wake drafting is FELT with NO HUD readout. New arcs to draw: LIGHT TORPEDO twin sector; HEAVY / SUPERCAVITATING / MONITOR / MISSILE / MACHINE GUN bow sectors; the decoy takes the mine's rear sector; an empty slot has no arc. An empty weapon or belt slot is a dashed square — the empty IS the state; no placeholder text.

**Cross-cutting legibility (E8/E9)**

UX-DR58: **The 9 px floor holds on every v3 register — RAISED INTO, never excepted** (Eric: *"RAISE THEM"*): HDG/KTS labels in the helm globe, the W/S/A/D chips, the slot tier numerals and the tile pip labels all go to 9 px; the helm globe grows a few px; the 90 % UI-scale exemption for micro type stands.
UX-DR59: **One mono key-chip family across every v3 home**: the ghost gun chip, `Shift`, Q/E/R, the belt's 1–4 (and the same 1–4 on refit cards while Tab is open — one family, two homes), W/S at the helm crown, A/D at the track ends; the `5` chip is retired; F is the foghorn and not a slot. **No explanatory wording on a card face or a HUD element — a stat row or a glyph, never a sentence** (Eric 2026-09-11); new-feature explanation goes to How-to-Play only. The never-wrap law is scoped to the in-match row (the Ship & Deck grid wraps 5-wide and scrolls).

**The merged SHIP & DECK screen (E9)**

UX-DR60: **SHIP & DECK — one DOM layer, one surface (ratified 2026-09-11: *"Yes, build from this"*).** Replaces BOTH the shipped class-select layer (scroll rail, ghost card, class frame retired) AND the interim standalone `/decks` page — no route, no `/decks`, no `DECKS` link. A DOM layer over the dimmed home at the class layer's **1318 px** (not the 1100 chrome width); panes left→right: **class tiles 236 · collection grid 718 · deck column 280**; **CONFIRM SELECTION pinned bottom-right (50 px amber Primary Button)**; header `SELECT CLASS · WHAT WILL YOU SAIL?` with the account chip hard right (30 px). Opened ONLY by the deploy buttons / the Class Chip (`SELECT CLASS` / `CHANGE`). Keys carry over: 1–3 / arrows highlight a tile, **Enter = CONFIRM SELECTION** unless a text field has focus, ESC returns home (closing an open dropdown first). The grid is the only thing that scrolls (accepted at the floor). CONFIRM SELECTION returns to home with the Class Chip updated — its sub-line becomes **class · deck name** `[ASSUMPTION]`; a deck is never a forced choice; first-run `SELECT CLASS` with Torpedo Boat pre-focused stands. The ghost "MORE CLASSES IN DEVELOPMENT" card is DROPPED (Eric 2026-09-11) — three tiles, nothing else; a new class appears as a fourth tile when it exists. The layer, the dropdown and the account chip claim no z rung `[OPEN — the z register is not re-cut]`; feed/toasts 900 · modals 1000 · menu 1100 unchanged.
UX-DR61: **Class Tile (LEFT pane).** 236×158, `{colors.panel-deep}`, 10 px radius, three stacked at 8 px: silhouette (44×126 box, identity-board geometry) · class name 16 px/700 · three pip rows SPEED / ARMOR / TURNING (real values on absolute anchors — TB 4/2/4 · BS 2/4/2 · ML 3/3/3; labels **9 px** mono phosphor in a `[ASSUMPTION: 64 px]` column; 13×5 pips) · a 14 px hue swatch top-right. **No Q/E rows, no deck row, no pick button, no fantasy line, no GUN row.** Selected: personal-hue outline + `0 0 22px` glow, silhouette/name/pips in the hue; unselected: hairline + silver linework; hover `[ASSUMPTION]`. **The Color Hoist sits at the FOOT of the tile column** (`COLOR PREFERENCE:` label, wheel order, never implies locking; `[ASSUMPTION: the mock's 14 px swatches vs the 20 px token]`) — UX-DR26's "sole home" moves here.
UX-DR62: **Collection grid + the compact Deck Card tile (MIDDLE pane; ratified: *"compact grid tile"*).** All 29 lines at once, **5 wide**, 718 px, 17×10 px gaps, scrolling under a 70 px `panel` fade (spine: 638 px tall + scroll). Tile face: glyph (phosphor) · name (9 px) · the copies rail — **nothing else**; sharp corners; `[ASSUMPTION: 130×96 incl. the 28 px rail]`. **Hover/focus: the full five-row 216×226 refit card rises over the tile at 1:1** (`[ASSUMPTION: the slot-tooltip panel register]`), its tier slot reading copies owned (`×5`). **The 0.6-scale refit card in the grid is RETIRED** (every register fell under 9 px). **Locked (zero owned):** glyph + name `{colors.text-muted}` (≥ 4.5:1 on panel), a lock glyph on the rail at FULL alpha (the non-colour channel); unlocked IN PLACE by pressing rung 1 — the rail, not the face, is the control (Hearthstone / MTG Arena shape; no armory page).
UX-DR63: **Copies Rail — the per-copy grammar (RATIFIED 2026-09-11).** 216×28 under a refit-card face (no top border, `{panel-deep}` bed); 130 wide on the tile at 1:1 rungs (3 px gaps, 14 px steppers), each rung in a ≥ 16×12 px hit target `[ASSUMPTION]`; the rail never scales with a face. **One 16×8 rung per copy up to the line's cap** (4 px gaps). States: **HOLLOW** (silver `.42` hairline) = unowned · **SILVER-FILLED** (`.42`) = owned · **PHOSPHOR-LIT** = in deck (fill + hue, never colour alone). **Acquire = PRESS, THEN CONFIRM on the SAME rung:** the first press turns it AMBER (amber edge, `rgba(255,184,0,.22)` fill, 8 px glow; the rail edge amber + 14 px glow) and lights `⬢ n` (14 px/600 mono amber; the price flat `[DRAFT]`, Eric's dial); the second press buys that copy. **Unlocking a line IS buying copy 1** — the 14 px lock glyph marks a ZERO-OWNED line, `⬢ n` appears on the PRESSED hollow rung, bought by the confirming press; no separate lock affordance. One currency; copy packs, tiered pricing and buy-or-earn are DECLINED. A pending rung cancels on ESC or any click elsewhere; an unaffordable rung is inert with its price dimmed. `−`/`+` (20 px outlined steppers; 14 px on the tile; `.28` at a bound) are the IN-DECK steppers: `−` dim at in-deck 0; `+` dim at in-deck = owned, or for an equipment line outside the deck at 3/3.
UX-DR64: **Deck Column (RIGHT pane) + Deck Dropdown (NAMED decks).** 280 px, `{colors.panel}` bed `[ASSUMPTION]`, hairline, `{rounded.lg}`. **Head: the deck dropdown + `✎` rename + `×` delete** (16 px mono muted). Rows 30 px: glyph 16 px phosphor · name 13 px/600 · `×n` tabular. **Legality line `39 / 40 · 3 / 3 EQUIPMENT`** (16 px mono; a failing count in `{colors.denied}`). **SAVE** in the Primary Button register at 44 px, unlit (`.4` alpha, no glow) while illegal, lit on a legal edit; a successful save has no confirmation beyond the line settling (copy unruled). Legality = exactly 40 · ≤ 3 equipment lines · caps · owned copies, evaluated client-side by the ONE shared function the server runs at the door. **The dropdown** has ONE home — the column head: deck name 15 px mono `.12em` + phosphor caret, 30 px, hairline, 6 px radius, `panel-deep` bed; menu on `{colors.panel}`, hairline, `{rounded.md}`, 30 px rows, the selected deck in the personal hue with `✓`. Every account starts with the hull's starter as deck 1, default name **`DEFAULT`** (Eric 2026-09-11; was `STARTER`), renameable and deletable; defaults to the account's **last-used deck for that hull**; `+ NEW DECK` `[OPEN — not mocked; copy DRAFT]`; ESC closes an open menu before it closes the screen. **Names are the player's own labels — visible only to their author (class card, deckbuilder, history); the feed stays name-free.** (Eric: *"I want the player to be able to name their deck slots. Selecting it from a dropdown makes the most sense."*)
UX-DR65: **The Ship & Deck edge-case set (Eric 2026-09-11):** (a) `×` delete asks to confirm and **cannot delete a hull's last deck**; (b) **CONFIRM SELECTION with unsaved edits saves if legal, otherwise blocks on the legality line**; (c) a pending amber rung cancels on ESC or any click elsewhere; (d) an unaffordable rung is inert with its price dimmed; (e) **deck names are 1–16 characters under the callsign's own filter**, and a focused rename field suppresses the layer's keys as any text input does. Edits after queuing apply to the next match; the client sends a deck id, never contents.
UX-DR66: **ANONYMOUS Ship & Deck (RATIFIED: *"Yes, exactly that"*) — the same layout, differing by CAPABILITY, never by layout** (Eric: *"I do NOT like that the anonymous starter deck user gets some minimal fucking bullshit in it."*). ABSENT exactly: the deck dropdown, `✎` rename / `×` delete, SAVE, the `−`/`+` steppers, the lock + `⬢` price, the account chip. The column head reads `DEFAULT DECK` static (18 px mono, no caret; was `STARTER DECK` — Eric 2026-09-11), the starter's rows, `40 / 40 · 3 / 3 EQUIPMENT`; the whole grid is visible with the starter's copies filled and lit and everything else hollow; rails inert. Absence, not disabled.

**Account, home, sign-in, settings, history (E9)**

UX-DR67: **Account Chip and the signed-in home.** The chip reads `LV n · n ⬢` (account level · unlock tokens): 14 px mono `.18em` phosphor, the token glyph in sans `{text-primary}`; 34 px tall (30 px in the Ship & Deck header); hairline border, 6 px radius, `panel-deep` bed; **top-right by the settings gear, signed in ONLY**, and hard right in the Ship & Deck header; updates on sign-in, sign-out and every token spent; absent anonymous; `[ASSUMPTION: opens match history]`. **Signed-in home = today's home + the account chip. Nothing else** (Eric: *"DROP `DECKS` FROM THE UNDERPLAY LINKS"*): decks are reached only through the deploy buttons / the chip; the callsign field is unchanged (shows the stored callsign; local seeds the account on first sign-in, the account wins after); the mode buttons still carry no sub-line; Enter still runs SOLO. **The anonymous/signed-in fork itself — whether production allows anonymous play — is `[OPEN — Eric]`; both home states are specified so the fork is capability, not layout.**
UX-DR68: **Sign-In Row (anonymous home only).** `GOOGLE · DISCORD` buttons in the Primary Button register at 30 px `[ASSUMPTION: seated in the underplay block above PRIVACY]`; **renders ONLY while `/liveness` reports `account: true` — ABSENT otherwise, never disabled**; an OAuth popup (`openid` / `identify`, no email scope); a 30-day localStorage token surviving the return-to-port page load; a popup closed/blocked or a provider error leaves the home anonymous and reports on the home status line in the `denied` register `[ASSUMPTION: copy unruled under THE COPY LAW]`; signed in the row is absent and the chip stands. The provider fork is `[OPEN — Eric]`.
UX-DR69: **Settings overlay v3.** Signed in it gains **SIGN OUT** and **DELETE ACCOUNT (with a confirm)**; the binding reference lists the v3 scheme (Q/E/R generic, `Shift`, 1–4 belt, no `5`); **persistence is TWO-STATE — localStorage anonymous (byte-identical to today) · account-backed signed in** (local seeds the account on first sign-in, the account wins after, edits write both, sign-out leaves local as it was); one store, two backends; modals never stack is untouched. Key remapping stays deferred and needs a fresh reason now that "no accounts" is stale `[OPEN — Eric]`.
UX-DR70: **Match History page** (DOM standard page chrome on the modal bed; `[ASSUMPTION: reached from the account chip; no route/entry ruled]`): **own rows only** — brought / drawn / taken with `T+mm:ss` stamps, placement, kills — cursor-paged; enemy decks never; the hidden match pool appears only as the pool cards you drew (Eric 2026-09-11). Row grammar = the kill-feed line: mono 14 px uppercase, class and callsign in text-safe hues, numbers phosphor, columns `date · class · placement · kills · T+ afloat` `[ASSUMPTION]`; loading draws nothing; empty and failed fetch both render as absence `[ASSUMPTION]`.
UX-DR71: **The pre-queue legality gate, and the door backstop (Eric 2026-09-11, superseding the same day's "queue pop-up" ruling — Solo vs AI has no queue pop-up).** **You cannot queue into ANY mode with an illegal deck selected.** On the home screen the selected deck is checked by the ONE shared legality function; while it fails, BOTH mode buttons are refused (unlit, a `denied` pulse on press, never a dead screen) and the reason shows as **ONE word** in `{typography.label}` + `{colors.denied}` beside the deck on the Class Chip `[ASSUMPTION: chip sub-line]`; opening Ship & Deck shows the same word on the legality line. Anonymous players can never hit this (the default deck is always legal). **The door backstop** — the server refusing a deck that passed at home (a copy revoked, a cap changed): the one-word reason on the **home status line** (the channel connection failures already use, mode-agnostic) with ONE action **`SAIL DEFAULT DECK`** in the Primary Button register; no other copy (THE COPY LAW).
UX-DR72: **Privacy and How-to-Play copy obligations.** One added privacy paragraph on signed-in accounts (FR70's contents; Google and Discord named; deck names as stored user-entered text; third-person Hullcracker.io; copy at E9 dev time). How-to-Play's EQUIPMENT and UPGRADING sections re-cut for decks, the nine slots, consumables, `Shift` and the countdown REDRAW, keys as keycaps in the refit-card chip; new-feature explanation goes HERE, never in-game; still NO glossary.
UX-DR73: **Dev/staging host is ACCOUNT-ONLY — a FIRM access-control ruling whose presentation is not designed** (Eric: *"the dev/testing server should be 100% account only so I can control access."*); it composes with the `HC_STAGING_KEY` gate and the module's "inert without `DATABASE_URL`" posture; flagged to architecture and this epics run. **Keyboard-only access is OUT OF SCOPE** (a scope statement, not a floor change — NFR29): the mouse is mandatory to target and fire; no v3 surface specifies a keyboard-only path; hover-only tooltips stand; every reviewer finding premised on a keyboard-only player is WAIVED.
UX-DR74: **Unlock-economy presentation rules.** Unlock tokens gate COPIES for variety, never power (a starter is viable at the top); earned by play, never bought with money; one `⬢` price per copy `[DRAFT]`; **starter deck quantities: already re-cut by Eric, delivered at the deck-rules story**; token count and account level surface inside the deckbuilder at minimum (the chip).
UX-DR75: **Journey C ("Okafor", signed-in Battleship regular) — every beat a facilitator draft for Eric's redline, each implying UI work:** SIGN IN row → Google popup → home returns with the chip and nothing else changed; `CHANGE` opens Ship & Deck on the last hull; the rung-1 unlock sequence (hollow → amber `⬢ 1` → silver); create a second deck and name it; steppers swap lines; `39 / 40` in denied red until legal; SAVE lights; dropdown pick → CONFIRM SELECTION → the chip reads class + deck name; countdown REDRAW unpressed; SHIELD BLOCK press → `HULL 312/250` in the shielded colour → falls back to phosphor; results LOADOUT with `SHIELD BLOCK ×0` reading empty; history rows with a pool card listed; the account level ticks. Failure paths: door refusal → `SAIL DEFAULT DECK`; popup blocked → anonymous home, status line, SOLO still runs on the starter.
UX-DR76: **Design tokens added/retired in v3 (DESIGN.md `components:`).** ADDED (designed, not built): `hud-bar`, `hp-globe`, `helm-globe`, `belt-slot`, `cooldown-wipe`, `xp-strip`, `redraw-button`, `loadout-block`, `shield-ring`, `deck-card`, `copies-rail`, `deck-column`, `deck-dropdown`, `account-chip`, `lock-price`, `sign-in-row`, `match-history-row`, `door-refusal` (18). EXTENDED: `hotbar-slot` (keys / empty / heldFire / tier), `ammo-badge` (belt count), `bank-chip` (24 px tail, countdown), `refit-card` (226 fixed, rows, foot, greyed, placement), `slot-tooltip`, `class-card` (tile), `class-chip` (sub-line), `color-hoist`, `button-primary`, `modal`, `toast`. RETIRED: `xp-rail`, `hp-rail`, `damage-control-rail`, `telegraph-cluster`, the 9 px chamfer cut. **No colour token minted** — the loot ramp reuses phosphor / info / storm-readout / denied / amber; the shield colour is `[ASSUMPTION: info]`.
UX-DR77: **Working words — RATIFIED 2026-09-11 (Eric):** `SAVE`, `SIGN IN`, `SIGN OUT`, `DELETE ACCOUNT`, `SLOTS FULL`, `+ NEW DECK`, `REDRAW`, `LOADOUT`, `CONFIRM SELECTION`. **One change:** the player-facing word for the built-in deck is **`DEFAULT`**, not `STARTER` — deck 1 is named `DEFAULT`, the anonymous column head reads `DEFAULT DECK`, the refusal button reads `SAIL DEFAULT DECK`; "starter deck" survives only as the internal/design term. Still DRAFT: the sign-in failure line.
UX-DR78: **Key-rebinding editor (NEW, Eric 2026-09-11 — not yet designed).** The Settings overlay's view-only binding reference (UX-DR28) becomes EDITABLE: one row per bindable action in the existing key-chip family, click a chip and press a key to rebind, conflicts refused with a `denied` pulse on the clashing row, a RESET TO DEFAULTS action, live everywhere the chips render (hotbar, refit cards, helm globe, How-to-Play keycaps). Two-state persistence per UX-DR69. Copy and layout are a facilitator sketch for Eric's redline — a `gds-ux` mini-pass or an in-story design gate is owed before it is built.


### v3 Supersession Register (Epics 8–9, 2026-09-11)

The text of FR1–FR38, NFR1–NFR19, AR1–AR18 and UX-DR1–UX-DR39 above is left byte-identical (Eric ruling 2026-09-10: extend in place). Where Epics 8–9 change what an existing entry says, the change is recorded HERE rather than by an in-place stamp, so the pre-v3 inventory reads as the record of what Epics 0–7 built. Each row: the existing id → what v3 does to it → the governing new id.

**Functional**
- FR1 "carrying a fitted loadout" → a hull carries a DECK; the fantasy is what its starter deck delivers → FR41, FR56.
- FR2 slot grammar (gun + 2 specials + 1 economy slot) → SUPERSEDED: slotless deck gun + Shift + three GENERIC weapon slots empty at 0:00 + four consumable slots → FR45.
- FR3 standard gun identical → STANDS, and the gun gains three ladder lines (DECK GUN / TURRET / BARREL) → FR52.
- FR4 class loadouts → SUPERSEDED by the three starter decks; speed boost universal on `Shift`; the broadside leaves the Battleship starter; the radar buoy is DELETED; the smoke screen returns as a consumable → FR49, FR53, FR55, FR56, FR57.
- FR5 Equipment interface → STANDS; consumables have NO reload and ride the same interface → FR46.
- FR6 arcs → re-cut register (three 360°, five bow, two twin, one rear shared by mines + decoy) → FR58.
- FR7 torpedoes outrun every hull / never self-hit → the outrun law is RETIRED as a requirement (Eric 2026-09-11: NO FRIENDLY FIRE — own ordnance never damages own hull, so speed is irrelevant to safety); three torpedo LINES; the hydrophone clause stays deferred → FR53, AR49.
- FR8 mines capped per player and globally → SUPERSEDED: no per-player cap, NO room ceiling; shooting any spotted mine detonates it → FR57.
- FR9 shells fly to the click or first obstacle → EXCEPTIONS: the monitor shell ARCS over islands; the missile flies flat and islands block it; the machine gun streams; the precision bonus is DROPPED (never built, never ruled — cycle 126) → FR53, FR58.
- FR10 deception entity → the law has LIVE CASES again: CHAFF fakes returns; the DECOY BUOY draws homing and blocks ordnance → FR55, FR57.
- FR11 movement → + the universal Shift boost, wake drafting, the TURNING ladder → FR49, FR50, FR51.
- FR12 keyboard scope → + `Shift`, `1`–`4` consumables with the window closed, `5` gone, click-and-hold for the machine gun → FR45, FR58.
- FR14 only ships paint on radar → CHAFF fakes paint; the decoy is a physical entity (radar paint unruled) → FR55, AR48.
- FR16 signal set → per-shell `mz`/`hc`/`sp` for every gun-family weapon incl. the stream; new `missile` kind; the reveal's `w` field → AR46.
- FR18 XP → in-match economy STANDS; a second, ACCOUNT layer of XP (placement-scaled, Solo-vs-AI-discounted) is added → FR65. (Pre-v3 staleness noted: PvE fleet fractions have read ¼ / ½ / ¾ since 2026-08-16.)
- FR19 offers → four DIFFERENT card LINES from the player's own 50, equal weight, no categories, level zero at countdown; never-reroll gains its ONE exception (the mulligan) → FR44, FR48.
- FR20 boon catalog v1 "not stat multipliers" → catalog v3: ladders ARE tiers, add-ons are the nature-changers; rarity and exclusivity extinct → FR42, FR51–FR55.
- FR21 off-class abilities in offers / the extra slot → RETIRED: the catalog is hull-agnostic, decks author contents, the acquisition card is gone → FR41, FR42.
- FR22 the build must be felt → STANDS; tier I landing in an empty slot is v3's biggest moment → UX-DR57.
- FR23 hold-SPACE refit, 1–4 pick → `Tab` toggles; `1`–`4` pick with it open and FIRE consumables with it closed; `5` retired → FR45, UX-DR53.
- FR30 PvE fleet fit → a fixed card list (`FLEET_FIT`) applied at spawn → FR45, AR38.
- FR32 heal reserved for a future sinking → alive → the heal is HULL REPAIR, afloat-only; D4's reversibility is NOT reopened → FR47.
- FR33 results (accrued boons + last offer) → results show the FINAL LOADOUT only; brought/drawn/taken → E9 match history → FR60, FR66.
- FR35 / FR36 bots pick classes, consume `observe()` → bots carry authored 40-card decks + the pool; a total consumable-tactic table → FR59, AR50.
- FR38 reconnection → consumable slot contents ride server-owned ship state, so reconnect keeps them for free → FR46.

**Non-functional**
- NFR2 "no install, no account" → "no account REQUIRED" → NFR30.
- NFR4 anti-cheat → extended to deck state, the pool, the door and `MatchRecord` → NFR20.
- NFR5 determinism → wake drafting / smoke / the fold join the parity surface → NFR22.
- NFR6 the ~15:00 estimate and ~17:30 ceiling → re-derived on the heal bound (≤ 10 heals, ≤ 4 stocked) and ARMOR's 450 hp cap → FR47.
- NFR14 "no accounts, no server player DB at beta" → SUPERSEDED: OAuth accounts, the first persistent store, server-side match history (provider + opaque subject id only) → NFR24, NFR26.
- NFR18 / Story 7-7 deferred → STANDS; the account store adds no deployable and must not pre-empt it; the revival brief gains the account-API CORS line → NFR28.
- NFR19 no PII → extended by the log's first PII rule → NFR24.

**Architecture**
- AR1 "work item #0 = Colyseus 0.16 → 0.17" → a SECOND framework upgrade, 0.17 → 0.18, is E9 story 0; the hosting posture (single Render service, vertical scaling, no Colyseus Cloud, no autoscaling) is REAFFIRMED; D8's "Redis arrives with the Colyseus Cloud move" trigger is STRUCK → AR19, AR22.
- AR2 0.17 capabilities → the platform is 0.18; `@colyseus/auth`, `@colyseus/database`, `@colyseus/admin` are adopted; `setSimulationInterval` → `setTimestep` → AR19.
- AR4 D2 boon model (`{ id, category, effects[] }`, `slotReplace`, `behavior` hooks) → AMENDED: lines with tiers, a `stock` kind, `slotReplace` DELETED, `HOOK_REGISTRY` empty and staying empty → AR35, AR36.
- AR5 signal registry → gains `missile` / `missileU` / `smoke` / `decoy` rows, loses `buoy`; the counter-intel carve-out gains a second SOURCE (chaff) → AR46.
- AR7 equipment registry contents → 18 TOTAL rows; `radarBuoy` and the card-fitted speed boost DELETED → AR56.
- AR8 STEP_ORDER → + `stepSmoke` → AR43.
- AR10 new homes → extended → AR56.
- AR12 harnesses → `--deck authored | random-legal` arms, pool per match, live-mine peak → AR50, NFR23.
- AR13 no event bus → REAFFIRMED; the `AccountWriter` is a PORT with a queue, not a dispatcher → AR33.
- AR14 dev tools → + `deckOverride`, `poolOverride`, `matchOverride.mulligan`, `devSignIn`, `HC_DEV_DB=pglite` → AR55.
- AR16 "no torpedo variety (one torpedo design per fit)" → NARROWED to within-a-weapon (Eric 2026-09-03, GDD note 17): three torpedo LINES are legal and one fit may carry two → FR53.
- AR17 backburnered "~4 consumable slots"; post-beta "accounts" and "key remapping" → SUPERSEDED: four consumable slots are BUILT (E8), accounts SHIP (E9, *"go big or go home"*), and KEY REBINDING ships in E9 (Eric 2026-09-11); the rest of AR17 (Hunter class, supply drops, sonar, medals, teams, ranked, cosmetics, private lobbies, spectate-others, premium colours) stands → FR45, FR62, FR72.
- D7 "no accounts, localStorage only" (architecture) → SUPERSEDED in full by D9–D18 → AR22–AR31.

**UX design**
- UX-DR1 tokens → no colour token minted; 18 component tokens added, 5 retired → UX-DR76.
- UX-DR2 size ramp → the HUD readout moves inside the helm globe (HDG 15/600 · KTS 9); several registers RAISED to the 9 px floor → UX-DR47, UX-DR58.
- UX-DR3 activated-ability chamfer → RETIRED; belt framing + key chips carry the distinction → UX-DR41.
- UX-DR4 Z-order → refit placement is now bar-relative; the Ship & Deck layer, dropdown and account chip claim no rung `[OPEN]` → UX-DR53, UX-DR60.
- UX-DR5 purple carve-out → widens (still chrome only) to the deck-card, LOADOUT and slot numerals → UX-DR51.
- UX-DR6 Color Hoist → moves to the foot of the Ship & Deck tile column → UX-DR61.
- UX-DR7 "the decoy-buoy truth-marker is gone" → REVERSED: the DECOY BUOY renders in the owner's hue; the radar buoy leaves the propagation row → UX-DR56.
- UX-DR11 hotbar (four bottom-left slots, chamfer, conic track, "— awaiting refit —") → SUPERSEDED: nine slots on one bottom-centre bar; the clock-wipe; empty = dashed + `—`; new Stocked / Held-fire states → UX-DR40–UX-DR44.
- UX-DR12 XP rail, 30 px bank chip, cue line → XP strip, 24 px chip at its tail, `TAB TO REFIT` → UX-DR45.
- UX-DR13 slot tooltip → lists drawn line + tier / consumable + stock; also the refit card's hover panel → UX-DR48.
- UX-DR14 refit window (`5` for DAMAGE CONTROL, the below-centre band, own-hull keep-out, whole-hotbar dim) → `5` RETIRED; bar-relative 388/344 px; keep-out WAIVED; slot groups only dim; the digit grace; REDRAW; the greyed state → UX-DR50–UX-DR54.
- UX-DR15 own-vitals cluster (bottom-right) → GONE: HP globe + helm globe on the bar; the telegraph as a tick arc; `HULL n/n` with the shield variant → UX-DR46, UX-DR47.
- UX-DR16 / UX-DR17 / UX-DR22 chrome bar, kill feed, nameplates → UNCHANGED, explicitly retained; the feed stays deck-name-free → UX-DR40, UX-DR64.
- UX-DR20 / UX-DR21 on-water grammar → smoke-screen puffs by SHAPE; chaff no effect; decoy in owner hue; three torpedo lines share `{torpedo}`; new weapon effects `[OPEN]` → UX-DR56.
- UX-DR23 toasts → `card fitted` / `consumable stocked` → UX-DR48.
- UX-DR25 home → signed-in adds the account chip by the gear; anonymous gets the SIGN IN row (absent when accounts are not live); no `DECKS` link; the Class Chip sub-line becomes class · deck name; the anonymous/signed-in fork `[OPEN]` → UX-DR67, UX-DR68.
- UX-DR26 class-select layer → REPLACED by the merged Ship & Deck screen (tiles, grid, deck column; CONFIRM SELECTION pinned; 1318 px) → UX-DR60–UX-DR66.
- UX-DR27 results modal → BOONS ACCRUED / LAST OFFER → the `LOADOUT` block; ~735 px accepted; SPECTATE + RETURN TO PORT and no-re-queue STAND → UX-DR55.
- UX-DR28 settings → + SIGN OUT, DELETE ACCOUNT; two-state persistence; the binding reference becomes an EDITOR → UX-DR69, UX-DR78.
- UX-DR29 How-to-Play → EQUIPMENT + UPGRADING re-cut; still no glossary → UX-DR72.
- UX-DR30 boarding → the locked weapons are the deck gun, Shift and seven empty slots; the countdown hosts the auto-open offer + REDRAW → UX-DR54.
- UX-DR31 / UX-DR32 input → Q/E/R generic + empty; `1`–`4` two meanings; `5` unbound; `Shift` tap (Sticky Keys accepted); machine-gun click-and-hold; decoy prime; revert on release; blur clears the stream; "key remapping deferred post-beta" is REVERSED (Eric 2026-09-11) → UX-DR42, UX-DR52, UX-DR53, FR72, UX-DR78.
- UX-DR33 key-chip family → the ghost gun chip, `Shift`, Q/E/R, 1–4 (two homes), W/S/A/D; `5` retired → UX-DR59.
- UX-DR34 settings persistence / UI scale → two-state persistence; UI scale applies to the bar; keyboard-only OUT OF SCOPE → UX-DR69, UX-DR73, NFR29.
- UX-DR35 attention tiers → Tier 1 = the HP GLOBE's crimson pulse; Tier 3 = XP-STRIP wrap → UX-DR49.
- UX-DR36 / UX-DR37 sound map → twins re-pointed (`heal`, `point`, `placeBuoy`); the shield cue `[OPEN]`; the cue count moves with v3 → UX-DR49.
- UX-DR38 reveal survivor set → the WHOLE bottom bar dies with the hull → UX-DR40.
- UX-DR39 anatomy → bottom-centre bar, nothing bottom-left/right; never-wrap scoped to the in-match row; Ship & Deck at 1318 px → UX-DR40, UX-DR59, UX-DR60.
...
FR13: Epic 4 — Sensor suite. **The third (listening) tier is DEFERRED with Story 4-1 (Eric, 2026-08-04); two tiers shipped.**
...
FR17: Epic 4 — The Bounty / **KILL LEADER** (identity-only throne + extra XP; **the bloom was struck** — Story 7.6)
...
### Story 0.4: Ad / Lifecycle Adapter Seam (Null Implementation)

> **RECONCILED 2026-08-21 (Story 7.6).** The story is `done` and its completion record stands
> unaltered below. Its purpose clause pointed at a destination that never arrives: **no portal SDK
> lands at Epic 7** (rescope 2026-08-18; Eric: *"I'm controlling my game and servers. no portals."*).
> The seam's real payoff is what actually happened — it **survived a full change of destination
> untouched**, and Google AdSense H5 Games Ads (Ad Placement API) landed behind it at Story 7.4
> with no retrofit. The interface NAME `PortalAdapter` is retained deliberately per AR11 (renaming
> ratified code to match a changed destination is churn); read "portal SDK" below as "third-party
> ad/lifecycle SDK". Title restated from *"Portal Adapter Seam"* for the same reason.
...
So that a third-party ad/lifecycle SDK lands at Epic 7 behind an existing seam instead of a retrofit.
...
**And** no game code imports any third-party ad/lifecycle SDK directly (there is none to import yet — the seam is the rule).
...
**Then** the server assigns each human a unique hue at match start (preference granted unless contended; contention = fair random draw, losers to nearest free hue) *(assignment clause superseded by Eric ruling 2026-07-23, shipped in 1.12: FCFS at join, nearest-free-clockwise on tie, seeded-random-free — see UX-DR6)*, with the color index riding the roster so every screen agrees (UX-DR6)
...
**And** the kill feed restyles to the ratified spec: mono 14 px uppercase top-right, max 6 lines / 8 s TTL (**figures corrected 2026-08-21, Story 7.6** — `killFeed.ts:23-24`, set by the Public Register, cycle 45; was 5 lines / 6 s), vessel names 600-weight in their text-safe personal variants, drones grey, connective text secondary, long legacy names mid-ellipsized (UX-DR17).
...
**And** the class-select layer shows the three Class Cards on a horizontal rail (silhouette box, SPEED/TOUGHNESS/TURNING pips with values derived from Story 1.3's CONFIG envelopes — real numbers, not placeholders; Eric wants pips as a balancing aid (resolves UX open question #13), two special-slot rows keyed Q/E) *(card anatomy superseded post-merge 2026-07-24: fantasy line + GUN row REJECTED)* plus the dashed ghost card; keys 1–3/arrows highlight, Enter picks, ESC closes without change (UX-DR26)
...
> **DEFERRED — Eric ruling 2026-08-04, re-affirmed 2026-08-21 (Story 7.6).** *"Very deferred. Sonar
> might come back in the future, but radar is plenty deep enough."* This story was never built; the
> acceptance criteria below are the record of what was specced, **not live guidance**. Consequences
> that landed elsewhere: FR13's third sensor tier is reduced to two, AR6 and UX-DR18 are dormant with
> it, the shipped bearing-grade surface is the **foghorn chevron** (Story 4.5), UX-DR36's mono-audio
> backstop is each cue's own visual twin instead of the ring, and Story 4.8's Tier-1 attention set is
> the low-HP rail pulse + live denied pulse with no listening channel. See
> `implementation-artifacts/bmad-dev-auto-result-4-1-listening-ring-deferred.md`.
...
> **SUPERSEDED 2026-08-21 (Story 7.6) — Eric rulings 2026-08-10, epic-4 amendments 216-227.** The
> story SHIPPED, but **without any position disclosure**: the Bounty Bloom was deleted end to end,
> which strikes FR17's *"sanctioned non-sweep radar paint"* clause and retires UX-DR19. The criteria
> below are the pre-ruling record; the shipped behaviour is stated after each.

**Given** a match with a current kill leader
**When** the throne changes hands on a STRICT OVERTAKE of captain-only kills (a tie never transfers it)
**Then** ~~the leader blooms on every player's radar at true position — an expanding ring (1 to 3 px, ~2 s decay) in their personal color around their class blip, visually distinct from sweep paints (FR17, UX-DR19)~~ — **STRUCK.** Shipped: **identity only**, published as `ArenaState.bountyId`, which discloses nothing a client could not already derive from the roster's per-player kill counts. No paint, bloom, ring, bearing, range or area, ever
**And** ~~it lands as a registry row explicitly marked as the one sanctioned non-sweep radar paint, with its invariant case codifying the exception~~ — **STRUCK.** No seventh declared exception to the master perception invariant was needed; the count still stands at SIX (`sp`, `hc`, `mz`, `sunk`, `sm`, `fh`)
**And** killing the bounty holder grants the extra XP (`CONFIG.bounty.killLevels`, +1) through the Epic 2 `grantXp` pipeline — **shipped as written**
**And** ~~the feed announces "BOUNTY: <NAME>" on activation, and the bloom has its audio twin~~ — **RESTATED.** The player-facing name is **KILL LEADER**; the `BOUNTY: ` label is retired. Shipped: a persistent chrome-bar `☠︎ <NAME>` register in the leader's text-safe hue, a `☠︎` mark riding the leader's name in any feed line (as killer OR victim) with the claim register `☠︎ <NAME> IS THE NEW KILL LEADER`, and a `YOU ARE THE KILL LEADER` toast + the `bounty` audio cue when the throne lands on you. Internal naming (`CONFIG.bounty`, `bountyId`, `bounty.ts`, the `bty` wire key) is deliberately unchanged — this was a copy ruling, not a rename.
...
Story numbering matches execution order. The split was deliberately LAST (Eric, 2026-08-18) so beta cut clean at `0.1.0` / `0.1.0` — **superseded 2026-08-21: Story 7.7 is DEFERRED IN FULL, the `0.1.0` cut is off, beta ships as `0.17.X`, and Epic 7 closes at Story 7.8.**
...
> **DEFERRED IN FULL — Eric ruling 2026-08-21.** Neither half ships for beta: no cross-origin split, and no per-service versioning (*"I'm just going to keep one version for the game. I can keep separate server/client/shared versions later."*). The game keeps ONE version number on the existing `0.17.X` scheme, and the single Render web service keeps serving `client/dist` alongside the arena. Static asset serving is revisited **when load makes it a problem**, not on a schedule — revival tripwires and the evidence behind the deferral are in `deferred-work.md`. Deferring is cheap because the client-side seam already exists and ships: `client/src/net/connection.ts:232` already resolves the WS endpoint as `VITE_WS_URL` override > dev default `:2567` > same origin, so a future split touches only server/infra, never `shared/` or the wire contract. The story text below is retained verbatim as the revival brief. **Epic 7 now closes at Story 7.8, which remains in scope for beta.**
...
So that accounts can be built on it without the game ever depending on it to boot.

**This is a FLOOR story on the Story 0.1 / 8.0 precedent — no player-visible change.** The `render.yaml` `databases:` entries land in THIS PR with the code that reads them, never ahead (AR24); the six `sync: false` secrets are set in the Render dashboard BEFORE this PR reaches `main` (NFR27).

**Acceptance Criteria:**

**Given** the game on Colyseus 0.18 (8.0) with `@colyseus/database` present but unmounted
**When** the store foundation lands
**Then** `server/src/account/` exists as an in-process module whose `index.ts` exposes exactly `accountEndpoints()` and `createAccountWriter()`, mounted once in `app.config.ts`; nothing under `game/` imports it and `world.ts` / `match.ts` keep zero persistence imports (AR22)
**And** the module is ABSENCE-GATED on `DATABASE_URL`: without it nothing mounts, `GameDatabase` is never constructed (a pin test asserts no construction without the URL — its default is a silent SQLite file), `/liveness` reports `account: false`, and the game is byte-identical to today; with it set but the database unreachable or a migration failing, the process STILL STARTS with the module OFF and logs `account.disabled { reason }` once (AR23, NFR25)
**And** storage is Render managed Postgres per environment (`hullcracker-db`, `hullcracker-dev-db`; region `oregon`; smallest paid tier) declared under `databases:` in `render.yaml` with `DATABASE_URL` via `fromDatabase.property: connectionString`; no persistent disk on either web service, ever; Render managed backups only (AR24, NFR26)
**And** the data layer is `@colyseus/database` (Drizzle + `postgres`), migrations are checked-in SQL under `server/drizzle/` generated by `drizzle-kit` and run at boot — never `"auto"` in production; `applyRouterDefaults` / the auto-mount is never called; unused plugin tables are not wired; `drizzle-orm`'s release candidate is pinned exactly (AR25)
**And** local dev and the server test suite run the REAL schema on PGlite (`HC_DEV_DB=pglite`) so `npm run check` exercises the real SQL with no Postgres installed; `HC_DEV_DB=pglite HC_DEV_OPTIONS=1 npm run dev` is documented; plain `npm run dev` is accounts-OFF (AR55, NFR26)
**And** the fourth error zone is in place: HTTP endpoints answer Result-shaped JSON (`{ ok, code, reason }`, stable dot.case codes, status carrying only the class); no stack trace, driver message or SQL leaves the process; anonymous joins are untouched by any account failure (NFR25)
...
So that the game can keep things for me — and nothing about my match changes because I did.
...
**And** the session is a JWT `{ id, tokenVersion }` with `expiresIn: '30d'`, kept in localStorage across the return-to-port reload; sign-out bumps the token version so every issued token dies at once (`revocationCheck`) (FR62, AR27)
...
So that I get the same ship on any browser — while an anonymous player keeps today's browser-only behaviour untouched.
...
### Story 9.4: The Ship & Deck Screen

As a captain,
I want one screen where I pick my ship and see my whole card collection,
So that choosing a hull and seeing what I own are the same act — and the anonymous view is the same screen, just without the controls.

**Acceptance Criteria:**

**Given** sign-in (9.2), the catalog (8.1) and the shipped class-select layer
**When** the merged screen lands as ratified (Eric 2026-09-11: *"Yes, build from this"*; anonymous frame *"Yes, exactly that"*)
**Then** ONE DOM layer replaces BOTH the shipped class-select layer (scroll rail, ghost card, class frame retired; `classSelect.ts` and the planned `deckEditor.ts` MERGE) and any standalone `/decks` idea — no route, no `DECKS` link; opened ONLY by the deploy buttons / the Class Chip (`SELECT CLASS` / `CHANGE`); at the class layer's 1318 px; panes left→right class tiles 236 · collection grid 718 · deck column 280; **CONFIRM SELECTION pinned bottom-right** (50 px amber Primary Button); header `SELECT CLASS · WHAT WILL YOU SAIL?` with the account chip hard right; 1–3 / arrows highlight, **Enter = CONFIRM SELECTION** unless a text field has focus, ESC returns home; the grid is the only thing that scrolls (FR64, UX-DR60)
**And** the Class Tile is 236×158 `{panel-deep}` with silhouette · name 16 px/700 · SPEED / ARMOR / TURNING pip rows on absolute anchors (labels 9 px) · a 14 px hue swatch; NO Q/E rows, no deck row, no pick button, no fantasy line, no GUN row; selected = personal-hue outline + glow; **the Color Hoist sits at the FOOT of the tile column** (UX-DR26's sole home moves here); **the ghost "MORE CLASSES IN DEVELOPMENT" card is DROPPED** (Eric 2026-09-11) (UX-DR61)
**And** the collection grid shows all 29 lines at once, 5 wide, scrolling under a 70 px fade; the compact Deck Card tile is glyph · name (9 px) · the copies rail and nothing else; hover/focus raises the full five-row 216×226 refit card at 1:1 with its tier slot reading copies owned; the 0.6-scale card is RETIRED; a locked line (zero owned) is `{text-muted}` glyph + name with a full-alpha lock glyph on the rail (UX-DR62)
**And** the copies rail renders its three states — HOLLOW unowned · SILVER-FILLED owned · PHOSPHOR-LIT in deck — read-only in this story (9.6 wires acquisition, 9.5 wires the `−`/`+` steppers) (UX-DR63)
**And** the ANONYMOUS frame is the SAME layout differing by CAPABILITY, never by layout: the deck dropdown, `✎` / `×`, SAVE, `−`/`+`, lock + price and the account chip are ABSENT (not disabled); the column head reads `DEFAULT DECK` static, `40 / 40 · 3 / 3 EQUIPMENT`; the whole grid is visible with the default deck's copies lit and everything else hollow (UX-DR66)
**And** CONFIRM SELECTION returns to home with the Class Chip updated (sub-line class · deck name); first-run `SELECT CLASS` with Torpedo Boat pre-focused stands; the layer follows the yield rule (no inline `visibility` survives a hidden ancestor — `deferred-work.md:1374`, `:1379`); every register ≥ 9 px; the z register is left as the class layer's (UX-DR60, UX-DR84-equivalent)
**And** client-only, no PV change.

### Story 9.5: Named Decks and the Pre-Queue Gate

As a signed-in captain,
I want several named decks per hull that I build against the rules, and I want the game to refuse to queue me with an illegal one,
So that what I bring is mine, legal before I press SOLO, and loaded by the server — never by my browser.

**Acceptance Criteria:**

**Given** the Ship & Deck screen (9.4), `deckRules` (8.2) and `loadDeckFor` (8.2)
**When** deck authoring and the door's account path land
**Then** `decks (id, user_id, hull, name, cards jsonb [{lineId, copies}], updated_at)` exists with `/api/account/decks` CRUD in `account/decks.ts`; every account is SEEDED with each hull's default deck as deck 1 named **`DEFAULT`** (Eric 2026-09-11; "starter" is the internal word), renameable and deletable; **a hull's LAST deck cannot be deleted**; delete confirms; names are 1–16 characters under the callsign's own filter and are visible ONLY to their author — the feed stays name-free (FR64, AR26, UX-DR64, UX-DR65)
**And** the Deck Column: head = the deck dropdown (ONE home; selected deck in the personal hue with `✓`; defaults to the account's LAST-USED deck for that hull; `+ NEW DECK`; ESC closes an open menu before it closes the screen) + `✎` rename + `×` delete; rows glyph · name · `×n`; the legality line `39 / 40 · 3 / 3 EQUIPMENT` with a failing count in `{denied}`; SAVE unlit while illegal, lit on a legal edit; the `−`/`+` IN-DECK steppers on the copies rail (`−` dim at in-deck 0; `+` dim at in-deck = owned, or at 3/3 equipment lines); legality evaluated client-side by the ONE shared `checkDeck` (UX-DR63, UX-DR64)
**And** the edge-case set (Eric 2026-09-11): CONFIRM SELECTION with unsaved edits saves if legal and otherwise blocks on the legality line; a pending amber rung cancels on ESC or any click elsewhere; a focused rename field suppresses the layer's keys; edits after queuing apply to the next match (UX-DR65)
**And** **THE PRE-QUEUE GATE (Eric 2026-09-11): you cannot queue into ANY mode — SOLO or SOLO VS AI — with an illegal deck selected.** On the home screen the selected deck is checked by `checkDeck`; while it fails both mode buttons are refused (unlit, a `denied` pulse on press) and the one-word reason shows beside the deck on the Class Chip; anonymous players can never hit it (FR63, UX-DR71)
**And** the door's account path: the client sends `deckId` (never contents — a `deck` key is REJECTED by the sanitizer at both doors); `loadDeckFor(userId, deckId, hull)` loads the account's deck, runs `checkDeck` against caps and unlocks, and freezes the 40 ids in the seat; signed in without a `deckId` → the LAST-USED deck for that hull, else the default; module disabled → `account.unavailable` for a token-bearer, silent anonymous for everyone else; **the door backstop** (a deck that passed at home but fails at the door — a copy revoked, a cap changed) refuses with a stable reason, never a silent substitution, and the client shows the one word on the home status line with ONE action **`SAIL DEFAULT DECK`** (FR63, AR32, UX-DR71)
**And** Journey C's deck beats (UX-DR75: create and name a second deck, steppers swap lines, `39 / 40` in denied red until legal, SAVE lights, dropdown pick → CONFIRM SELECTION → the chip reads class · deck name) run end to end on the dev host as this story's acceptance walkthrough, with each `[ASSUMPTION]` Eric redlines recorded
**And** the log gains `deck.rejected { userId, code }`; `/metrics` `account.deckRejected`; one account in two browsers is Tuesday — last save wins, the door freezes whatever it loads; no PV change (`deckId` is a room option, sanitized, not a frame field).

### Story 9.6: Tokens, XP and Per-Copy Unlocks

As a signed-in captain,
I want to earn tokens by playing and spend them one copy at a time on cards I don't own,
So that my collection grows by variety, never by power — and a new player's default deck stays viable against mine.

**Eric gate:** `CONFIG.progression.matchesToCatalog` has NO number — Eric: *"It should take a while, but not too long."* This story PROPOSES a candidate against that sentence for him to confirm and ships nothing invented; the flat per-copy `tokenPrice` is `[DRAFT]`, his dial.

**Acceptance Criteria:**

**Given** named decks (9.5) and the `NullWriter` port (8.19)
**When** progression lands
**Then** `account_progress (user_id, xp, tokens_spent)` and per-copy `unlocks` exist; level and available tokens are DERIVED (`level = f(xp)`, `available = level − tokens_spent`), never stored, so the match-end write is one atomic `UPDATE … SET xp = xp + $1` (FR65, AR26)
**And** `shared/src/sim/progression.ts` + `CONFIG.progression` carry the placement-scaled per-match XP curve, `soloXpFactor` (Solo vs AI discounted), `matchesToCatalog` (the OPEN intent dial, measured over 114 cards) and `tokenPrice [DRAFT]`; one token per account level; `PROTOCOL_VERSION` bumps ONLY if the client reads the block (the `CONFIG.fleet` precedent) (FR65, AR52)
**And** **COPIES are acquired per copy** (Eric 2026-09-11): unlocking a line IS buying its copy 1; every further copy up to the cap is bought individually at the flat price, any order; one currency; no copy packs, no tiered pricing, never bought with money; all three hulls and their default decks are unlocked from day one (the defaults' cards are the initial unlock list); the unlock surface beyond a default is every unhomed line plus every line another hull's default carries (FR65)
**And** the copies rail becomes the control: **PRESS, THEN CONFIRM on the SAME rung** — the first press turns a hollow rung AMBER with `⬢ n` (the price), the second press buys that copy; the 14 px lock glyph marks a zero-owned line and folds into the same gesture; a pending rung cancels on ESC or any click elsewhere; an unaffordable rung is inert with its price dimmed; no armory page; unlocked IN PLACE (Hearthstone / MTG Arena shape) (UX-DR63)
...
**And** the account chip's `LV n · n ⬢` updates on every token spent; the log gains `account.write.ok { matchId, rows }` / `account.write.failed`; `/metrics` `account.writeOk / writeFailed / queueDepth`; the starter-vs-veteran win band is the measurement that unlocks are variety, never power; token count and account level surface on the chip and in the deckbuilder; nothing is ever bought with money (FR61, FR65, UX-DR74).

### Story 9.7: Match History and the Admin Console

As a signed-in captain,
I want to read back my own deck from every match I've played — and as the operator, I want to read everyone's,
So that I learn from what I brought, drew and took, and Eric can see how the catalog performs in live play.

**Acceptance Criteria:**

**Given** the real writer (9.6)
**When** history and admin land
**Then** `matches (id, mode, started_at, ended_at, roster_size, winner_class)` and `match_participants (match_id, user_id nullable FK ON DELETE SET NULL, role, class, placement, kills, deck_brought, deck_drawn [{lineId, atMs}], deck_taken [{lineId, atMs}])` exist and are written by the writer; `MatchRecord.pool` is stored whole (FR66, AR26)
**And** `GET /api/account/history?cursor=` returns ONLY the caller's own rows — brought / drawn / taken with `T+mm:ss` stamps, placement, kills — cursor-paged, keyed on the caller's user id and never on match id, so **enemy decks are never returned by any route**; **the hidden pool appears only as the pool cards that player DREW** (Eric 2026-09-11) (FR66, AR30)
...
**And** the reconnect `sunk` catch-up note (`deferred-work.md:544`) and the quitter-wins question (`:1212`) are checked against what history now persists and either carried or raised; the `endedBy` enum persists only after `:921` is resolved (8.19)
...
So that leaving is one honest action — and the match records stay, with my name gone from them.

**Acceptance Criteria:**

**Given** every account table (9.2–9.7)
**When** deletion lands
**Then** an authenticated `DELETE /api/account` runs ONE transaction: bump the token version FIRST, delete identities, profile, decks, unlocks and progress, delete the `colyseus_users` row, set `match_participants.user_id = NULL` on every referencing row, respond 204; the anonymised match rows are RETAINED (Eric's metrics survive as anonymous rows) (FR68, AR30)
...
So that the scheme fits my hands — saved in my browser if I'm anonymous, and with my account if I'm signed in.

**Design gate:** UX-DR78 is a facilitator SKETCH, not a ratified design. This story opens with a `gds-ux` mini-pass (or an in-story design gate with Eric) for the editor's layout and copy before anything is built.
...
So that every sentence on that page is still a claim about shipped behaviour.

**Acceptance Criteria:**

**Given** every account feature landed (9.2–9.9)
**When** the paragraph lands in `client/src/privacy/policyCopy.ts`
**Then** ONE added paragraph on signed-in accounts states: what is held (provider name + opaque subject id; chosen callsign, colour preference, last class; the settings store incl. key bindings; decks and **deck names as stored user-entered text**; unlocks and progress; per-match rows of own deck, draws and picks with placement and kills), the 30-day session token in localStorage, self-serve deletion leaving match rows with no account reference, that nothing is stored for a player who does not sign in, that the operator's admin console can read these tables, and Google and Discord named as providers (FR70, UX-DR72)
