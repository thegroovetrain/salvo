...
- Signature abilities: speed boost (TB — inherited from the cut gunboat); star shells (BB); the Mine Layer's second special. *(It resolved to the decoy buoy on 2026-07-22, which shipped and was then **replaced outright by the RADAR BUOY** — a stationary, destructible sensor relaying its own radar returns, 2026-08-19. **The decoy role is gone from the game: nothing fakes a ship contact any more.**)* **Superseded by catalog v3 (2026-09-09, E8):** the radar buoy is deleted entirely; the speed boost is the universal `Shift` ability on every hull, not equipment; a DECOY BUOY consumable and CHAFF bring deception back.
...
**Playable deliverable:** a lobby where picking any of the three classes yields a genuinely different ship at 0:00. *(Shipped as fitted loadouts; under v3 every hull spawns with the gun and the universal Shift boost only, and the 0:00 difference is the deck — E8.)*
...
4. Torpedo Boat loadout (tubes + speed boost). *(Catalog v3: the boost is universal on `Shift`; the TB starter runs Light Torpedo · Heavy Torpedo · Machine Gun — E8.)*
5. Battleship loadout (main battery + star shells) — shipped as the long-range cannon, since replaced by the broadside barrage.
6. Mine Layer loadout (proximity mines + second special) — shipped as mines + decoy buoy, now mines + radar buoy. *(Catalog v3: the radar buoy is deleted; the ML starter runs Naval Mines · Captive Mines · Flak Gun — E8.)*
...
**Guardrails (compass vetoes):** no torpedo variety (variant mutations via E2 boons *replace* the slot's design, so the veto holds — 2026-07-19; narrowed to within-a-weapon 2026-09-03, and catalog v3 carries three torpedo LINES — Light, Heavy, Supercavitating — one design per slot), no damage-control parties, no sectional damage.
...
- Boon catalog: Hades-hammer model — stat lifts + doctrine cards that change how a piece of equipment behaves (same slot, added verb; **doctrines are upgrades, never starting kit**) + acquisition cards for the extra slot. **The stat-lift vs build-defining tension is RESOLVED: commons are the stat ladders, rares are the nature-changers** *(v3 restates it without rarity: ladders carry the weapon and its tier bundles, add-ons are the nature-changers)*. **Exclusivity is deleted** — doctrines are independent verb flags that stack, so PHOSPHOR+DAZZLE are buildable *(catalog v3: CAPTIVE MINES is its own equipment line and FOULING MINES applies to Naval Mines only, so the CAPTIVE+PROP-FOULING pairing is gone — R25/R28)*. *(The smoke screen was earmarked here and never built — deferred, then returned as a catalog v3 consumable, E8.)*
...
## E8 — The Deck (v3 + catalog v3)

**Goal:** every match is played on a **deck** — a 40-card authored, hull-labelled, legality-checked deck, plus the match's hidden pool of 10 consumables (50 at queue), that fills three generic weapon slots and four consumable slots through the untouched `Tab` offer — with **catalog v3's content on the water** (Eric's authored catalog, 2026-09-09). Everything in this epic works **anonymously on the starter decks**: signing in (E9) changes what you keep, never what you can do here.

**In scope:**
- **Card model (A):** a weapon is one ladder line (copy 1 = the weapon at base, fitted into a weapon slot — **tier I is the bare weapon**, Eric's standing rule; the remaining copies = authored tier bundles) plus separate one-copy add-ons (doctrine verbs) drawable at any time and holdable ahead of the weapon. Universal lines (ARMOR, SPEED, TURNING, RADAR SWEEP, RELOAD, and the deck gun's DECK GUN / TURRET / BARREL) are ladders on the same rule, each at its own cap (4 / 4 / 4 / 5 / 5 / 4 / 1 / 2). **Copies = tier ceiling.** Add-ons target a weapon family — ACOUSTIC HOMING on Light and Heavy torpedoes, never the Supercavitating (catalog v3 R22) — legal now that the torpedo-variety veto is narrowed to within-a-weapon (Eric 2026-09-03, GDD note 17). The acquisition card and its "taking one burns the rest" rule are retired.
- **The catalog is hull-agnostic; nothing is class-locked.** The catalog's v3 contents are Eric's authored document, `catalog-v3.md` (2026-09-09): **29 lines / 114 cards** — 11 equipment lines, 5 universal ladders, 3 deck-gun lines, 5 add-ons, 5 consumables — with its per-tier stat sheets (§4) as the build reference. This epic builds the model, the hooks the catalog plugs into, **and the catalog's content**: six net-new weapons (LIGHT and SUPERCAVITATING TORPEDO, HORIZONTAL MISSILE, MACHINE GUN, FLAK GUN, MONITOR GUN), CAPTIVE MINES as its own line, the TURNING ladder, the retuned RELOAD ladder (−5%/tier; every reload runs at 75% of base at the cap) with the per-equipment reload step composed before it, the HEAT SEEKING add-on, and the five consumables. **Deletions:** the RADAR BUOY and its GUN / JAMMING doctrines (the jamming fake-blip machinery is reused by CHAFF); the speed-boost equipment (the boost is universal on `Shift`); the per-player live-mine cap and its oldest-eviction (a mine exists until triggered or destroyed — the room-wide ceiling's fate is a build-time call). **Rule changes:** shooting any mine you can see detonates it (Eric 2026-09-04); every `[DRAFT]` cell in the catalog is a CONFIG dial the harness tunes once bots sail v3 decks.
- **The match consumable pool (Eric, catalog v3 R4/R44):** every match rolls 10 consumables at random from the five launch consumables, respecting each line's cap within the pool alone, hidden from everyone, and shuffles the SAME 10 into every deck at queue — deck at queue = 40 authored + 10 = 50. A pre-match server insertion, after the legality check, consumables only. The draw-pile counter counts from 50; the harness rolls the pool per match.
- **The Shift boost as a universal ability (Eric 2026-09-04; R7/R9):** every hull, no card, `Shift` — +25% of hull max speed for 10 s on a 20 s cooldown [DRAFT], scaled by the RELOAD ladder. **Wake drafting as a base rule (Eric 2026-09-04):** a small speed lift inside any hull's wake, magnitude semi-realistic [DRAFT]; trails merge, echoes don't — a shared-sim kinematics change, so `gds-game-architecture` owns its prediction seam.
- **Legal deck**, server-checked once at queue and frozen there. Two composition rules (Eric 2026-09-03): exactly 40 authored cards (final for launch, Eric 2026-09-03; CONFIG dial, band test dropped) and no more than three lines that fit an equipment slot. Ownership bounds the rest (every card unlocked; a line has only its existing copies — its catalog cap: 5 equipment / 4–5 ladder / 1 add-on / 4–1–2 for DECK GUN–TURRET–BARREL / 5 for every consumable (Eric 2026-09-03 and 2026-09-09, GDD note 18)). No other composition requirements (gunboat and zero-heal decks are legal). **Starter decks** per hull are catalog v3's three (TB: Light Torpedo · Heavy Torpedo · Machine Gun + Acoustic Homing; BS: Horizontal Missile · Monitor Gun · Star Shells + Dazzle Shells + Heat Seeking; ML: Naval Mines · Captive Mines · Flak Gun + Fouling Mines; every starter carries Hull Repair ×5, Deck Gun ×1, Turret ×1, Barrel ×2, Radar Sweep ×3, Reload ×3, and its own Armor/Speed/Turning tilt) — ordinary decks passing the same rules against a fresh account.
- **Draw rule: equal weight per card.** No rarity weighting, no class weighting, no pity (the 2026-07-30 soft pity is retired). A card leaves the deck when taken. Exhaustion legal but rare. A **draw-pile counter** on the HUD (Eric: "Yes"), counting down from 50.
...
- **Four consumable slots**, keys `1`–`4` with `Tab` closed (default, Eric 2026-09-03; rebindable controls are a maybe, out of scope here): a consumable leaves the deck on pick; full slots grey the card and the **server refuses the pick** (no instant-use); slot contents are server-owned ship state (reconnect-safe). Engine supports **key-fires** and **key-primes-then-click-fires**; the card face says which. **Content closed for launch by catalog v3 — five consumables, cap 5, no reload** (R36–R39): HULL REPAIR (key), SHIELD BLOCK (key; 100 hp absorbed for 10 s), SMOKE SCREEN (key; a sight-only occluder trail), CHAFF (key; 10 fake radar returns around you for 15 s), DECOY BUOY (aimed into the mine's rear arc; 50 hp, undying, homing bait + physical block). The 09-04 damage-reduction buff is out.
- **Heal as a card:** the `5` key and the DAMAGE CONTROL rail are retired; DAMAGE CONTROL is a stockable consumable — **HULL REPAIR** in catalog v3, cap 5 — with the shipped 100 hp effect, 5 copies per starter deck to start (harness-tuned per hull; the match pool may add up to five more), scarcer than today and never renewable. "Heals during the collapse" closes by construction. *(The fate of the shipped free per-level auto-heal is an Eric call — GDD open note 14.)*
- **The opening:** spawn with the deck gun and the Shift boost only; **level zero at countdown start**; **mulligan** = one free redraw of that offer, countdown only (the single exception to never-reroll); **weighted first draw** guarantees ≥1 actively usable card (a consumable, or any equipment's Tier I); the mulligan redraw carries the same guarantee. Pinned-card spawn is a later CONFIG-gated experiment.
- **Bots:** each of the six profiles carries an authored 40-card deck in the player format (plus the match pool, like everyone), legality-checked at room build; a **total** consumable-tactic table (no consumable ships without a bot use rule — five rows now) and a tactic per new equipment line; the batch-sim harness runs **authored** and **random-legal** deck arms (legal against every catalog cap, pool rolled per match) and pins the bars: starter-vs-veteran win band, torpedo-less TB, pure gunboat, heal-take rate, levels wasted, one-copy appearance rate (re-measured at 50). Balance cycle 1's numbers are void once the Shift boost lands on every hull — rerun before tuning tiers.
...
**Out of scope:** the account, the deck editor, unlocks, match history UI (E9); *authoring* the catalog (Eric's document — this epic builds what it says, and never invents a line or a number); the post-launch candidates R1 ruled off the sheet (Forward Mortar / Hedgehog, Depth Charge, Harpoon, Vertical-Launch Missiles, the hydrofoil boost ladder, the launcher-gated Shkval round, the anchor, the torpedo net); positional slots / per-slot arcs, pinned-card spawn as default, passive per-level heal, draft mode, enemy-deck reveal, ghost decks (parked); deck codes (an unruled proposal); a hand, merging, storefront verbs, rarity weighting, soft pity, instant-use-on-pick, class-locked / deck-manipulation / hull-mod / salvage cards, discard/reshuffle (rejected).

**Dependencies:** E2 (the `Tab` offer and XP pipeline it builds on), E5 (bot profiles), the Eric-authored catalog v3 (`catalog-v3.md`, written 2026-09-09 — satisfied; its `[DRAFT]` cells are harness-tuned, not blockers), and `gds-game-architecture` for wake drafting's prediction seam and smoke as a sight occluder.

**Playable deliverable:** pick a hull, sail its starter deck: spawn with the gun and `Shift`, mulligan the opening offer during the countdown, draw catalog v3's weapons into `Q`/`E`/`R`, stock and spend consumables on `1`–`4` — including ones the match dealt — heal from a card, and see the deck you played in results — identical rules for humans and bots.
...
7. The opening: gun-and-Shift spawn, level zero at countdown, mulligan, weighted first draw; pinned-card experiment behind CONFIG.
8. Bot decks + consumable tactics + harness arms and pinned bars (pool rolled per match; balance rerun after the Shift boost).
9. Own-deck results view (brought / drawn / taken) and the server-side per-match deck record.
10. The match consumable pool: per-match roll respecting caps within the pool, hidden composition, the same 10 shuffled into every deck after the legality check.
11. Catalog v3 content — the ladders and the deck gun: ARMOR / SPEED / TURNING (new) / RADAR SWEEP / RELOAD (retuned, with the per-equipment reload step composed before it); DECK GUN / TURRET / BARREL.
12. Catalog v3 content — the equipment lines and add-ons: the three torpedoes (Light twin-sector, Heavy, Supercavitating), Naval Mines without a live cap + Captive Mines as a line, Horizontal Missile (flat, island-blocked) + Heat Seeking, Machine Gun (held-fire stream), Flak Gun (air-burst that hits ordnance), Monitor Gun (arcing, no burst), Broadside Gun, Star Shells + Dazzle / Phosphor; Acoustic Homing across Light and Heavy; Fouling Mines on Naval Mines only.
13. Catalog v3 content — the consumables: Hull Repair, Shield Block, Smoke Screen (a sight occluder), Chaff (reusing the jamming fake-blip machinery), Decoy Buoy (undying, homing bait + physical block); the radar buoy and its doctrines deleted.
14. The Shift boost as a universal ability on every hull; retire the speed-boost equipment.
15. Shoot-any-spotted-mine, and wake drafting as a shared-sim base rule (architecture-gated).

**Guardrails:** the `Tab` offer is untouched (no hand, no second clock, no storefront); the passive XP tick stays the anti-snowball floor; the master perception invariant keeps exactly six exceptions (CHAFF rides the jamming carve-out — a fake discloses nothing real); never invent a card, a number or a consumable without Eric — every `[DRAFT]` in the catalog is his to confirm, and the harness tunes, never re-designs.
...
- **Unlocks:** a whole line per unlock (every copy up to the line's catalog cap — 5 equipment / 4–5 ladder / 4–1–2 deck gun / the one copy of an add-on), flat price [DRAFT], any order; all three hulls and starter decks unlocked day one; the starter decks' cards are the initial unlock list. Variety, never power.
- **Earn:** account level → one unlock token per level; XP per match **placement-scaled**, **Solo vs AI discounted**. Both dials derive from one OPEN intent number — matches to unlock the launch catalog — shipped as a CONFIG dial with no placeholder (Eric 2026-09-03), set now that the catalog's line count is known (29, catalog v3) and tuned from live history. Catalog breadth target ~100 cards / ~25–30 lines — MET by catalog v3 (114 cards / 29 lines, hull-agnostic).
