...
- **Legal deck**, server-checked once at queue and frozen there. Two composition rules (Eric 2026-09-03): exactly 40 authored cards (final for launch, Eric 2026-09-03; CONFIG dial, band test dropped) and no more than three lines that fit an equipment slot. Ownership bounds the rest (every card unlocked; a line has only its existing copies — its catalog cap: 5 equipment / 4–5 ladder / 1 add-on / 1–2 deck-gun / 5 for every consumable (Eric 2026-09-03 and 2026-09-09, GDD note 18)). No other composition requirements (gunboat and zero-heal decks are legal). **Starter decks** per hull are catalog v3's three (TB: Light Torpedo · Heavy Torpedo · Machine Gun + Acoustic Homing; BS: Horizontal Missile · Monitor Gun · Star Shells + Dazzle Shells; ML: Naval Mines · Captive Mines · Flak Gun + Fouling Mines; every starter carries Hull Repair ×5, Deck Gun ×1, Turret ×1, Barrel ×2, Radar Sweep ×3, Reload ×3, and its own Armor/Speed/Turning tilt) — ordinary decks passing the same rules against a fresh account.
...
14. The Shift boost as a universal ability on every hull; retire the `speedBoost` equipment.
...
- **Unlocks:** a whole line per unlock (every copy up to the line's catalog cap — 5 equipment / 4–5 ladder / the one copy of an add-on), flat price [DRAFT], any order; all three hulls and starter decks unlocked day one; the starter decks' cards are the initial unlock list. Variety, never power.
- **Earn:** account level → one unlock token per level; XP per match **placement-scaled**, **Solo vs AI discounted**. Both dials derive from one OPEN intent number — matches to unlock the launch catalog — shipped as a CONFIG dial with no placeholder (Eric 2026-09-03), set now that the catalog's line count is known (29, catalog v3) and tuned from live history. Catalog breadth target ~100 cards / ~25–30 lines — MET by catalog v3 (111 cards / 29 lines, hull-agnostic).
