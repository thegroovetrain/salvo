...
- **Legal deck**, server-checked once at queue and frozen there: exact size (CONFIG min/max dials — a 25–40 band MUST be tested before 40 is final); per-line copies ≤ max (5 ladder / 1 add-on; consumable cap unruled — GDD open note 18); every card unlocked; no composition requirements (gunboat and zero-heal decks are legal). **Starter decks** per hull are ordinary decks passing the same rules against a fresh account.
