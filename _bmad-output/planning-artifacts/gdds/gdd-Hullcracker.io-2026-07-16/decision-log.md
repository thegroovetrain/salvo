...
## Brainstorm outcome — 2026-09-04 (catalog v3 content session; recorded 2026-09-09)

Source: `_bmad-output/brainstorming-session-2026-09-04.md` (15 equipment lines mined from real naval weapons; 41 entries). Not logged here at the time — the session's own next-step list commissioned this GDD update to record its structural rulings. Recorded now, as Eric ruled them, with what the 2026-09-09 catalog then did to each:

- **Shift boost universal (Eric):** *"SPEED BOOST BECOMES A UNIVERSAL ABILITY ON SHIFT — no longer equipment; every hull has it."* A universal-ability layer *"needs a key or a new concept entirely around universal abilities."* → Catalog v3 R7/R9 fixes the numbers (+25% of hull max speed, 10 s, 20 s) with no card; the session's HYDROFOIL BOOST LADDER never reached the sheet (R1).
- **Radar buoy → consumable (Eric):** *"the RADAR BUOY becomes a CONSUMABLE (→ JAMMING/GUN BUOY add-ons lose their host; ML starter loses its signature ability)."* → Superseded by catalog v3 R1: the radar buoy is gone entirely and the DECOY BUOY consumable replaces it (R36); the NOISEMAKER find (a buoy that attracts torpedoes) is the decoy's ancestor.
- **Torpedo split (Eric):** HEAVY TORPEDO = the shipped torpedo re-read (~65 kn, bow-fired); LIGHT TORPEDO (~45 kn, side arcs); SUPERCAVITATING TORPEDO (~200 kn straight-runner) *"specced as its own line 'just in case'"* AND as a launcher-gated consumable. *"Torpedo veto narrowed to within-a-weapon: separate torpedo lines are allowed; in-weapon selectors stay banned."* → Catalog v3 keeps all three as lines (R17–R19) and drops the consumable half (R1: the sheet is the launch catalog).
- **Wake drafting — a base RULE, not a card (Eric):** *"any hull's wake, magnitude semi-realistic (facilitator: a few percent; the wake trail merges, both hull echoes stay, so it is not a hide mechanic)."* Eric: *"Interesting! I like that, we can fuck with that a bit."* → Not on the sheet by construction; recorded in the GDD under Movement with the magnitude [DRAFT]; architecture owns the prediction seam.
- **Shoot-any-spotted-mine (Eric):** *"SHOOTING ANYONE'S SPOTTED MINE DETONATES IT — extend the shipped owner-only rule to any mine you can see. 'Not a bad idea.'"* → Recorded in the GDD's mine rules; it is the counterplay that makes catalog v3's uncapped mines (R23) survivable.
- **Missiles (Eric):** *"two SEPARATE lines (36, 37). Homing is perhaps an add-on; mechanics unknown — 'I just know I want them in the game.'"* → Catalog v3 ships the HORIZONTAL line (R29, flat flight, island-blocked) with HEAT SEEKING as the add-on (R32); VERTICAL-LAUNCH is a post-launch candidate (R1), and the over-the-island role went to the MONITOR GUN (R30).
- **Also ruled that day and carried:** consumables settled at three (superseded by R4's five); no dud chance on anything; no endure column — *"Armor IS HP"*, fire does not enter the game, *"this is deliberately more arcadey than World of Warships"*; all eight facilitator gun add-ons *"extremely bad"*; CAPTIVE MINE = its own line, DEPTH CHARGE = a line, SMOKE SCREEN and CHAFF = consumables, SEARCHLIGHT dead; "signature abilities" are not a concept — starter decks are built to a fantasy. Murat's caution that a universal Shift boost voids balance cycle 1's numbers is carried into the GDD's harness bars.

---

## Update pass — 2026-09-09 (catalog v3, gds-gdd Update mode)

Change signal: `catalog-v3.md` in this folder (Eric's authored card catalog: his spreadsheet, sheet v2, walked line by line on 2026-09-09 — 29 lines / 114 cards, three 40-card starters, 44 rulings incl. the match consumable pool; its §5 lists what this update must carry). Run as a background job with no live Eric prompt, on the 2026-09-03 precedent: Eric's R-rulings are applied as decided, every `[DRAFT]` cell stays a draft, and every facilitator call made here is named as such below. Not a fundamental change — pillars and core loop stand; this is content reconciliation plus one new mechanic.

**The new mechanic (Eric's):** THE MATCH CONSUMABLE POOL — every match rolls 10 consumables at random from the five launch consumables, respecting each line's cap within the pool alone, hidden from everyone, and shuffles the SAME 10 into every deck at queue: 40 authored + 10 = 50. GDD note 8's 40 is the AUTHORED size; the legal-deck rules are unchanged (the check runs on the 40, the pool arrives after it, consumables only). Recorded as its own paragraph under Consumables, and threaded through the loop, slot grammar, deck model, healing bound, matchmaking, anti-cheat, bots, economy, E8.

**Reversals / supersessions applied (all Eric's, by catalog v3 or the 2026-09-04 session):**

- The 2026-09-03 loadout table (signature weapon + signature ability per hull) → catalog v3's starters: three equipment lines + one add-on + a ladder tilt per hull, the same 15 universal cards in every starter; the Broadside Gun leaves the Battleship starter (unhomed); the speed boost is the universal `Shift` ability; the Radar Buoy is DELETED entirely (equipment, consumable, GUN / JAMMING doctrines) — the 2026-09-04 "buoy → consumable" step is recorded as the intermediate state and R1 as the final one.
- *"The decoy role is gone from the game — nothing fakes a ship contact any more"* (2026-08-19) → RETIRED: the DECOY BUOY returns as a consumable (homing bait + physical block, undying), CHAFF fakes radar returns and inherits the jamming machinery as the counter-intel law's live case.
- The smoke screen's 2026-08-21 deferral → RETURNED as a launch consumable (R38).
- CAPTIVE MINES as a doctrine add-on stacking with PROP-FOULING → its own 5-tier equipment line (R25); PROP-FOULING renamed FOULING MINES and applies to Naval Mines only (R28). The Hades-hammer example is corrected; PHOSPHOR + DAZZLE stacking stands (R33).
- "Live cap 5, oldest silently evicted" → NO per-player live cap (R23); the owner-only mine-detonation rule → shoot-any-spotted-mine (2026-09-04).
- "Copies 2–5 are tier bundles / 5 for a ladder / all five tiers" → each line at its own cap (4 / 5 / 1 / 2); tier I is the bare weapon (standing rule); the DECK GUN's tier I is a real step.
- RELOAD −10%/card to 50% → −5%/tier to 75%, scoping the Shift cooldown too (R12/R40); the per-equipment −5% reload step composed before it (standing rule from R14).
- The torpedo veto's "Light, Heavy" → three lines: Light, Heavy, Supercavitating (R17–R19, note 17 confirmed).
- "Torpedoes launch in a bow sector ±30°" → the arc register re-cut: three 360° weapons, five bow sectors, two twin sectors, one rear sector shared by both mine lines and the decoy buoy.
- Consumable content "open" → closed for launch at five (Hull Repair, Shield Block, Smoke Screen, Chaff, Decoy Buoy), cap 5 each; the 2026-09-04 damage-reduction buff is out (R4). DAMAGE CONTROL is named HULL REPAIR.
- "Boon catalog due a v3 re-cut that is NOT yet written" → WRITTEN; note 12 CLOSED; note 11's line count set (29) and breadth target MET (111 / 29); E8's catalog dependency satisfied.

**Additions:** the universal ladders paragraph (ARMOR, SPEED, TURNING — new, RADAR SWEEP, RELOAD) with caps; the Shift boost paragraph; wake drafting as a base rule; the equipment-lines table (11 lines, tier I → tier V, arcs, starters) with the add-on roster, missiles' entry, the Flak Gun's SHRAPNEL GUN lineage, deck-gun ladders and the floor rule; `Shift` in Controls; reference numbers at tier I and the ARMOR-cap hull HP; the match-length ceiling restated at 450 hp and ten heals; the harness bars' pool-per-match and balance-cycle-1-void notes; Out of Scope's post-launch candidates list (R1) and the three items that left the bank; the E8 summary row and full E8 entry re-cut (15 stories); E9's unlock-cap and breadth-met corrections.

**Facilitator calls, named (each tagged inline):** SHIELD BLOCK stops ALL damage sources [DRAFT — Eric did not state the scope]; a SMOKE puff hides its own occupant [DRAFT]; the bot consumable-tactic mapping (heal threshold / disengage posture / rear-arc solve) is a proposal, not a ruling; the match-length arithmetic at the ARMOR cap is arithmetic, not a ruling; "the pool is a pre-match insertion, not a card or a draw weight, so it reopens none of the forge's rejections" is the facilitator's framing of R4; wake drafting's "a few percent" is the facilitator's placeholder and is marked so.

**Conflicts surfaced, not resolved (new open notes 19–21, plus 22 from the validator — see the Finalize paragraph):** (19) the LIGHT TORPEDO at 45 u/s against the "torpedoes outrun every hull / never self-hit" law — R18's numbers stand, the law needs Eric's re-reading; (20) the match pool at its edges — post-match reveal of the pool, and the pool dealing cards an account has not unlocked (both facilitator readings stated inline); (21) the catalog's `[DRAFT]` cells and two build-time calls (the room-wide mine ceiling; the missile's en-route hit damage). Note 14 (auto-heal) is unblocked, not resolved.

**Not carried, deliberately:** the 2026-09-04 session's LAUNCHER-GATED CONSUMABLES category (stranded when R19 dropped the Shkval consumable); the session's "15 equipment lines / three consumables" counts (the sheet governs: 11 / 5); catalog §4's intermediate tier numbers (the catalog is the per-tier authority; the GDD carries tier I and the cap); code identifiers from catalog §5 (`CONFIG.mine.maxLive`, `cooldownScale`, `addMine`) — stated as design consequences, not as code.

**Files touched:** `gdd.md` (frontmatter → updated 2026-09-09; Background + reference key, Pillar 3 steer, Core Loop step 5, Slot grammar 1–3, Movement + three new paragraphs, Radar returns' live case, Eighths ladder, Upgrade economy, The Deck Model v3 (lead, card model, draw rule, counter, legal deck), Hades-hammer, Consumables + the match consumable pool, Healing, Controls, Fitted loadouts + table, smoke screen, Mine Layer kit + bullets, Broadside + the equipment-lines table, weapon laws, Arcs, Flight rules, Enemy Design and AI, Multiplayer matchmaking, Spending, The opening, Account progression, Match length, Economy, Technical anti-cheat, E8 row, Gameplay Metrics, Out of Scope (Decoy Ship note, banked + post-launch candidates, smoke line, Rejected, Parked), open notes 1, 3, 8–12, 14, 17, 18 + new 19–21, Dependencies); `epics.md` (preamble, E1 lines 20/35/37/42, E2 line 55, E8 in full, E9 unlocks + earn); `_bmad-output/gds-workflow-status.yaml` (gdd stamp + last_updated). Two Finalize passes (input reconciliation, validation) ran as subagents after the edits; their findings are recorded in the next entry.

**Propagation note:** E8 stories are `gds-create-epics-and-stories` work — the catalog now exists, so E8 is unblocked; wake drafting's prediction seam and smoke-as-occluder go to `gds-game-architecture`; the batch-sim harness must generate the pool per match and re-baseline after the Shift boost (`/balance-sim`, on Eric's ask only).

**Finalize passes (same day, second commit):** an input-reconciliation subagent (catalog v3 + the 2026-09-04 session vs. gdd.md/epics.md) and a validator subagent (checklist, shooter genre guide) ran; findings applied. **Two corrections of record against the change signal itself, both arithmetic, both now fixed IN `catalog-v3.md` with a dated note as well as here:** (1) the catalog's total is **114 cards, not 111** — its own component figures (55 + 22 + 7 + 5 + 25) and §1's cap column both sum to 114; the wrong total had been copied into ten GDD/epics sites and the workflow-status stamp, all corrected. (2) **The Battleship starter carries TWO add-ons** (Dazzle Shells + Heat Seeking, sheet rows 27–28) — the catalog's prose "its 1 add-on" is short for the BS, and the first draft of this update transcribed the prose rather than the sheet, leaving a 39-card starter that fails the GDD's own legality rule (validator BLOCKER); fixed in the loadout table and E8. Other findings applied: Pillar 3's promise contract gains the pool clause (a promised quantity of unpromised consumables); the machine gun's click-and-hold input enters Controls; the Arena island law names the monitor gun's exception; the general mine law no longer claims blast > trigger for the captive; RELOAD is phrased one way across both files (runs at 75% of base); the banked-content list strikes its three departures in place; `speedBoost` leaves epics prose. **Four facilitator over-reaches the reconciler caught are now tagged inline** (the bot consumable-tactic mapping, the key-fires classification of four consumables, the no-cap ↔ shoot-any-mine causal link, "any gun" clears the decoy → the deck gun or a flak burst). **Omissions from the 2026-09-04 session carried:** the endure ruling (*"Armor IS HP"*, fire never enters), the counter filter as a stated balance law, the session's rejections into the Rejected list, its parked items into Parked, the flak gun's canister-vs-air-burst reversal quote, the arcade law's 09-04 restatement, the "signature" term retired per Eric's 09-04 ruling. **New open note 22** (validator WARNING, Eric's to rule): the Battleship's fantasy cell still describes the broadside, which its catalog v3 starter no longer carries. **Note 21 widened** with four more undecided design edges: whether the Shift boost's +25% scales base or the ladder-raised max; the decoy's owner exemption; the machine gun leaving its arc mid-stream; and the signal emissions (flash / splash / hit call) of every new line. Note 19's arithmetic corrected: the light torpedo's tier V (55) only equals the SPEED-capped TB. Deliberately not taken: the validator's suggestion to invent a wake-drafting percentage and radius (tagged [DRAFT] instead — Eric sets it); the smoke expansion's "linear" (rate tagged [DRAFT]); a pre-existing assumption-index/inline-tag mismatch that predates this update.
