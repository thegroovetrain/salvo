...
describe('the radar lattice is pinned beside the protocol version', () => {
  it('radarCellU is 9 at PROTOCOL_VERSION 57 — retune both together, deliberately (34 → 35 was sudden death appending the storm timeline\'s collapse group; 35 → 36 was Story 5.6, the bigger ocean + the self-private Contact.aggro; 36 → 37 was Story 6.3\'s arena-side requeue signal; 37 → 38 was the Intel Range merge; 38 → 39 deleted the FRAGMENTATION CASING card; 39 → 40 merged the two mine-ring cards; 40 → 41 deleted the retired silhouette radar grammar; 41 → 42 was Story 7-5 wave 1, the upgrade-catalog rewrite + the doctrine verb flags; 42 → 43 was wave 2, replacing the cannon with the BROADSIDE BARRAGE and the decoy with the RADAR BUOY; 43 → 44 was the fix cycle giving the radar buoy its OWN tagged scope; 44 → 45 replaced the broadside\'s designed fan with PER-TURRET FIRING ARCS, so the spread emerges from guns that cannot all bear rather than from a tuned width; 45 \u2192 46 deleted the RANGE I\u2013IV card, freezing the eighths ladder at its base for every observer; 46 \u2192 47 was balance cycle 1, doubling hull hp and retuning the broadside and torpedo; 47 \u2192 48 added SunkEvent.kcls, the KILLER\'s hull class and only ever a PvE fleet hull\'s; 48 \u2192 49 re-cut the broadside\'s per-turret arc ladder to zero overlap at tier I; 49 → 50 was the Colyseus 0.18 upgrade, a transport/encoder break with no message shape change; 50 → 51 was Story 8.1, catalog v3 replacing the v2 boon catalog wholesale and OwnShip.boons becoming OwnShip.cards; 51 → 52 was Story 8.5, the nine fixed-role slots widening OwnShip.ammo from 4 to 9; 52 → 53 was Story 8.8, the heal becoming a card — the -1 spend sentinel deleted and hullRepair dealt; 53 → 54 was Story 8.9, the legacy flat-bonus boost equipment id dying into the universal Shift `boost`, with a new CONFIG.boost block the client reads; 54 → 55 was Story 8.10, THE OPENING — the MULLIGAN_CHOICE (-2) redraw sentinel joining SpendMsg and the interim spawn seed deleted, so every hull sails with the gun and Shift alone; 55 → 56 was Story 8.13, catalog v3\'s torpedo and mine ladders — four lines changing kind or existence (supercavTorpedo to the consumable id space, foulingMines into the equipment one, acousticHoming deleted, depthCharge added) plus an own-only MineView.c; 56 → 57 was Story 8.14, THE COMMON POOL — decks and the hidden match pool retired, the seat carrying `gun` and `OwnShip.gun` arriving on the own frame. The lattice itself moved for none of them)', () => {
