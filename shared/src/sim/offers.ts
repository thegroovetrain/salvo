 ...
 * id within each (rng.pick). Deterministic per rng state — the same stream
 * position always yields the same offer, which is what lets the server roll
 * once at earn-time and queue the result.
 */
export function rollOffer(rng: Rng): UpgradeOffer {
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    picked.push(rng.pick(UPGRADE_CATEGORIES[cats[i]]));
  }
