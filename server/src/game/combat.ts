// Gun fire control — the server-authoritative half of the guns system. Firing
// is DERIVED, not commanded (plan): while a ship holds `fire` with guns
// selected, each broadside mount puts a shell out iff its reload has elapsed AND
// the requested aim bearing falls in that mount's arc AND the ship is alive. No
// fire-rate or arc cheat is possible by construction. Pure over a ShipRecord's
// input + kinematics + per-mount cooldowns; returns the shells to spawn and
// resets the mounts that fired. The World owns shell storage + event emission.
