...
describe('decoy buoy — radar deception (the EXACT ship-blip gate, owner-id substitution)', () => {
  ...
  it("an observer whose OWN lit zone covers the buoy gets the truth, never the blip (zone parity)", () => {
    const { w, b } = observed();
    injectDecoy(w, 'd1', 'a', 400, 0);
    w.litZones.set('z1', { id: 'z1', ownerId: 'b', x: 400, y: 0, r: CONFIG.starShells.litRadius, until: 999_999, phosphor: false, dazzle: false });
  ...
  it('WIRE-INDISTINGUISHABILITY (cycle 63): same keys, NO id — and the lie IS the owner hull\'s coverage footprint at the buoy pose', () => {
    ...
    for (const ev of blips) {
      ...
      for (const forbidden of ['id', 'x', 'y', 'ext', 'cls', 'heading', 'speed']) {
  ...
  it("COEXISTENCE GUARD zone tier: an observer whose OWN zone covers the owner's hull gets no decoy blip", () => {
    ...
    const b = place(w, 'b', 0, 0);
    w.litZones.set('z1', { id: 'z1', ownerId: 'b', x: 900, y: 0, r: CONFIG.starShells.litRadius, until: 999_999, phosphor: false, dazzle: false });
    injectDecoy(w, 'd1', 'a', 400, 0); // buoy in b's swept annulus
    windowAround(b, 0);
    const f = buildFrame(w, 'b');
...
describe('decoy buoy — decoys channel tiers (the truth)', () => {
  it('the OWNER always sees its buoy, even far beyond sight; a fogged non-owner gets NO decoys key', () => {
    ...
    const fa = buildFrame(w, 'a');
    expect(fa.decoys).toEqual([{ id: 'd1', x: 1500, y: 0, until: 42_000, own: true, by: 'a' }]);
  ...
  it('a truesighted enemy receives the DecoyView; just outside sight it vanishes (and only ever blips)', () => {
    ...
    expect(buildFrame(w, 'e').decoys).toEqual([{ id: 'd1', x: SIGHT, y: 0, until: 42_000, own: false, by: 'a' }]);
