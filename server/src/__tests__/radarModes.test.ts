...
// flags (amendments 51-57, spec R1/R2/R3): the pure env resolvers (fail-safe,
...
// aspect-only `ext` scalar (amendment 55's anti-cheat bound, pinned), the
...
// ---------- the return grammar (AC5, R2, amendment 55) -------------------------
...
describe('return grammar — the pose deletion and the aspect-only ext scalar', () => {
  ...
  it("AMENDMENT 55's ANTI-CHEAT BOUND (fail-proven): ext is UNCHANGED by granting boons or changing hp/damage state", () => {
