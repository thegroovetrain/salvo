...
function placeClear(world: World, id: string, d: number): void {
  ...
  for (let i = 0; i < 360; i++) {
    ...
    const clear = world.map.islands.every((is) => Math.hypot(x - is.x, y - is.y) > is.r + CONFIG.ship.length);
    if (clear) {
      ship.state.x = x;
      ship.state.y = y;
      ship.state.speed = 0;
...
describe('zone lifecycle — starts ONLY via startZone', () => {
  it('is idle (full map, no storm) until startZone is called', () => {
    ...
    expect(rec.hp).toBe(CONFIG.ship.hp); // idle zone deals no storm damage
  });
...
describe('storm damage', () => {
  it('accumulates at stormDps granularity (4 HP/s => 0.2 per 50ms tick) outside', () => {
    ...
    expect(rec.hp).toBeCloseTo(CONFIG.ship.hp - perTick, 6);
    w.step();
    expect(rec.hp).toBeCloseTo(CONFIG.ship.hp - 2 * perTick, 6);
  });
  ...
  it('emits no per-tick dmg event for storm damage (relies on OwnShip.hp)', () => {
    ...
    w.startZone();
    w.step();
    expect(rec.hp).toBeLessThan(CONFIG.ship.hp); // took storm damage
