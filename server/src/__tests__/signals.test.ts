...
describe('SIGNAL_REGISTRY — materialized key order (msgpack wire shape)', () => {
  ...
  it('wk row (Story 4.12): [k,t,a,gx,gy,w,h,bits] — the ReturnBlipEvent order with the age bucket after t, and NO identity key of any kind', () => {
    ...
    const wire = row.materialize(ctx, seg) as Record<string, unknown>;
    expect(Object.keys(wire)).toEqual(['k', 't', 'a', 'gx', 'gy', 'w', 'h', 'bits']);
    expect(wire.t).toBe(w.now);
  ...
  it('wk row fails closed: the default grammar (P1), spectators (no radar), the sight bubble, an unswept bearing, and a world-emitted wire shape (no `ax`)', () => {
    ...
    // A fabricated world-emitted `wk` WIRE event has no `ax` — dropped.
    const wireShaped = { k: 'wk', t: w.now, a: 0, gx: 55, gy: 0, w: 2, h: 1, bits: [3] };
...
describe('SIGNAL_REGISTRY — sunk: the public register (PV 23, 4th declared exception)', () => {
  ...
  it('materialize appends `bty` LAST in both WITNESSED shapes (Story 4.6): [k,id,by,seen,bty] / [k,id,seen,bty]', () => {
    ...
    w.sinkShip('b', 'a');
    const ctx = foggedCtx(w, c);
    const withKiller = row.materialize(ctx, { k: 'sunk', id: 'b', by: 'a', bty: true }) as SunkEvent;
    expect(Object.keys(withKiller)).toEqual(['k', 'id', 'by', 'seen', 'bty']);
    expect(withKiller.bty).toBe(true); // only ever true, never false
    const noKiller = row.materialize(ctx, { k: 'sunk', id: 'b', bty: true } as SunkEvent) as SunkEvent;
