...
function control(ctx, target, armed) {
  const inp = { seq: ++ctx.seq, throttle: 0, rudder: 0, aim: 0, fireSeq: ctx.fireSeq, aimDist: 0, slot: 2, fireT: 0, actSeq: 0, actSlot: 0, hornSeq: 0 };
  if (ctx.you && target) {
...
function idle(ctx) {
  ctx.room.send('i', { seq: ++ctx.seq, throttle: 0, rudder: 0, aim: 0, fireSeq: ctx.fireSeq, aimDist: 0, slot: 2, fireT: 0, actSeq: 0, actSlot: 0, hornSeq: 0 });
}

/** Full-astern + hard-rudder break-off input for a hull pinned on an island. */
function backOff(ctx) {
  return { seq: ++ctx.seq, throttle: -1, rudder: 1, aim: 0, fireSeq: ctx.fireSeq, aimDist: 0, slot: 2, fireT: 0, actSeq: 0, actSlot: 0, hornSeq: 0 };
}
...
function strafeFire(shooter, target, armed) {
  ...
  const inp = { seq: ++shooter.seq, throttle: 1, rudder: 0, aim: 0, fireSeq: shooter.fireSeq, aimDist: 0, slot: 2, fireT: 0, actSeq: 0, actSlot: 0, hornSeq: 0 };
...
async function main() {
  ...
  try {
    ...
    // THE RULING over real sockets (Eric 2026-08-11, superseding amendment 8):
    // the results table is CAPTAINS ONLY, and placement is captain-relative — so
    // this 2-captain match reads 1st and 2nd, not 1st and 20th. NOT ONE drone
    // row appears, though the ROSTER below still shows a fill of them afloat.
